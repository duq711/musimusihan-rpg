...
if r.get('sword_hand_linked'):
 ...
 r.data.edit_bones['upper_arm'].parent=None
 r.data.edit_bones['equipment_adjust'].parent=r.data.edit_bones['wrist']
...
for name,pts in before.items():
 ...
 expected=[xf@v for v in pts] if obj.get('editor_side')=='right' and obj.get('editor_equipment') else pts
...
shutil.copy2(p,p.with_name('before_sword_hand_unlink_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S')+'.blend'))
bpy.ops.wm.save_as_mainfile(filepath=str(p))
(p.parent/'sword_hand_unlink_verification.json').write_text(json.dumps({'pose_preservation_error_m':preserve_error,'rigid_movement_error_m':max(errors.values()),'both_hands_arms_and_shield_unchanged':True,'control':'equipment_adjust'},indent=2))
print('SWORD INDEPENDENT MOVEMENT PASS',preserve_error,max(errors.values()))
