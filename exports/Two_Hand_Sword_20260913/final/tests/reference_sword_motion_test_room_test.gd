...
func _run() -> void:
	...
	print("REFERENCE SWORD MOTION TEST ROOM %s: five playable fixtures, actual run/jump/landing/collision and target damage, repeated F2, reset and original expedition restoration" % ("PASS" if failures.is_empty() else "FAIL"))
