...
func _update_character_arms() -> void:
	...
	if is_instance_valid(right_relaxed_arm) and right_relaxed_arm.visible:
		...
		if _equipment_draw_reaching():
			...
			var progress := MOTION.smooth_phase(t / 0.46)
			# Cross the left edge while still visible, then sink toward the hilt.
			# Both coordinates advance continuously; there is no intermediate hold.
			var drop := .15 * progress + .85 * pow(progress, 4.0)
			var reach := Vector3(lerpf(.41, -.88, progress), lerpf(-.06, -.70, drop), -.38)
