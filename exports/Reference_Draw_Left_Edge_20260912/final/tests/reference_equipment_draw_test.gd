...
func _run() -> void:
	...
	var reached_visible_left_edge := false
	for i in 25:
		...
		if screen.x < .10 and screen.x > -.10 and screen.y > .0 and screen.y < 1.0:
			reached_visible_left_edge = true
	_check(reached_visible_left_edge, "palm must reach the left screen edge before disappearing below the frame")
	...
	_check(drawn_grip.x < -.50 and drawn_grip.y < -.25, "the held sword must start being drawn from the same left screen edge")
