		...
		for surface in reference.mesh.get_surface_count():
			...
			if valid:
				for index in vertices.size():
					if (restored_frame * vertices[index]).distance_to(reference_frame * original_vertices[index]) > 0.000015:
						valid = false
						break
			if not valid: errors.append("Actual rendered vertices must preserve the supplied sword/hand relationship within 0.015mm: " + source_name)
...
func _test_equipment_and_stowing(fixture: Dictionary) -> void:
	...
	for id: String in ["hunting_bow", "chain_flail", "weathered_staff"]:
		...
		player._update_viewmodel(0.0)
		_check(player.weapon_arm == original_arm and original_arm.is_visible_in_tree(), "switching back must reuse the original supplied grip without duplicates")
