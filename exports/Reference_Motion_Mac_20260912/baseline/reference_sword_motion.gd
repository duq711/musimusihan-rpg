...
## Optional Windows-authored samples. Presentation only: never advances combat,
...
static func _sample_clip(data: Dictionary, time_seconds: float, track: String) -> Transform3D:
	...
	var rotation: Quaternion = before.rotation
	rotation = rotation.spherical_cubic_interpolate_in_time(after.rotation, pre.rotation, post.rotation, amount, interval, pre_time, post_time).normalized()
...
static func _decode_manifest(value: Variant) -> Dictionary:
	...
	if not _finite_number(raw_version) or float(raw_version) not in [1.0, 2.0] or str(manifest.get("status", "")) != "authored_windows_output":
...
static func _metadata_error(manifest: Dictionary, version: int) -> String:
	...
	if not provenance is Dictionary or str(provenance.get("execution_os", "")) != "Windows" or int(provenance.get("exit_code", -1)) != 0:
		return "Reference sword motion requires successful Windows authoring provenance."
