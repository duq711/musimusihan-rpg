extends SceneTree
## Actual production sword rig and Windows-authored trajectories, rendered only
...
const SEQUENCE_IDS := ["idle", "run", "jump", "right_diagonal", "left_reverse", "overhead"]
...
func _run() -> void:
	...
	if not REFERENCE_MOTION.is_available():
		push_error("Windows-authored reference motion must load before visual QA: " + REFERENCE_MOTION.get_load_error())
		quit(2)
		return
	...
	var manifest := {
		...
		"capture_kind": "Actual production DungeonPlayer static glove/sword and shield, with production movement/combat clocks and loaded Windows trajectories",
...
func _capture_sequence(sequence_id: String) -> void:
	...
	if partial_overhead and not motion_only:
		...
		if not bool(sleeve.get("ok", false)) or int(sleeve.get("bone_count", 0)) != 7 or int(sleeve.get("mesh_count", 0)) != 3:
			...
			return
	var action_result := {}
	var movement := Vector2(0, -1) if sequence_id == "run" else Vector2.ZERO
	var sprinting := sequence_id == "run"
	if sprinting:
		for step in range(PHYSICS_FPS):
			await _advance_physics(player, movement, true)
	...
	if sequence_id != "idle" and not sprinting and not bool(action_result.get("accepted", false)):
