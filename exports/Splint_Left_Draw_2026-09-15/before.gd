...
func set_time(seconds: float) -> void:
	...
	var orbit_weight := smoothstep(1.75, SUPPORT_END, elapsed)
	var outward := radial.rotated(_axis, -PI * press).normalized()
	# Keep the original carrying grip. During a turn the wrist rotates with
	# its forearm, instead of bending backwards against an orbiting elbow.
	right_basis = right_basis.slerp(_hand_basis(outward, _axis), orbit_weight)
	...
	var elbow_direction := right_basis.z.lerp(outward, orbit_weight).normalized()
	var wrist_world: Vector3 = right_arm._active_visual.global_position
	var elbow_world := wrist_world + global_basis * elbow_direction * 0.28
	var shoulder_world := to_global(Vector3(0.60, -0.60, -0.35)).lerp(elbow_world + global_basis * outward * 0.30, orbit_weight)
	right_arm.fit_arm(shoulder_world, elbow_world)
	# Separate the actual weighted hand AND sleeves, not just finger joints.
	# The elbow follows the outside of the orbit so the forearm never cuts
	# through the receiving arm on its way to a fixed shoulder.
	if orbit_weight > 0.0:
		var left_extent := _mesh_projection(left_arm, outward)
		var right_extent := _mesh_projection(right_arm, outward)
		var correction := maxf(0.0, left_extent.y + 0.008 - right_extent.x)
		right_arm.position += outward * correction * orbit_weight
