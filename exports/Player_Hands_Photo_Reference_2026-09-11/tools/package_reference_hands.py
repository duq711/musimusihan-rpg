...
def main():
    ...
    destination = project / 'exports/Player_Hands_Photo_Reference_2026-09-11'
    ...
    for source in (stage / 'tools').glob('*.cpp'):
        copy(source, destination / 'tools' / source.name)
    for name in ('verify_hands_realistic.py','joint_verification_core.py'):
        copy(stage.parent / 'player_hands_realism_20260911/tools' / name, destination / 'tools/verification_support' / name)
    copy(stage / 'reference/user_hand_photo.png', destination / 'reference/user_hand_photo.png')
    copy(Path(summary['approved_geometry_input']['path']).with_name('geometry_report.json'), destination / 'validation/geometry_report.json')
    ...
    copy(stage / 'visual_review.json', destination / 'validation/visual_review.json')
    copy(stage / 'mac_output/final_03/metadata_preservation_independent_report.json', destination / 'validation/metadata_preservation_independent_report.json')
