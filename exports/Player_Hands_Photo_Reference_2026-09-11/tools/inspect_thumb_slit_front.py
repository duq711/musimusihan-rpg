...
candidate=sys.argv[sys.argv.index('--')+1] if '--' in sys.argv else 'geometry_02'
source=stage/'mac_output'/candidate/'bilateral_hands_reference_geometry.blend'
...
report={'status':'read_only_diagnosis','source_sha256':original,'target_native':list(target),'source_unchanged':hashlib.sha256(source.read_bytes()).hexdigest()==original,
...
output=stage/'diagnostics'/('thumb_slit_front_'+candidate+'.json');output.parent.mkdir(parents=True,exist_ok=True)
