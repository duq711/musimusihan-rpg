...
const REALISM_CLOSEUP_IDS := ["skin_dorsal_neutral", "skin_palm_neutral"]
...
func _run() -> void:
	...
	var manifest := {
		...
		"gameplay_static_count": POSE_IDS.size(), "neutral_skin_closeups_enabled": closeups_enabled,
		"neutral_skin_closeup_count": REALISM_CLOSEUP_IDS.size() if closeups_enabled else 0,
...
func _capture(viewport: SubViewport, output: String, name: String, inspection: Dictionary, fixture: Dictionary) -> void:
	...
	var joins := HANDS.inspect_active_skin_cuff_joins(fixture.player)
	inspection["skin_cuff_joins"] = joins
	inspection.passed = inspection.passed and joins.passed
	if not joins.passed: failures.append("Actual rendered skin and cuff surfaces separate: " + name)
...
func _capture_neutral_closeups(fixture: Dictionary, output: String) -> void:
	if not configure_pose(fixture, "open"):
		failures.append("Could not prepare the real open hand for neutral skin inspection.")
		return
	var player := fixture.player as DungeonPlayer
	var arm := player.left_support_arm.get("detailed_visual") as Node3D
	var hand := arm.find_child("ContinuousAnatomicalHand*", true, false) as MeshInstance3D
	if hand == null:
	...
	inspection_viewport.name = "NeutralSkinInspectionViewport"
	...
	inspection_camera.name = "NeutralSkinInspectionCamera"
	...
	for pose: String in REALISM_CLOSEUP_IDS:
		var direction := dorsal if pose == "skin_dorsal_neutral" else -dorsal
		...
		var inspection := {
			...
			"subject": "Actual posed left ContinuousAnatomicalHand, same live skeleton, materials and world",
		...
		for mesh: MeshInstance3D in arm.get("arm_meshes") + arm.get("hand_meshes"):
			for surface in mesh.mesh.get_surface_count():
				...
				for texture: Texture2D in [material.albedo_texture, material.normal_texture, roughness]:
					...
					passed = passed and texture.get_width() >= 1024 and texture.get_height() >= 1024
	var roles := HANDS.inspect_active_hand_roles(player)
	return {"passed": passed and bool(roles.passed), "hand_roles": roles, "material_count": materials.size(), "rendered_surface_count": surfaces, "minimum_bound_texture_size": [minimum_size.x, minimum_size.y], "binding_source": "Live bare-skin/nail/cuff/arm materials; source hashes cover GLBs, extracted PNGs and texture import settings"}
...
static func inspect_pose(fixture: Dictionary, pose: String) -> Dictionary:
	...
	result["skin_cuff_joins"] = HANDS.inspect_active_skin_cuff_joins(player)
	result.passed = result.passed and result.skin_cuff_joins.passed
	for side in [-1, 1]:
