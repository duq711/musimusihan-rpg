...
func _test_independent_joints_and_correctives() -> void:
	...
	for side in [-1, 1]:
		...
		_check(bool(HAND_PREVIEW.inspect_hand_roles(arm).passed), "joint review must deform an actual bare hand with all five nails and the retained wrist/arm assembly")
		arm.reset_pose()
		...
		_check(arm.set_digit_flexion("thumb", Vector3.ONE), "the actual thumb must reach its full three-joint pose for wrist seam inspection")
		var thumb_join := HAND_PREVIEW.inspect_skin_cuff_join(arm)
		_check(bool(thumb_join.passed), "full thumb flexion must retain the actual skin-to-cuff join: " + str(side) + " " + JSON.stringify(thumb_join))
		if OS.get_environment("PLAYER_WRIST_QA_DIAGNOSTIC") == "1":
			print("SKIN CUFF JOIN DIAGNOSTIC: " + JSON.stringify({"context": "thumb maximum/" + str(side), "join": thumb_join}))
		arm.reset_pose()
...
func _audit_corrective(arm: Node3D, digit: String, joint: int, context: String) -> Vector3:
	...
	for mesh in _meshes(arm):
		...
		for surface: Dictionary in source.surfaces:
			_check(str(surface.material_name).begins_with("Detailed_Skin"), "joint correction must belong to actual bare skin rather than retained glove trim: " + context)
			...
			for vertex in vertices.size():
				...
				if owned and absf(point.y) < 0.004 and str(surface.material_name).begins_with("Detailed_Skin"):
					section_min = section_min.min(point)
					section_max = section_max.max(point)
					section_samples += 1
				# The smoothed finger web legitimately shares target-digit weights
				# with its neighbour. Require exact zero only where the actual skin
				# palette has no influence from this digit; mixed web deltas still
				# undergo the same magnitude and spatial-locality checks below.
	...
	# Reference geometry keeps the original bone planes while smoothly moving
	# the skin and every corrective together. Measured geometry_03 extents reach
	# 17.60/16.20/8.83 mm for finger root/middle/tip and 17.04/21.99/16.18 mm
	# for the thumb. Keep narrow per-section bounds instead of the old uniform
	# 14.2 mm window; radial locality, magnitude and ownership remain independent.
	var axial_limit: float = [0.018, 0.023, 0.017][joint] if digit == "thumb" else [0.018, 0.017, 0.0142][joint]
	if OS.get_environment("PLAYER_FINGER_JOINTS_QA_DIAGNOSTIC") == "1":
		print("JOINT GEOMETRY DIAGNOSTIC: " + JSON.stringify({"context": context, "changed_vertices": changed, "maximum_delta_m": maximum, "skin_section_samples": section_samples, "skin_section_radius_m": farthest, "axial_extent_m": axial_extent, "axial_limit_m": axial_limit, "unrelated_vertices_changed": wrong_digit, "unrelated_maximum_delta_m": unrelated_maximum, "mixed_web_vertices_changed": mixed_web_changed}))
	...
	_check(section_samples >= 8 and farthest < 0.04 and axial_extent < axial_limit and wrong_digit == 0, "corrective geometry must stay near its skin joint and leave vertices with no target-digit weight exactly unchanged: " + context + " radius=" + str(farthest) + " axial=" + str(axial_extent) + "/" + str(axial_limit) + " unrelated=" + str(wrong_digit) + " unrelated_max=" + str(unrelated_maximum))
