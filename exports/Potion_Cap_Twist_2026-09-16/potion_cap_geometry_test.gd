...
func _run() -> void:
	...
	motion.setup()
	var adapter: Node3D = motion.left_arm._active_visual
	...
	FileAccess.open("res://../exports/Potion_Cap_Twist_2026-09-16/rig.json",FileAccess.WRITE).store_string(JSON.stringify(result))
	motion.queue_free()
	await process_frame
