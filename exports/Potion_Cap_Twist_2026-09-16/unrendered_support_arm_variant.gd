...
func set_time(seconds: float) -> void:
	...
	# A small counter-motion in the supporting hand accompanies each turn.
	var opening_effort := sin(PI * smoothstep(0.98, 1.40, elapsed)) + 0.75 * sin(PI * smoothstep(1.70, 2.14, elapsed))
	bottle_basis = bottle_basis * Basis(Vector3.UP, -0.025 * opening_effort)
...
func _animate_cap(bottle_basis: Basis) -> void:
	...
	elbow = elbow.lerp(to_global(Vector3(-0.34, -0.25, -0.17)), 0.60)
