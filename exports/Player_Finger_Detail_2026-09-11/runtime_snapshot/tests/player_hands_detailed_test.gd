...
func _test_imported_rigs_and_actual_skin() -> void:
	...
	for side in [-1, 1]:
		...
		_check(dense_hand != null and previous_hand != null and _geometry_fingerprint(dense_hand) != _geometry_fingerprint(previous_hand), "the proportion-adjusted glove must retain real detailed hand geometry beyond the original generic source")
		previous.free()
		var skinned_count := 0
