...
p='exports/Potion_Grip_2026-09-15/fit.json';d=json.load(open(p));v=np.array(d['vertices']);g=np.array(d['glass']);print('vertices',len(v),'bones',d['bones'])
...
bones={a[0]:np.array(a[1:]) for a in d['bones']};groups=[v[np.linalg.norm(v-bones[k],axis=1)<.7][::3] for k in ['index2','middle2','ring2','little2','thumb2']]
def sdf(p):
...
def fun(t):
 ...
 return 200*np.mean(np.minimum(dist-.04,0)**2)+np.sum(contact**2)*.8+.002*np.dot(t,t)
rng=np.random.default_rng(2);best=(1e9,None)
for start in range(60):
 t=rng.uniform([-3,-1,-3],[3,1,3]);f=fun(t)
