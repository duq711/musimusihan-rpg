...
func set_time(seconds: float) -> void:
	...
	var tilt := (1.20 + 0.20 * drink + 0.045 * gulp) * sip
	var bottle_basis := Basis(Vector3.RIGHT, tilt).rotated(Vector3.FORWARD, -0.13 * sip)
	var position_target := Vector3(0.17, -0.22, -0.40).lerp(Vector3(0.035, -0.025, -0.10) - bottle_basis * Vector3(0, 5.143 * MODEL_SCALE, 0), sip)
	position_target += Vector3(0.003 * gulp, -0.006 * gulp, 0.004 * gulp)
	position_target.y -= 0.55 * (1.0 - lift + smoothstep(4.95, 5.35, elapsed))
	bottle.transform = Transform3D(bottle_basis.scaled(Vector3.ONE * MODEL_SCALE), position_target)
	var hand_basis := bottle_basis * _hand_basis(Vector3(0.75, -0.60, -0.15).normalized(), Vector3.UP)
	right_arm.transform = Transform3D(hand_basis, position_target - hand_basis * GRIP)
	for digit: String in ["index", "middle", "ring", "little"]:
		right_arm.set_digit_flexion(digit, Vector3(0.68, 0.72, 0.60) + Vector3.ONE * gulp * 0.035)
	right_arm.set_digit_flexion("thumb", Vector3(0.60, 0.65, 0.55))
	right_arm.position += position_target - _grip(right_arm)
	right_arm.fit_arm(to_global(Vector3(0.60, -0.60, 0.02)), to_global(Vector3(0.40, -0.38, -0.05)))
	...
	liquid_up = bottle_basis.inverse() * Vector3.UP
