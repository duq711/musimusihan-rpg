...
const DRINK_MOUTH := Vector3(0.025, -0.025, 0.015)
var bottle: Node3D
...
func set_time(seconds: float) -> void:
	...
	# Bring the neck up and back to the lips first, then raise the base as
	# the flask empties. The opening stays near the face throughout the gulps.
	var tilt := (1.18 + 0.46 * drink + 0.025 * gulp) * sip
	var bottle_basis := Basis(Vector3.RIGHT, tilt).rotated(Vector3.FORWARD, -0.06 * sip)
	# Roll the held flask with the wrist so the fingers wrap across its side
	# as it reaches the face, keeping the body visible beneath the grip.
	bottle_basis *= Basis(Vector3.UP, -1.85 * sip)
