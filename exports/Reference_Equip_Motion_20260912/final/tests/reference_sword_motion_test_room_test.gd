...
const FEATURES := {"motion_sword_draw": "draw","motion_sword_run": "run", "motion_sword_jump": "jump", "motion_sword_sequence": "sequence"}
const STEP := 1.0 / 60.0
...
func _run() -> void:
	...
	print("REFERENCE SWORD MOTION TEST ROOM %s: four playable fixtures, actual run/jump/landing/collision and target damage, repeated F2, reset and original expedition restoration" % ("PASS" if failures.is_empty() else "FAIL"))
