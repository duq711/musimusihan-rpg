...
func _run() -> void:
	...
	sandbox.begin()
	for sequence_id: String in (["draw"] if draw_only else SEQUENCE_IDS):
	...
	var manifest := {
		...
		"qa_scope": "overhead_motion_on_existing_arm" if motion_only else ("overhead_integration_and_existing_motion_regressions" if partial_overhead else ("equipment_draw" if draw_only else "full_motion_delivery")),
...
func _save_contact_sheet() -> void:
	...
	var ids: Array = ["draw"] if draw_only else CONTACT_IDS
