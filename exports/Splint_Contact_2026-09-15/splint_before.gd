...
func _fit_support() -> void:
	# Fit one rigid stick to the taper of the actual receiving sleeve.
	var a := _cloth_frame * Vector3(0, -_sleeve_radius(0.07).y, 0.055)
	var b := _cloth_frame * Vector3(0, -_sleeve_radius(0.235).y, 0.235)
	var forward := (b - a).normalized()
	var side := (_binormal - forward * _binormal.dot(forward)).normalized()
	var up := forward.cross(side).normalized()
	_support_frame = Transform3D(Basis(side, up, forward * (a.distance_to(b) / 0.32)), (a + b) * 0.5 - up * 0.007)
