...
func _point(angle: float, across: float = 0.0) -> Vector3:
	...
	if y > 0:
		# The upper half of the wrap encloses the board on the dorsal forearm.
		...
		var roof_y := lerpf(start.y, finish.y, along) + 0.003 + angle / TAU * 0.0007
		y = cos(theta) * maxf(radius.y, roof_y)
