...
func _run() -> void:
	...
	for i in ceili((player.potion_hands.DRINK_DURATION - 0.05) * 60.0):
		...
		if motion.elapsed > motion.DRINK_START + 0.2 and motion.elapsed < motion.DRINK_END - 0.15:
			...
			_check((mouth - bottom).normalized().dot(Vector3.BACK) > 0.80, "bottle neck points toward the face rather than down across the screen")
			_check(mouth.y > bottom.y and mouth.z > bottom.z + 0.14, "bottle neck stays above the body and closer to the face during drinking")
