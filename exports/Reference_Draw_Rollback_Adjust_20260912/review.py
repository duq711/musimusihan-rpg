...
sheet=Image.new('RGB',(1440,472),(24,24,24))
for slot,i in enumerate(indices):
 x=slot%3*480;y=slot//3*236
 im=Image.open(frames[i]).convert('RGB').crop((400,360,1280,720)).resize((480,196))
...
output=root/'rollback_adjust_draw.mp4'
...
count=0
for selected,repeat,label in [(frames,1,'실제 Godot 렌더링 · 정상 속도'),(frames[:44],4,'좌측 하단으로 연속 이동 · 0.25배속 (실제 프레임 반복)')]:
 for f in selected:
