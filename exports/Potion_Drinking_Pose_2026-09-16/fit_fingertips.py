...
def fun(q,verts=None):
 if np.any(q[5:][2::3] < .30) or np.any(q[5:]<0) or np.any(q[5:]>1) or np.max(np.abs(q[3:5]))>.45:return 1e6
...
for trial in range(3):
 previous=json.load(open(out/'grip_solution.json'))
 axis0=np.array(previous['bottle_basis_in_hand'])[:,1]
 q=np.r_[previous['center_in_hand'],[axis0@u/(axis0@a),axis0@vaxis/(axis0@a)],np.array(previous['amounts']).ravel()]
 q[5:][2::3]=np.maximum(q[5:][2::3],.32)
 if trial:q[:3]+=rng.uniform(-.003,.003,3)
 f=fun(q)
...
print(result,flush=True);json.dump(result,open(out/'fingertip_solution.json','w'),indent=2)
