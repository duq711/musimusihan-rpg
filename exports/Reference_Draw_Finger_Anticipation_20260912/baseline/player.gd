...
func _update_character_arms() -> void:
	...
	if is_instance_valid(right_relaxed_arm) and right_relaxed_arm.visible:
		...
		if _equipment_draw_reaching():
			...
			var reach := Vector3(0.15, -0.29, -0.37).lerp(Vector3(0.035, -0.12, -0.33), MOTION.smooth_phase(t / 0.12))
			...
			for digit: String in ["index", "middle", "ring", "little"]:
				var amounts: Vector3 = {"index": Vector3(.06,.65,.25), "middle": Vector3(.09,.70,.28), "ring": Vector3(.08,.65,.30), "little": Vector3(.10,.60,.32)}[digit]
				right_relaxed_arm.call("set_digit_flexion", digit, amounts)
			right_relaxed_arm.call("set_digit_flexion", "thumb", Vector3(.01,.025,.035))
