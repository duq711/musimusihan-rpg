...
static func entries() -> Array[Dictionary]:
	var result: Array[Dictionary] = [
		...
		_entry("motion_sword_draw", "기본", "영상 참고 검·방패 꺼내기", "멈춤 없이 화면 왼쪽 끝보다 조금 안쪽으로 손 뻗기·이동 중 손가락 감기 → 왼쪽 가장자리 안쪽에서 발검 → 검·방패 들기 · F2 반복 · LMB/RMB 전환", "reference_sword_motion", "draw"),
