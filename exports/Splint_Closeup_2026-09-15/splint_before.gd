...
func set_time(seconds: float) -> void:
	...
	# Hold the receiving arm across the body with a small, steady breathing arc.
	var sway := Vector3(0, sin(winding) * 0.008, 0)
	_wrist = Vector3(0.22, -0.18, -0.55) + shift + sway
	_elbow = Vector3(-0.24, -0.19, -0.49) + shift + sway
