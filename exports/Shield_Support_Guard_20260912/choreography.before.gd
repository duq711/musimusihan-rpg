...
static func guard_sword(raise_progress: float) -> Transform3D:
	var braced := ready()
	braced.origin += Vector3(0.01, 0.005, 0.0)
	braced.basis = braced.basis * Basis.from_euler(Vector3(0.07, -0.06, -0.045))
	return mix(ready(), braced, raise_progress)
...
static func shield_guard() -> Transform3D:
	...
	return pose(Vector3(0.0, -0.52, -0.65), Vector3(8.0, 0.0, 0.0))
