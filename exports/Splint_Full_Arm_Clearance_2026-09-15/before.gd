...
func set_time(seconds: float) -> void:
	...
	_wrist = Vector3(0.02, -0.18, -0.30) + shift + gesture + sway
	_elbow = Vector3(-0.46, -0.13, -0.10) + shift + gesture * 0.35 + sway
	...
	var arm_direction := Vector3(0.15, -0.85, -0.50).normalized()
	...
	_avoid_receiving_hand(right_basis.z, carrying_board)
	var wrist_world: Vector3 = right_arm._active_visual.global_position
	right_arm.fit_arm(to_global(Vector3(0.60, -0.60, -0.35)), wrist_world + global_basis * right_basis.z * 0.25)
...
func _avoid_receiving_hand(elbow_direction: Vector3, carrying_board: float) -> void:
	if carrying_board > 0.999: return
	var receiving := _hand_clearance_samples(left_arm)
	var working := _hand_clearance_samples(right_arm)
	var wrist := Vector3(working[0].x, working[0].y, working[0].z)
	# The enlarged working sleeve also crossed the receiving palm, even when
	# the working fingertips were clear. Include its wrist-to-elbow volume.
	for i in 6:
		var along := i * 0.05
		var p := wrist + elbow_direction * along
		working.append(Vector4(p.x, p.y, p.z, lerpf(0.045, 0.095, i / 5.0)))
	# Keep the working hand and sleeve on the elbow side of the receiving
	# hand. A fixed separating direction is continuous through every orbit;
	# nearest-point push-outs can flip sides as the fingers close.
	var clearance := 0.0
	for a in working:
		for b in receiving:
			var separation := Vector3(a.x - b.x, a.y - b.y, a.z - b.z)
			clearance = maxf(clearance, a.w + b.w + 0.012 - separation.dot(_axis))
	right_arm.position += _axis * clearance * (1.0 - carrying_board)
