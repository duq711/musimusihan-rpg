...
func set_time(seconds: float) -> void:
	...
	# Pull the forearm close; the receiving elbow and upper arm stay off-screen.
	var sway := Vector3(0, sin(winding) * 0.008, 0)
	_wrist = Vector3(0.02, -0.18, -0.30) + shift + sway
	_elbow = Vector3(-0.46, -0.13, -0.10) + shift + sway
	_axis = (_elbow - _wrist).normalized()
	_normal = (Vector3(0, 0.65, 1) - _axis * _axis.dot(Vector3(0, 0.65, 1))).normalized()
	var left_basis := Basis(_normal.cross(_axis).normalized(), _normal, _axis).rotated(_axis, 0.06 * sin(winding))
	left_arm.transform = Transform3D(left_basis, _wrist)
	# A relaxed open hand closes firmly as the first wrap takes tension.
	var fist := smoothstep(SUPPORT_END, SUPPORT_END + 0.85, elapsed)
	for digit: String in ["index", "middle", "ring", "little"]:
		var relaxed := Vector3(0.10, 0.15, 0.12)
		left_arm.set_digit_flexion(digit, relaxed.lerp(Vector3(1.0, 1.0, 0.96), fist))
	# The thumb folds over after the fingers start closing.
	var thumb_close := smoothstep(SUPPORT_END + 0.18, SUPPORT_END + 0.95, elapsed)
