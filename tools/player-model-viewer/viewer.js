...
const center=new THREE.Vector3(0,.89,0);let model=null,height=1.78,width=.72,ready=false,pose=null;
...
function updateBounds(){const box=new THREE.Box3().setFromObject(model);box.getCenter(center);const size=box.getSize(new THREE.Vector3());height=size.y;width=Math.max(size.x,size.z);}
function setPose(enabled){if(!ready)return;pose.setEnabled(enabled);document.querySelector('#tpose').setAttribute('aria-pressed',String(enabled));updateBounds();setView([0,0,-1]);}
...
try{
 ...
 let meshes=0;model.traverse(n=>{if(n.isMesh)meshes++;});ready=true;reset();if(new URLSearchParams(location.search).get("pose")==="t")setPose(true);if(new URLSearchParams(location.search).get("view")==="axilla")focusAxilla();loading.hidden=true;status.textContent='자유 회전 · 확대 · 이동';
 // Read-only diagnostics for checking that the viewer loaded the production model.
 window.playerViewer={model,camera,controls,renderer,info,pose,meshCount:meshes};
