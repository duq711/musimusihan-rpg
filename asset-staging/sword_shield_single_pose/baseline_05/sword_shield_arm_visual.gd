...
func set_shield_grip_surface(shield_from_hand: Transform3D) -> void:
	...
	# Never treat the arched 44 mm loop as a flat, wrist-aligned ribbon.
...
func _shield_grip_contact(digit: String, along_grip: float, tension: float) -> Dictionary:
	...
	var angle := deg_to_rad(-4.0)
	var v := clampf((point.x + 0.18) * sin(angle) + (point.y - 0.015) * cos(angle), -0.075, 0.075)
	var u := 0.015 if is_thumb else 0.0
	var surface := _authored_shield_loop(u, v)
	var tangent_u := _authored_shield_loop(u + 0.0001, v) - _authored_shield_loop(u - 0.0001, v)
	var tangent_v := _authored_shield_loop(u, v + 0.0001) - _authored_shield_loop(u, v - 0.0001)
	var normal := tangent_u.cross(tangent_v).normalized()
	if normal.z > 0.0: normal = -normal
	# Middle pads pull the far face while the thumb opposes on the near face.
	if is_thumb: normal = -normal
	var clearance := lerpf(0.0018, 0.0014, tension)
	if digit == "index": clearance += 0.0012
	elif digit == "ring": clearance += 0.0005
	elif is_thumb: clearance += 0.00035
	var target := surface + normal * (0.002 + clearance)
	return {"target": _shield_surface_inverse * target, "surface": _shield_surface_inverse * (surface + normal * 0.002), "inward": -(_shield_surface_inverse.basis * normal).normalized(), "uv": Vector2(u, v)}
...
func _solve_shield_power_digit(digit: String, seed: Vector3, strength: float) -> void:
	...
	var patch := digit if is_little else digit + "_middle"
	...
	if strength > 0.0:
		for iteration in 30:
			for joint in ([2, 1, 0] if is_little else [1, 0]):
			...
			_apply_digit_angles(digit, angles, lateral)
			if not is_little:
				# Once the middle pad bears on leather, the terminal hinge folds
				# toward the palm independently instead of reaching back to it.
				var frame := skeleton.get_bone_global_pose(skeleton.find_bone(digit + "2"))
...
func _settle_shield_terminal(digit: String, angles: Vector3, lateral: float) -> Vector3:
	...
	for sample in 55:
		var candidate := lerpf(-1.35, -0.40, float(sample) / 54.0)
