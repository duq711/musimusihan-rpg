...
func _update_viewmodel(delta: float) -> void:
	...
	if is_instance_valid(shield_pivot):
		...
		if paired_sword and phase == "recovery" and _sword_clash_recovering:
...
func _fit_sword_grip(grip_local: Vector3) -> void:
	...
	for iteration in 4:
		...
		back = back - x * back.dot(x)
		if back.length_squared() < 0.00001: back = weapon_pivot.global_basis.z
		var z := back.normalized().rotated(x,grip_roll)
		_place_hand_contact(weapon_arm,contact,Basis(x,z.cross(x).normalized(),z),"sword")
		wrist = weapon_arm.global_position
