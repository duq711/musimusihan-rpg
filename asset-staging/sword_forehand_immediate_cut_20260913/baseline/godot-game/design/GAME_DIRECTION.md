...
- 2026-09-13 최신 사용자 지시: 정베기의 우→좌 이동은 승인했으며, 이동을 유지한 채 검 모델 방향을 반대로 바꾼다.
- 현재 구현: 최종 실제 파지점을 중심으로 검과 오른쪽 장갑을 함께 로컬 Z축으로 회전하여 활성 0.145초 타격까지 점진적으로 180도에 도달한다. 왼손 추종과 소매 IK는 기존 코드로 접촉을 유지한다. manifest·원본 GLB·역베기와 기존 공격 시간·차지·피해·기력·방패 규칙을 보존하고 WINDUP 준비는 추가하지 않는다. 관련 검사 8종과 실제 양손·방패 촬영을 완료했으며, 기존 역베기 182 PNG와 원본 모션·모델을 보존했다. 회수 중 화면 밖 구간과 기존 자연 검격 교차 시험의 동일한 5개 실패는 남아 있다. 시각 품질의 사용자 승인을 뜻하지 않으며 영상·변경 전 자료·검사 근거는 [검증 기록](../../asset-staging/sword_forehand_grip_direction_20260913/validation_summary.json)에 기록한다.
