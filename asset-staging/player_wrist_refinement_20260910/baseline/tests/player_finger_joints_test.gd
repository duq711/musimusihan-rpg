...
func _audit_corrective(arm: Node3D, digit: String, joint: int, context: String) -> Vector3:
		...
		for surface: Dictionary in source.surfaces:
			...
			for vertex in vertices.size():
				...
				for influence in int(surface.stride):
					...
					if float(surface.weights[entry]) > dominant_weight:
						dominant_weight = surface.weights[entry]
						dominant = source.bones[surface.bones[entry]]
				var owned := dominant >= 0 and skeleton.get_bone_name(dominant).begins_with(digit)
				if owned and absf(point.y) < 0.004 and surface.material_name != "Detailed_Trim":
					section_min = section_min.min(point)
					section_max = section_max.max(point)
					section_samples += 1
				if delta.length() >= 0.0000001 and not owned: wrong_digit += 1
	...
	_check(section_samples >= 8 and farthest < 0.04 and axial_extent < 0.0142 and wrong_digit == 0, "corrective geometry must stay near its own skin joint without affecting wrist/other-digit dominant vertices: " + context + " radius=" + str(farthest) + " axial=" + str(axial_extent) + " other=" + str(wrong_digit))
...
func _test_room_lifecycle() -> void:
	...
	room.run_feature("player_finger_joints")
	_check(room.player.get_finger_joint_review_snapshot().active, "independent joint feature must remain executable after reset")
	room.leave_room()
