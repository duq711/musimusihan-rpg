...
func run():
	...
	output=ProjectSettings.globalize_path("res://artifacts/visual_qa/wooden_torch_billowing")
	...
	for frame in 4:
		...
		await capture(vp,"billow_%02d" % frame)
	...
	for direction in [-1.0, 1.0, 0.0]:
		p.velocity = p.camera.global_basis.x * p.WALK_SPEED * direction
		for i in 90: p._update_torch(1.0/60.0)
