...
func run():
	...
	check(p.torch_arm.get_joint_snapshot().angles_radians.thumb.x>0.1,"torch restores authored unbending after reset")
...
func audit_surface(p):
	...
	var handle: MeshInstance3D=p.torch_pivot.find_child("CharredHandle",true,false)
	var hv: PackedVector3Array=handle.mesh.surface_get_arrays(0)[Mesh.ARRAY_VERTEX]
	var rings={}
	for v in hv:
		var a: Vector3=p.torch_pivot.to_local(handle.to_global(v));var y=snappedf(a.y,.00001)
