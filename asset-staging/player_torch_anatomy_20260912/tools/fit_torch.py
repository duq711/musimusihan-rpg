...
ratio=float(os.environ.get('TORCH_RADIUS_RATIO','1.0'));variant=os.environ.get('TORCH_VARIANT','full')
from pathlib import Path
s=Path(__file__).resolve().parents[1];sys.path.insert(0,str(s/'tools/python_deps'))
import numpy as np
from scipy.optimize import least_squares
x=np.load(s/'audit/fit_input_reweighted.npz');v=x['vertices'];B=x['bones'];Bi=np.linalg.inv(B);names=list(x['names']);parents=x['parents'];W=x['weights'];normals=x['normals']
...
def transforms(q):
 ...
 for i,n in enumerate(names):
  ...
  if n=='thumb0':
   R=rot(1,q[15])@R
   if len(q)>23:
    a=q[23];c=np.cos(a);sn=np.sin(a);rz=np.eye(4);rz[:2,:2]=[[c,-sn],[sn,c]];R=rz@R
  if n in ['index0','middle0','ring0','little0'] and len(q)>19:
   a=q[19+['index0','middle0','ring0','little0'].index(n)];c=np.cos(a);sn=np.sin(a);rz=np.eye(4);rz[:2,:2]=[[c,-sn],[sn,c]];R=rz@R
...
rng=np.random.default_rng(711);sample=np.unique(np.concatenate([np.arange(0,len(v),1)]+contact));vs=v[sample];ws=W[sample];ds=deltas[keyindices][:,sample];homo=np.column_stack([vs,np.ones(len(vs))]); maps=[np.where(np.isin(sample,c))[0] for c in contact]
...
lo=np.r_[lo,[-.45]*4];hi=np.r_[hi,[.45]*4]
lo=np.r_[lo,-1.3];hi=np.r_[hi,1.3];lo[15]=-1.3;hi[15]=1.3;lo[0]=-1.1
if variant=='power':
 for i in [4,7,10,13]:hi[i]=-.15
 for i in [5,8,11,14]:hi[i]=0
qstart=np.r_[np.radians([-12,-15,-10]+[-20,-25,-15]*4+[20]),.085,-.045,.12,0,0,0,0,0]
# Encourage middle phalanges to follow shaft, and adjacent fingers to close.
midmaps=[]
for d in digits[1:]:
 i=names.index(d+'1'); ns=normals@B[i,:3,:3]; ids=np.where((W[:,i]>.55)&(ns[:,2]<-.3))[0];midmaps.append(np.where(np.isin(sample,ids))[0])
cf=s/('audit/collision_constraints_'+variant+'.json');cc=np.array(json.loads(cf.read_text())) if cf.exists() else np.empty((0,7))
preserve=np.array(json.loads((s/('audit/fitted_pose_'+variant+'.json')).read_text())['parameters'])
def residual(q):
 ...
 collision=np.minimum(0,dist-.0007)*120
 contacts=[]
 for m in maps:
 ...
 midcontact=[]
 for m in midmaps:
  a=np.sort(dist[m]); midcontact.append(np.mean(a[:max(3,len(a)//10)])-.0012)
 # Joint root positions must converge across the shaft instead of fanning out.
 transforms_world=transforms(q)@B
 tips=np.array([transforms_world[names.index(d+'2'),:3,3] for d in digits[1:]])
 gaps=np.linalg.norm(np.diff(tips,axis=0),axis=1)
 # Thumb lays across shaft near index, instead of standing axially above it.
 axis=np.array([np.cos(q[18]),np.sin(q[18]),0]);tc=p[maps[0]][np.argsort(dist[maps[0]])[:12]].mean(0);ic=p[maps[1]][np.argsort(dist[maps[1]])[:12]].mean(0)
 td=transforms_world[names.index('thumb2'),:3,1]
 thumb_axial=np.dot(tc-ic,axis)-.012
 thumb_dir=np.dot(td,axis)
 self_pen=np.array([])
 if len(cc):
  tri=p[cc[:,1:4].astype(int)];norm=np.cross(tri[:,1]-tri[:,0],tri[:,2]-tri[:,0]);norm/=np.maximum(np.linalg.norm(norm,axis=1)[:,None],1e-10);target=np.einsum('ij,ijk->ik',cc[:,4:],tri);signed=np.einsum('ij,ij->i',p[cc[:,0].astype(int)]-target,norm);self_pen=np.minimum(signed-.0005,0)*120
 return np.r_[(q-preserve)*.15,self_pen,thumb_axial*110,thumb_dir*.3,collision,np.array(contacts)*190,np.array(midcontact)*130,np.maximum(gaps-.022,0)*50,reg,q[19:]*.006,q[18]*.001]
runs=[]
for seed in range(1):
 start=qstart.copy();start[:15]+=np.random.default_rng(seed+17).normal(0,.17,15);start[15]=[.3,-.3,.6,-.6][seed];start[16]=[.08,.09,.065,.06][seed]
 start=np.clip(np.array(json.loads((s/('audit/fitted_pose_'+variant+'.json')).read_text())['parameters']),lo+1e-5,hi-1e-5)
 rr=least_squares(residual,start,bounds=(lo,hi),diff_step=1e-4,max_nfev=100,ftol=1e-6,xtol=1e-6,gtol=1e-6)
...
result={'parameters':q.tolist(),'degrees':dict((d,np.degrees(q[di*3:di*3+3]).tolist()) for di,d in enumerate(digits)),'thumb_opposition_degrees':float(np.degrees(q[15])),'shaft_in_hand':{'center':[0,float(q[16]),float(q[17])],'axis':[float(np.cos(q[18])),float(np.sin(q[18])),0]},'full_skin_min_clearance_m':float(dist.min()),'penetrating_vertices':int(np.count_nonzero(dist<-.0005)),'contacts':dict((l,{'minimum_m':float(dist[c].min()),'closest_patch_mean_m':float(np.sort(dist[c])[:max(3,int(len(c)*.06))].mean())}) for l,c in zip(labels,contact)),'root_adduction_radians':dict(zip(digits[1:],q[19:23].tolist())),'thumb_root_adduction':float(q[23]),'radius_ratio':ratio,'cost':res.cost,'evaluations':res.nfev,'runtime_delta_reproduction_error':float(np.max(np.abs(T-x['runtime_deltas']))),'handle_rings':ring}
(s/('audit/fitted_pose_'+variant+'.json')).write_text(json.dumps(result,indent=2));np.save(s/('audit/fitted_vertices_'+variant+'.npy'),p)
