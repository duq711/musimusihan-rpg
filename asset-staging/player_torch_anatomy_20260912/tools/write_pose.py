...
import numpy as np
s=Path(__file__).resolve().parents[1];root=s.parents[1];d=json.loads((s/('audit/fitted_pose_'+variant+'.json')).read_text());q=np.array(d['parameters'])
...
def euler(x,y,z):
 ...
 return Ry@Rx@Rz
P=euler(-25,8,13);pos=np.array([-.48,-.46,-.62]);shoulder=np.array([-.29,-.34,.10]);pole=np.array([-.65,-.85,.14])
def frame(angle):
...
angle=np.radians(-35);alignment,basis,origin=frame(angle)
...
text+='const CARRY_POSITION := '+v(pos)+'\n'
...
text+='}\n'
text+='const THUMB_ROOT_ADDUCTION := '+str(d['thumb_root_adduction'])+'\n'
text+='const SHAFT_RADIUS_RATIO := '+str(d['radius_ratio'])+'\n'
