...
static func entries() -> Array[Dictionary]:
	var result: Array[Dictionary] = [
		...
		_entry("body_health", "생존", "7부위 체력 · 선택 부위 치료", "머리35·가슴85·복부70·양팔60·양다리65 · 각 부위 피해 후 I에서 치료 · F2 수치 조절/재보급", "body_health", "damage"),
	...
	for condition_id in ExpeditionSession.CONDITION_DRAIN_BONUSES:
		result.append(_entry("condition:" + condition_id, "생존", ExpeditionSession.get_condition_display_name(condition_id), "실제 부위 상태이상·이동/전투/지속 피해 · 대응 치료품 보급 · F2 재개 후 I 치료 / 재선택 재보급", "condition", condition_id))
