...
func get_melee_damage(charge: float = 0.0) -> float:
	if inventory_model == null:
		return lerpf(27.0, 53.0, clampf(charge, 0.0, 1.0)) * get_body_combat_multiplier()
	var stats := inventory_model.equipped_smithing_stats()
	return lerpf(float(stats.light_damage), float(stats.heavy_damage), clampf(charge, 0.0, 1.0)) * get_body_combat_multiplier()
...
func get_melee_stamina_cost(charge: float = 0.0) -> float:
	...
	return lerpf(18.0, 30.0, clampf(charge, 0.0, 1.0)) * scale / maxf(0.1, get_body_combat_multiplier())
