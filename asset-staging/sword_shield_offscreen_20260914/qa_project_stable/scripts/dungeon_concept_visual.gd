...
const SOLID_SHADOWS := preload("res://scripts/static_solid_shadows.gd")
static var _stone_variants: Dictionary = {}
static var _blocks: Dictionary = {}
static var _assembled_meshes: Dictionary = {}
static var _ring_meshes: Dictionary = {}
static var _mortar_material: StandardMaterial3D
static var _assembly_specs: Dictionary = {}
...
static func add_pointed_ring(parent: Node3D, origin: Vector3, width: float, rise: float, thickness: float, depth: float) -> void:
	for side in [-1.0, 1.0]:
		for index in 10:
			...
			node.mesh = _ring_wedge(width, rise, thickness, depth, side, index)
			node.material_override = stone_material(index)
			node.position = origin
			parent.add_child(node)


static func _ring_wedge(width: float, rise: float, thickness: float, depth: float, side: float, index: int) -> ArrayMesh:
	var key := str([width, rise, thickness, depth, side, index])
...
static func create_architecture(size: Vector3, kind: String) -> Node3D:
	...
	if kind in ["ossuary_wall", "wet_flagstone_floor", "crypt_ceiling"]:
		SOLID_SHADOWS.add(root, size)
	return root
...
static func _add_masonry(root: Node3D, size: Vector3) -> void:
	...
	mesh.mesh = _masonry_mesh(size)
	mesh.material_override = stone_material(2)
	root.add_child(mesh)


static func _masonry_mesh(size: Vector3) -> ArrayMesh:
	var cache_key := "masonry:" + str(size)
	_assembly_specs[cache_key] = {"kind": "masonry", "size": size}
	if _assembled_meshes.has(cache_key):
		return _assembled_meshes[cache_key]
	var mesh := source_assembly_mesh("masonry", size)
	_assembled_meshes[cache_key] = mesh
	return mesh


static func _assemble_masonry_mesh(size: Vector3) -> ArrayMesh:
	var along_x := size.x >= size.z
...
static func _paving_mesh(size: Vector3) -> ArrayMesh:
	var cache_key := "paving:" + str(size)
	_assembly_specs[cache_key] = {"kind": "paving", "size": size}
	if _assembled_meshes.has(cache_key):
		return _assembled_meshes[cache_key]
	var mesh := source_assembly_mesh("paving", size)
	_assembled_meshes[cache_key] = mesh
	return mesh


static func _assemble_paving_mesh(size: Vector3) -> ArrayMesh:
...
static func source_assembly_mesh(kind: String, size: Vector3) -> ArrayMesh:
	# Independent source path for offline baking, regression validation and
	# user-authored dimensions that are not in the packaged cache.
	assert(kind in ["masonry", "paving"])
	return _assemble_masonry_mesh(size) if kind == "masonry" else _assemble_paving_mesh(size)


static func source_assembly_specs() -> Dictionary:
	return _assembly_specs.duplicate(true)
...
static func _add_mortar_core(root: Node3D, size: Vector3) -> void:
	...
	mesh.size = size
	node.mesh = mesh
	node.material_override = _shared_mortar_material()
	root.add_child(node)
...
static func _shared_mortar_material() -> StandardMaterial3D:
	if _mortar_material == null:
		_mortar_material = SURFACES.stone(Color(0.10, 0.115, 0.11), 2.0)
	return _mortar_material
