...
const SUPPLIED_MODELS := {
	"wooden_barrel_01": "res://assets/models/loot_containers/wooden_barrel_01.glb",
	"wooden_crate_01": "res://assets/models/loot_containers/wooden_crate_01.glb",
	"wooden_crate_02": "res://assets/models/loot_containers/wooden_crate_02.glb",
	"treasure_chest": "res://assets/models/loot_containers/treasure_chest.glb",
}
...
var visual_variant := ""
var supplied_visual: Node3D
var _visual_bounds := AABB(Vector3(-0.9, 0.005, -0.6), Vector3(1.8, 1.05, 1.2))
var _lid_closed_position := Vector3.ZERO
var _lid_contacts: Dictionary = {}
var _lid_contact_normals: Dictionary = {}
var _lid_pull := 0.0
var _lid_rest_height := 0.012


func configure_visual(variant: String) -> DungeonLootChest:
	assert(not is_inside_tree(), "Choose the container model before adding it to the map.")
	assert(variant.is_empty() or SUPPLIED_MODELS.has(variant), "Unknown loot container model: " + variant)
...
func _build_collision() -> void:
	...
	shape.size = _visual_bounds.size
	collision.shape = shape
	collision.position = _visual_bounds.get_center()
	add_child(collision)


func _build_visuals() -> void:
	if not visual_variant.is_empty():
		_build_supplied_visual()
		return
...
func _build_supplied_visual() -> void:
	var scene := load(str(SUPPLIED_MODELS[visual_variant])) as PackedScene
	assert(scene != null, "Missing supplied loot model: " + visual_variant)
	supplied_visual = scene.instantiate() as Node3D
	supplied_visual.name = "SuppliedContainerVisual"
	add_child(supplied_visual)
	_disable_imported_collisions(supplied_visual)
	_visual_bounds = _mesh_bounds(supplied_visual, self)
	lid_pivot = supplied_visual.find_child("LidPivot", true, false) as Node3D
	assert(lid_pivot != null, "Supplied container must have its actual separated lid.")
	_lid_closed_rotation = lid_pivot.rotation
	_lid_closed_position = lid_pivot.position
	if _supplied_lid_lifts():
		var body := supplied_visual.find_child("Body", true, false) as Node3D
		assert(body != null, "Loose lids need a body rim to rest on.")
		_lid_rest_height = maxf(0.012, _mesh_bounds(body, supplied_visual).end.y - _mesh_bounds(lid_pivot, supplied_visual).position.y + 0.006)
	_cache_supplied_contacts()


static func _mesh_bounds(node: Node3D, ancestor: Node3D) -> AABB:
	var result := AABB()
	var first := true
	...
	for mesh: MeshInstance3D in meshes:
		if mesh.mesh == null:
			continue
		var bounds := _transform_to_ancestor(mesh, ancestor) * mesh.get_aabb()
		result = bounds if first else result.merge(bounds)
		first = false
	return result


func _cache_supplied_contacts() -> void:
	# Read the real upper surface once, including the curved treasure lid and
	# circular barrel top. Cached points follow that separated lid as it opens.
	var bounds := _mesh_bounds(lid_pivot, lid_pivot)
	var faces := PackedVector3Array()
	for mesh: MeshInstance3D in lid_pivot.find_children("*", "MeshInstance3D", true, false):
		var transform_value := _transform_to_ancestor(mesh, lid_pivot)
		for vertex in mesh.mesh.get_faces():
			faces.append(transform_value * vertex)
	for approach in ["front", "back", "left", "right"]:
		for side in [-1, 1]:
			var point := bounds.get_center()
			if approach in ["front", "back"]:
				point.x += float(side) * (-1.0 if approach == "front" else 1.0) * bounds.size.x * 0.23
				point.z += (-1.0 if approach == "front" else 1.0) * bounds.size.z * 0.30
			else:
				point.x += (-1.0 if approach == "left" else 1.0) * bounds.size.x * 0.30
				point.z += float(side) * (1.0 if approach == "left" else -1.0) * bounds.size.z * 0.23
			point.y = bounds.end.y + 0.1
			var highest := -INF
			...
			for triangle in range(0, faces.size(), 3):
				...
				if hit is Vector3 and hit.y > highest:
					highest = hit.y
					normal = (faces[triangle + 1] - faces[triangle]).cross(faces[triangle + 2] - faces[triangle]).normalized()
					if normal.y < 0.0:
						normal = -normal
			assert(is_finite(highest), "No actual lid surface at hand contact: " + visual_variant)
			point.y = highest
			_lid_contacts[approach + str(side)] = point
			_lid_contact_normals[approach + str(side)] = normal


func _supplied_lid_lifts() -> bool:
	return visual_variant in ["wooden_barrel_01", "wooden_crate_02"]


func _set_lid_pull(pull: float) -> void:
	_lid_pull = clampf(pull, 0.0, 1.0)
	if _supplied_lid_lifts():
		# Lift and slide the loose cover onto the rear rim. Its final position
		# stays supported by the container instead of hovering after hands leave.
		lid_pivot.position = _lid_closed_position + Vector3(0.0, sin(_lid_pull * PI) * 0.24 + _lid_rest_height * _lid_pull, _visual_bounds.size.z * 0.40 * _lid_pull)
	else:
		lid_pivot.rotation = _lid_closed_rotation + Vector3(deg_to_rad(68.0) * _lid_pull, 0.0, 0.0)
...
func _build_interaction_area() -> void:
	...
	if not visual_variant.is_empty():
		focus_box.size = _visual_bounds.size + Vector3(0.25, 0.3, 0.25)
	focus_shape.shape = focus_box
	focus_shape.position = Vector3(0, 0.82, 0) if visual_variant.is_empty() else _visual_bounds.get_center()
...
func cancel_timed_interaction(player_ref: DungeonPlayer) -> void:
	...
	if is_instance_valid(lid_pivot):
		...
		_lid_pull = 0.0
		if not visual_variant.is_empty():
			lid_pivot.position = _lid_closed_position
...
func get_hand_contact_transform(side: int, progress: float) -> Transform3D:
	if not visual_variant.is_empty():
		var finger_axis := {"front": Vector3.BACK, "back": Vector3.FORWARD, "left": Vector3.RIGHT, "right": Vector3.LEFT}[_hand_approach] as Vector3
		return _surface_contact_transform(lid_pivot, _lid_contacts[_hand_approach + str(side)], _lid_contact_normals[_hand_approach + str(side)], finger_axis)
...
func is_within_hand_reach(player_ref: Node3D) -> bool:
	...
	if not visual_variant.is_empty():
		...
		var horizontal := Vector2(maxf(maxf(bounds.position.x - local_player.x, local_player.x - bounds.end.x), 0.0), maxf(maxf(bounds.position.z - local_player.z, local_player.z - bounds.end.z), 0.0))
		return horizontal.length() <= 1.15 and absf(local_player.y - 0.9) <= 0.65
...
func _animate_open() -> void:
	...
	if _supplied_lid_lifts():
		...
		_lid_tween.tween_method(_set_lid_pull, _lid_pull, 1.0, 0.42)
	else:
