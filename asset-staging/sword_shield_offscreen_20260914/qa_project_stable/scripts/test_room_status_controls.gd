...
signal body_part_selected(id: String)
signal body_damage_requested(id: String, amount: float)
signal body_reset_requested
...
var body_controls: VBoxContainer
var body_part_selector: OptionButton
var body_part_readout: Label
var body_damage_button: Button
var body_blackout_button: Button
var body_reset_button: Button
...
func set_snapshot(snapshot: Dictionary) -> void:
	...
	_set_body_snapshot(snapshot.get("body", {}))
	_syncing = false
...
func _ensure_built() -> void:
	...
	_build_body_controls(content)
	hint_label = _make_label("스트레스 40 불안 · 60 환청 · 80 환영\n전체 체력 조절·부위 시험 회복은 0부위 치료 제한을 우회하는 시험 초기화입니다.", 14, MUTED_COLOR)
	hint_label.name = "StatusAdjustmentHint"
	content.add_child(hint_label)


func _build_body_controls(parent: VBoxContainer) -> void:
	body_controls = VBoxContainer.new()
	body_controls.name = "BodyHealthTrialControls"
	body_controls.add_theme_constant_override("separation", 5)
	parent.add_child(body_controls)
	body_controls.add_child(_make_label("선택 부위 · 실제 피해와 치료", 16, TEXT_COLOR))
	var row := HBoxContainer.new()
	row.add_theme_constant_override("separation", 8)
	body_controls.add_child(row)
	body_part_selector = OptionButton.new()
	body_part_selector.name = "BodyPartSelector"
	body_part_selector.custom_minimum_size = Vector2(155, 34)
	body_part_selector.item_selected.connect(_on_body_part_selected)
	row.add_child(body_part_selector)
	body_part_readout = _make_label("", 14, TEXT_COLOR)
	row.add_child(body_part_readout)
	var actions := HBoxContainer.new()
	actions.add_theme_constant_override("separation", 8)
	body_controls.add_child(actions)
	body_damage_button = _body_button("BodyDamage10", "선택 부위 피해 10", actions)
	body_damage_button.pressed.connect(_request_body_damage.bind(false))
	body_blackout_button = _body_button("BodyBlackout", "선택 부위 0으로", actions)
	body_blackout_button.pressed.connect(_request_body_damage.bind(true))
	body_reset_button = _body_button("BodyReset", "부위 전체 시험 회복", actions)
	body_reset_button.pressed.connect(func() -> void: body_reset_requested.emit())
	body_controls.add_child(_make_label("0부위: 일반 치료 불가 → 수술·고위 마법으로 1 복구 → 회복약\n머리·가슴 0은 사망 시험 · F2 재개 후 I에서 선택 부위 치료", 13, MUTED_COLOR))


func _body_button(node_name: String, caption: String, parent: HBoxContainer) -> Button:
...
func _set_body_snapshot(snapshot: Dictionary) -> void:
	_body_snapshot = snapshot.duplicate(true)
	var parts: Dictionary = snapshot.get("parts", {})
	body_controls.visible = not parts.is_empty()
	if parts.is_empty():
		return
	var selected := str(snapshot.get("selected_part", ""))
	if selected.is_empty() and body_part_selector.item_count > 0:
		selected = str(body_part_selector.get_item_metadata(body_part_selector.selected))
	if not parts.has(selected):
		selected = str(parts.keys()[0])
	body_part_selector.clear()
	for part_id in parts:
		var part: Dictionary = parts[part_id]
		body_part_selector.add_item(str(part.get("name", part_id)))
		var index := body_part_selector.item_count - 1
		body_part_selector.set_item_metadata(index, str(part_id))
		if str(part_id) == selected:
			body_part_selector.select(index)
	var part: Dictionary = parts[selected]
	var conditions: Dictionary = part.get("conditions", {})
	var names: Array[String] = []
	for id in conditions:
		names.append(ExpeditionSession.get_condition_display_name(str(id)))
	body_part_readout.text = "%d / %d · %s%s" % [roundi(float(part.get("health", 0))), roundi(float(part.get("max_health", 0))), "회복 불가" if bool(part.get("blacked", false)) else "치료 가능", " · " + " · ".join(names) if not names.is_empty() else ""]
	body_blackout_button.disabled = bool(part.get("blacked", false))


func _on_body_part_selected(index: int) -> void:
	if _syncing or index < 0:
		return
	var id := str(body_part_selector.get_item_metadata(index))
	_body_snapshot["selected_part"] = id
	_set_body_snapshot(_body_snapshot)
	body_part_selected.emit(id)


func _request_body_damage(blackout: bool) -> void:
	if _syncing or body_part_selector.item_count == 0:
		return
	var id := str(body_part_selector.get_item_metadata(body_part_selector.selected))
	var parts: Dictionary = _body_snapshot.get("parts", {})
	if not parts.has(id):
		return
	var amount := float((parts[id] as Dictionary).get("health", 0)) if blackout else 10.0
	body_damage_requested.emit(id, amount)
