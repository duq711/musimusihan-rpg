...
func _run() -> void:
	...
	_check(hideout.door_button != null and hideout.door_button.name == "SanctuaryDoorButton", "hideout must retain its existing map-action connection")
	_check(not hideout.door_button.visible and not hideout.door_button.is_visible_in_tree(), "the removed persistent map button must stay hidden when the hideout opens")
	...
	_send_map_key(hideout)
	_check(hideout.modal_scrim.visible and hideout.destination_panel.visible and hideout.current_panel == hideout.destination_panel, "M must reveal the destination map while its persistent shortcut button is hidden")
...
func _send_map_key(hideout: SanctuaryHideout) -> void:
	var event := InputEventKey.new()
	event.keycode = KEY_M
...
func _finish() -> void:
	...
	if failures.is_empty():
		print("HIDEOUT FLOW TEST PASS: hidden persistent shortcut, M map reveal, close actions, and dungeon transition")
		quit(0)
		return
