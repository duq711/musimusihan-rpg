...
func _test_visual_contract() -> void:
	...
	_check(is_equal_approx(arrow.position.z - rest_position.z, ArcheryVisuals.STRING_DRAW_TRAVEL), "full bow draw must pull its actual nocked arrow by the authored two-hand draw travel")
