...
func _host_treatment(safe: bool) -> void:
	var host: Node3D = QuietHideout.new() if safe else QuietDungeon.new()
	root.add_child(host)
	...
	var before := host.inventory.count_item("healing_draught")
	overlay.consumable_requested.emit("healing_draught")
	...
	overlay.consumable_requested.emit("surgery_kit")
	...
	overlay.consumable_requested.emit("splint")
