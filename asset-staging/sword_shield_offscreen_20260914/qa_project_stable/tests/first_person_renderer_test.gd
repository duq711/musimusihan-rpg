...
func _run() -> void:
	...
	daylight.name = "WorldDaylight"
	daylight.light_energy = 0.8
	daylight.light_color = Color(0.72, 0.81, 1.0)
	daylight.rotation_degrees = Vector3(-50, -30, 0)
	stage.add_child(daylight)
	...
	_check(renderer.equipment_lights.size() == 4, "torch spot/fill, staff and real world daylight must illuminate the equipment camera")
	...
	var mirrored_daylight := renderer.camera.get_node("EquipmentWorldDaylight") as DirectionalLight3D
	_check(mirrored_daylight.visible and mirrored_daylight.light_energy == daylight.light_energy and mirrored_daylight.light_color == daylight.light_color, "putting out a torch in daylight must retain the actual scene's illumination on hands and weapons")
	daylight.visible = false
	renderer.sync_view()
	_check(not mirrored_daylight.visible and daylight.layers == 1, "turning off world daylight must remove its equipment light without changing the source layers")
...
func _check_window_pixel_budget() -> void:
	...
	fixture.content_scale_aspect = Window.CONTENT_SCALE_ASPECT_KEEP
	_check(fixture.get_visible_rect().size == Vector2(1280, 720) and budget.render_size_for(fixture) == Vector2i(2560, 1440), "the shipped keep-aspect setting must budget only the 16:9 content buffer inside Retina letterboxing")
	_check(budget.equipment_size_for(budget.render_size_for(fixture)) == Vector2i(1280, 720), "Retina letterboxing must retain the designed equipment aspect and pixel budget")
