...
for test_path in "${test_paths[@]}"; do
	...
	# These suites repeatedly construct real maps/players, including all loot
	# sites and seven cave landmarks. Measured sequential runs exceed 180s.
	# An explicit user override still controls every selected suite.
	if [[ -z "${GODOT_TEST_TIMEOUT_SECONDS:-}" ]]; then
		case "$test_name" in
			loot_spawn_test.gd|cooking_system_test.gd|cave_flow_test.gd)
