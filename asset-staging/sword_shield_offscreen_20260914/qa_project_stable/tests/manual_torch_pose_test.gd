...
func run():
	...
	audit_pocket_motion(p)
	audit_shoulder_carry(p)
	audit_flame_flow(p)
	...
	check(p.torch_arm.visible and not p.left_support_arm.visible,"hand keeps gripping during pocket return")
	p._update_viewmodel(2.0)
	check(not p.torch_arm.visible and not p.left_support_arm.visible,"extinguish leaves resting left hand stowed")
...
func audit_testroom():
	...
	for i in 2:
		...
		audit_pocket_motion(room.player)
		audit_shoulder_carry(room.player)
...
func point_in_view(p, point: Vector3) -> bool:
	if p.camera.is_position_behind(point): return false
	return p.camera.get_viewport().get_visible_rect().has_point(p.camera.unproject_position(point))

func audit_shoulder_carry(p):
	p.velocity = Vector3.ZERO
	p.set_torch_enabled(false)
	p._update_viewmodel(2.0)
	p.set_torch_enabled(true)
	p._update_viewmodel(p.TORCH_GRIP.DRAW_REVEAL_TIME)
	check(point_in_view(p, p.torch_pivot.to_global(Vector3(0, 0.81, 0))), "draw animation visibly passes through view")
	var draw_frame: Transform3D = p.torch_pivot.transform
	paused = true
	p._update_viewmodel(0.5)
	check(p.torch_pivot.transform == draw_frame, "pause freezes draw pose")
	paused = false
	p._update_viewmodel(2.0)
	for speed in [0.0, p.WALK_SPEED, p.SPRINT_SPEED]:
		p.velocity = p.global_basis * Vector3(speed, 0, 0)
		for i in 60:
			p._update_viewmodel(1.0/60.0)
			for height in [-0.1, 0.12, 0.44, 0.81, 1.5]:
				check(not point_in_view(p, p.torch_pivot.to_global(Vector3(0, height, 0))), "carried torch remains outside view while walking and sprinting")
	check(p.torch_pivot.visible and p.torch_flame.visible and p.torch_arm.visible, "carry uses real offscreen placement instead of hiding models")
	check(p.torch.visible and p.torch_fill.is_visible_in_tree(), "offscreen torch continues illuminating")
	var wrist: Vector3 = p.camera.to_local(p.torch_arm.global_position)
	var elbow: Vector3 = p.camera.to_local(p.get_first_person_motion_snapshot().joint_landmarks.torch.elbow)
	check(wrist.x < -0.45 and elbow.y < wrist.y, "left hand held beside shoulder with elbow below")
	p.velocity = Vector3.ZERO
	p._update_viewmodel(2.0)

func audit_pocket_motion(p):
	p.velocity = Vector3.ZERO
	p.set_torch_enabled(false);p._update_viewmodel(2.0)
	check(not p.torch_pivot.visible, "torch hidden only after pocket return completes")
	p.set_torch_enabled(true)
	var previous_y: float = p.torch_pivot.position.y
	for i in 70:
		p._update_viewmodel(1.0/60.0)
		check(p.torch_pivot.position.y >= previous_y - 0.004, "draw rises monotonically from below")
		check(p.torch_pivot.position.z >= p.TORCH_GRIP.STOW_POSITION.z - 0.01, "draw never pushes torch forward from chest")
		previous_y = p.torch_pivot.position.y
	p.set_torch_enabled(false)
	check(not p.torch_flame.visible and not p.torch.visible, "flame and light extinguish before pocket return")
	for i in 30:
		p._update_viewmodel(1.0/60.0)
		check(p.torch_pivot.position.y <= previous_y + 0.004, "stow lowers monotonically")
		check(p.torch_arm.visible and not p.left_support_arm.visible, "one gripping left hand stays throughout stowing")
		previous_y = p.torch_pivot.position.y
	var interrupted: Transform3D = p.torch_pivot.transform
	p.set_torch_enabled(true);p._update_viewmodel(0.0)
	check(p.torch_pivot.transform.is_equal_approx(interrupted), "reverse mid-stow without teleporting")
	p._update_viewmodel(0.1)
	check(p.torch_pivot.position.y > interrupted.origin.y, "interrupted stow resumes upward travel")
	p.set_torch_enabled(false);p._update_viewmodel(2.0)
	check(not p.torch_arm.visible and not p.left_support_arm.visible and not p.torch_pivot.visible, "left hand stays stowed after pocket return completes")
	p.set_torch_enabled(true);p._update_viewmodel(2.0)
