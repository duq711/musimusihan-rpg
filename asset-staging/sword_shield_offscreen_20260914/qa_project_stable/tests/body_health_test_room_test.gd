...
func _run() -> void:
	...
	var medicine_count := room.inventory.count_item("healing_draught")
	room.inventory_overlay.health_panel.treatment_buttons.healing_draught.pressed.emit()
	...
	var surgery_count := room.inventory.count_item("surgery_kit")
	room.inventory_overlay.health_panel.treatment_buttons.surgery_kit.pressed.emit()
...
func _check_condition_trials() -> void:
	for condition_id in ExpeditionSession.CONDITION_DRAIN_BONUSES:
		...
		_check(ExpeditionSession.has_condition(condition_id) and body.parts[selected].conditions.has(condition_id), "condition fixture must apply the actual condition to the selected body part: " + condition_id)
		...
		if not treatment.is_empty():
			room.inventory_overlay.health_panel.treatment_buttons[treatment].pressed.emit()
