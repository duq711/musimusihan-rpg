...
func _test_inventory_visual_metadata() -> void:
	...
	_check(ExpeditionInventory.ITEM_DEFINITIONS.size() == 36 + alchemy_count, "item catalog must retain all existing items and catalog-derived alchemy supplies and quality variants")
