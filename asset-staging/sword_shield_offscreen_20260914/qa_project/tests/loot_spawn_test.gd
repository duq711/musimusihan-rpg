...
func _test_trials() -> void:
	...
	for variant: String in VARIANTS:
		...
		_check(room.player.is_timed_interacting() and room.player.chest_hands.active, "model trial must run the real timed hands instead of opening a demonstration overlay")
		_send_f2(room)
