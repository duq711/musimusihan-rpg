...
func _test_every_recipe(capped: bool) -> void:
	for recipe_id: String in COOKING.ordered_recipe_ids():
		...
		_check(is_equal_approx(player.health, minf(100.0, float(before.health) + float(recipe.health))) and is_equal_approx(player.stamina, minf(100.0, float(before.stamina) + float(recipe.stamina))), "food applies authored health/stamina effects and caps")
