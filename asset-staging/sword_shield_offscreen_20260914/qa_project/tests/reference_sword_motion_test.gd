...
func _test_actual_player_uses_delivery() -> void:
	...
	for variant: String in REFERENCE.ATTACK_CLIPS:
		...
		var started := player.begin_sword_attack(variant)
		_check(bool(started.get("accepted", false)), "Contact fixture must initialize the real coordinated attack: " + variant)
		if not bool(started.get("accepted", false)):
			continue
		var coordinated := player._uses_coordinated_sword_motion()
		var hit_time := player.get_melee_hit_time()
		_check(coordinated and is_equal_approx(hit_time, 0.145), "Equipped sword and shield must retain the actual 145ms coordinated contact clock: " + variant)
		# This test samples an exact authored contact rather than advancing a
		# whole attack. Enter ACTIVE only after production start captures its
		# stance/timing flags; the state-machine tests below cover progression.
		player.combat_state = DungeonPlayer.CombatState.ACTIVE
		player.attack_charge = 0.0
		player.state_time = hit_time
		player._shield_impact = 0.0
		player._motion_clock = 0.0
		player._update_viewmodel(0.0)
		var expected := MOTION.sword("active", hit_time, 0.0, false, coordinated, variant)
