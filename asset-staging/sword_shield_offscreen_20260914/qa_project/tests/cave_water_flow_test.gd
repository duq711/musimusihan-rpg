...
func _run() -> void:
	...
	for iteration in range(2):
		...
		_check(cave.enemies_alive == 6 and preload("res://tests/loot_test_helpers.gd").valid_count(cave), "water trial must keep the real playable mine rather than a demonstration scene")
