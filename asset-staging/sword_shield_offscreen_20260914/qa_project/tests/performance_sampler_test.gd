...
func _run() -> void:
	...
	for key in counters:
		if key not in SAMPLER.DIMENSION_NAMES and key != "rendering":
	...
	dormant.size_2d_override = Vector2i(236, 280)
	dormant.size_2d_override_stretch = true
	dormant.scaling_3d_scale = 0.5
	var scaled := sampler.record(15.0).viewports[path] as Dictionary
	check(scaled.width == 472 and scaled.height == 560 and scaled.logical_width == 236 and scaled.render_3d_width == 236, "sampler must distinguish physical target, logical UI and scaled 3D pixels")
	var summary := sampler.summary()
	check(summary.wall_ms.over_16_67ms_percent == 50.0, "frame-time budget must count slow measured frames")
