...
const OPTIONAL_BASELINE := "../asset-staging/sword_shield_left_clear_20260914/baseline/godot-game/assets/animations/reference_sword_motion/motion_manifest.json"
...
func _test_attack(player: DungeonPlayer, variant: String, hold: float, stowed: bool) -> void:
	...
	for frame in 180:
		...
		if not stowed and player.combat_state in [DungeonPlayer.CombatState.WINDUP, DungeonPlayer.CombatState.ACTIVE, DungeonPlayer.CombatState.RECOVERY]:
			...
			_check(shield.origin.distance_to(previous_shield.origin) < 0.06 and shield.basis.get_rotation_quaternion().angle_to(previous_shield.basis.get_rotation_quaternion()) < 0.05, context + ": phase boundaries must not jump or twist the shield.")
			if player.combat_state != DungeonPlayer.CombatState.RECOVERY:
				...
				if elapsed >= 0.12 - 0.000001:
					_check(shield.origin.x < held_shield.origin.x - 0.24, context + ": the shield must clear left before the 145ms sword contact.")
