...
func _run() -> void:
	...
	for i in 239:
		...
		if i > 26 and i < 169:
			...
			if i == 27: first_working_basis = player.bandage_hands.left_arm.basis
	...
	_check(receiving_arm_rotation > 0.35, "receiving arm must pronate and move with the winding hand")
	...
	_check(is_equal_approx(player.bandage_hands.winding, TAU * 2.0), "bandage must wind two close reference passes")
	...
	print("BANDAGE MOTION TEST %s: real consumption, 2 close wraps, continuous pose, pause, completion, interruption, isolation" % ("PASS" if failures.is_empty() else "FAIL"))
