"""Build v4 by repairing the v3 upper-torso cut without regressing limb cleanup.
...
shoulder faces together with the old collar shell.  V4 keeps the donor below
...
def replace_once(source: str, old: str, new: str, label: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"v4 patch {label!r} expected one match, found {count}")
    return source.replace(old, new, 1)


# Reuse the validated v3 pipeline, but write independent v4 artifacts.
source = V3_SCRIPT.read_text(encoding="utf-8")
source = source.replace("v3", "v4").replace("V3", "V4")
...
if source.count(limb_rule) != 2:
    raise RuntimeError(
        "v4 expected two v3 limb material rules, found %d"
...
if source.count(old_upper_vest_rule) != 2:
    raise RuntimeError(
        "v4 expected old/new v3 upper-vest literals, found %d"
...
compiled = compile(source, str(V3_SCRIPT) + " [v4 upper-torso repair]", "exec")
