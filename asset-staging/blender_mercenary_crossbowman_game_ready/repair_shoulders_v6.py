...
top-surface shards, adds two thin closed outer-wool shoulder panels, weights
them to the existing Rigify deform rig, renders QA views and exports v6.
"""
...
OUTPUT_SUMMARY = STAGING / "build_summary_v6.json"
OUTPUT_BLEND = STAGING / "mercenary_crossbowman_game_ready_v6.blend"
OUTPUT_GLB = (
    WORKSPACE
    / "godot-game"
    / "assets"
    / "3d"
    / "dark_fantasy"
    / "mercenary_crossbowman_game_ready_v6.glb"
)
...
def shoulder_top_z(abs_x, transverse):
    outward = (abs_x - 0.20) / 0.30
    return (
        1.548
        - 0.054 * outward
        - 0.030 * transverse * transverse
        + 0.004 * math.cos(transverse * math.pi)
    )
...
def make_shoulder_panel(side):
    ...
    for layer in range(2):
        for ix in range(nx):
            ...
            for iy in range(ny):
                ...
                z = top_z if layer == 0 else top_z - 0.022
    ...
    panel["source"] = "V6 fitted outer-wool shoulder cap"
...
def render_ortho(key, location, target, scale, resolution=(1200, 1200)):
    ...
    path = PREVIEWS / f"mercenary_game_ready_v6_{key}.png"
...
summary["asset"] = "mercenary_crossbowman_game_ready_v6"
...
summary["shoulder_repair"] = {
    "mode": "two fitted closed outer-wool caps weighted to Rigify",
...
print("V6_SUMMARY", json.dumps(summary))
