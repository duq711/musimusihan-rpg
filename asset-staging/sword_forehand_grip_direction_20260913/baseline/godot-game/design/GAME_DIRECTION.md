...
- 2026-09-13 최신 사용자 지시: 복원된 `left_reverse`를 보존하고, `right_diagonal`을 그 역베기와 유사하되 방향만 반대로 바꾼다. 앞선 정베기 승인본 고정과 얕은 우상향·고정 날면 기준은 이번 후속 요청으로 대체한다.
- 현재 후보 03 구현 판단: 정베기 제작 시각 0~0.80초는 원본 역베기의 `1.42 - 새 시각`에 해당하는 동일한 `sword`·`right_arm` 포즈를 사용한다. 회수에서는 원본 0.341666667~0.62초의 준비 정지를 생략하고 새 0.803333333~1.42초에 원본 0.341666667→0초의 기존 포즈로 대기 복귀를 잇는다. 공간 반전·새 팔 생성 없이 기존 포즈를 재사용한다. 별도 0.14초 손목 준비를 제거하여 WINDUP의 시작 자세·높이를 유지하고, 정베기는 활성 0.145초 타격 시점까지 직접 연결한다. 역베기·내려베기의 연결은 기존 0.10초다. 정베기 방패 곡선은 새 시각 격자에서 원래 같은 시각 포즈를 재표본화해 유지하며 바이트 동일성을 뜻하지 않는다. 기존 최소 입력 0.22초·차지 판정 0.24초, 타격 0.145초·활성 0.30초, 피해·기력·방패 규칙은 유지한다.
- 현재 상태: 후보 03을 적용하고 관련 자동 검사 5종과 실제 Vulkan/embedded 촬영을 완료했다. 방패·양손에서 기존 역베기의 총 182 PNG가 변경 전과 같으며, 수정 정베기의 우→좌 진행·대기 복귀를 직접 확인했다. 정베기 진입의 한 프레임 화면 밖 이동은 남아 있으며 상시 가시성·사용자 미학적 승인을 주장하지 않는다. 검격 교차 시험과 이전 후보 이력은 최신 통과 기록과 구분한다. 근거·제한·변경 전 자료는 `../../asset-staging/sword_forehand_from_reverse_20260913/validation_summary.json`, `visual_review.json`, `baseline/`에 보존한다.
