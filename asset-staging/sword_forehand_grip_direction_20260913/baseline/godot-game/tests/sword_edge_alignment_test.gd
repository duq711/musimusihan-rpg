...
## Preparation/cut keep full inverse timing through .80s; recovery skips the
## original preparation hold and returns through existing source poses to idle.
...
func _run() -> void:
	...
	var result := {"contract": "inverse_preparation_and_cut; existing_pose_recovery_without_source_hold", "aesthetic_quality_evaluated": false, "baseline": _test_optional_preservation()}
	if available:
...
func _test_pose_correspondence() -> Dictionary:
	...
	var axis_error := 0.0
	var joint_error := 0.0
	# Preparation and cut retain inverse timing. Recovery intentionally skips
	# the old preparation hold, so its separate test checks existing key poses.
	for index in range(161):
		var time := 0.80 * float(index) / 160.0
	...
	_check(position_error < 0.00005 and axis_error < 0.00005 and joint_error < 0.00005, "The actual sampler must reproduce the same sword and arm poses at opposite times through preparation/cut 0-.80s, without spatial reflection.")
	...
	return {"samples": 161, "max_position_m": position_error, "max_axis_error": axis_error, "max_joint_m": joint_error}


func _test_recovery_preservation() -> Dictionary:
	var original: Dictionary = _manifest.get("clips", {}).get("left_reverse", {})
	var current: Dictionary = _manifest.get("clips", {}).get("right_diagonal", {})
	var source_keys: Array[Dictionary] = []
	for row: Dictionary in original.get("tracks", {}).get("sword", []):
		var time := float(row.time_seconds)
		source_keys.append({"pose": MOTION.sample("left_reverse", time), "arm": MOTION.sample_right_arm("left_reverse", time)})
	var matched_keys := 0
	var current_rows: Array = current.get("tracks", {}).get("sword", [])
	for row: Dictionary in current_rows:
		var time := float(row.time_seconds)
		var pose := MOTION.sample("right_diagonal", time)
		var arm := MOTION.sample_right_arm("right_diagonal", time)
		var matched := false
		for source: Dictionary in source_keys:
			var source_pose: Transform3D = source.pose
			if pose.origin.distance_to(source_pose.origin) >= 0.00002 or not pose.basis.is_equal_approx(source_pose.basis): continue
			var joints_match := true
			for joint: String in ["shoulder", "elbow", "wrist"]:
				joints_match = joints_match and (arm[joint] as Vector3).distance_to(source.arm[joint]) < 0.00003
			if joints_match:
				matched = true
				break
		_check(matched, "Every new forehand key must reuse an existing reverse sword and joint pose; recovery may retime or omit held poses but may not invent spatial poses.")
		if matched: matched_keys += 1
	for index in range(125):
		var time := 0.80 + (DURATION - 0.80) * float(index) / 124.0
		var pose := MOTION.sample("right_diagonal", time)
		_check(pose.origin.is_finite() and pose.basis.is_finite() and pose.basis.get_scale().distance_to(Vector3.ONE) < 0.00001 and absf(pose.basis.determinant() - 1.0) < 0.00001 and (pose.basis.transposed() * pose.basis).is_equal_approx(Basis.IDENTITY), "Recovery interpolation must retain finite, unscaled proper rotations.")
	var end := MOTION.sample("right_diagonal", DURATION)
	var idle := MOTION.sample("idle", 0.0)
	_check(end.origin.distance_to(idle.origin) < 0.00002 and end.basis.is_equal_approx(idle.basis), "Retimed recovery must finish on the original idle sword pose.")
	var end_arm := MOTION.sample_right_arm("right_diagonal", DURATION)
	var idle_arm := MOTION.sample_right_arm("idle", 0.0)
	for joint: String in ["shoulder", "elbow", "wrist"]:
		_check((end_arm[joint] as Vector3).distance_to(idle_arm[joint]) < 0.00003, "Retimed recovery must finish on the original idle arm joint: " + joint)
	return {"matched_existing_pose_keys": matched_keys, "total_keys": current_rows.size(), "idle_endpoint_checked": true}
...
func _test_wrist_anchors(clip: String) -> Dictionary:
	...
	_check(wrist_error < 0.00003 and length_error < 0.00003, clip + ": all retained original joint poses must preserve arm lengths and the original right-hand REST_WRIST anchor, without mirrored hand coordinates.")
