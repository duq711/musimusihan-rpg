...
func _test_combat_sampling(player: DungeonPlayer, fps: int) -> void:
	for variant: String in DATA.ATTACK_CLIPS:
		...
		for frame in range(fps * 2):
			...
			if phase == "windup":
				...
				_check(not player._uses_direct_sword_entry(phase), "After the variant's entry endpoint, contact and recovery must use the original authored path.")
	...
	for variant: String in DATA.ATTACK_CLIPS:
		...
		_check(not player._uses_direct_sword_entry("active") and _near_pose(player.weapon_pivot.transform, boundary_sample.raw_sword), "The exact variant entry endpoint must restore the original blade pose: " + variant)
