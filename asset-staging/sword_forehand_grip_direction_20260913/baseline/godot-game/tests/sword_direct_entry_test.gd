...
func _exercise_attack(player: DungeonPlayer, variant: String, hold: float, stowed: bool) -> void:
	...
	for frame in 420:
		...
		if (player.combat_state == DungeonPlayer.CombatState.ACTIVE and player.state_time >= entry_duration - 0.0000001) or player.combat_state == DungeonPlayer.CombatState.RECOVERY:
			...
			if phase == "active" and absf(player.state_time - 0.145) < 0.000001:
				_check(not player._uses_direct_sword_entry("active"), context + ": exact 145ms contact must use the source pose with no remaining entry override")
	...
	_check(checkpoint_100ms_seen and rejoin_seen and contact_seen and preserved_samples > 10 and source_position_error < 0.00003 and source_angle_error < 0.06, context + ": the variant's entry endpoint, exact 145ms contact and all following active/recovery poses must match the authored track")
...
func _finish() -> void:
	...
	print("SWORD DIRECT ENTRY %s: held WINDUP, forehand 145ms / reverse-overhead 100ms entry, exact contact, release/charge/cost clocks and expedition preservation; target damage is covered by sword_attack_variants" % ("PASS" if failures.is_empty() else "FAIL"))
