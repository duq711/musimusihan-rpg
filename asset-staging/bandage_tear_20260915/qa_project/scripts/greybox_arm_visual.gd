...
func setup(side: int, profile := "greybox", torch_source := false) -> bool:
	...
	if torch_source and profile == "detailed" and _side == -1:
		_source_path = "res://assets/3d/player/hands_detailed/left_hand_torch.glb"
...
func _apply_joint_angle(digit: String, joint: int, radians: float, authored_torch := false) -> void:
	var limit := joint_limits(digit)[joint]
	var angle := radians if authored_torch else clampf(radians, -limit, 0.0)
	...
	if authored_torch and joint == 0 and digit == "thumb":
		joint_rotation = Quaternion(Vector3.BACK, TORCH_GRIP.THUMB_ROOT_ADDUCTION) * joint_rotation
	if authored_torch and joint == 0 and digit != "thumb":
		joint_rotation = Quaternion(Vector3.BACK, TORCH_GRIP.ROOT_ADDUCTION[digit]) * joint_rotation
	if authored_torch:
		joint_rotation = TORCH_GRIP.MANUAL_ROTATIONS[digit + str(joint)]
	if not authored_torch and angle == 0.0 and (joint != 0 or digit != "thumb" or _thumb_opposition == 0.0):
	...
	for binding: Dictionary in _correctives.get(key, []):
		(binding.mesh as MeshInstance3D).set_blend_shape_value(int(binding.index), clampf(-angle / limit, 0.0, 1.0))
...
func set_torch_grip() -> void:
	if _profile != "detailed" or _side != -1:
		set_grip(0.92)
		return
	# Only this measured pose may partially unbend the supplied rest shape.
	# Public sliders keep their original ranges. Reapply after profile/reset.
	var unchanged := is_equal_approx(_thumb_opposition, TORCH_GRIP.THUMB_OPPOSITION)
	for digit: String in DIGITS:
		unchanged = unchanged and (_joint_angles.get(digit, Vector3.ZERO) as Vector3).is_equal_approx(TORCH_GRIP.JOINT_ANGLES[digit])
	if unchanged: return
	_thumb_opposition = TORCH_GRIP.THUMB_OPPOSITION
	for digit: String in DIGITS:
		for joint in 3:
			_apply_joint_angle(digit, joint, TORCH_GRIP.JOINT_ANGLES[digit][joint], true)
	grip_amount = 1.0
	string_hook_amount = 0.0
