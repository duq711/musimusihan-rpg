...
static func add_flame(root: Node3D, label: String, offset: Vector3, size: Vector2, angle: float, phase: float, strength: float) -> void:
	...
	material.set_shader_parameter("billow_right", card.basis.inverse() * Vector3.RIGHT)
	material.set_shader_parameter("billow_forward", card.basis.inverse() * Vector3.BACK)
...
static func create() -> Node3D:
	...
	for i in 8:
		...
		add_flame(root,"ClothFlame_%d"%i,Vector3(sin(angle)*.085,0,cos(angle)*.085),Vector2(.27,.78),angle,float(i)*7.7,.4)
	# Crossing upper tongues keep the flame readable around the torch.
	for i in 3:
		add_flame(root,"TipFlame_%d"%i,Vector3(0,CLOTH_TOP-CLOTH_BASE-.06,0),Vector2(.50,.90),PI*float(i)/3.0,11.0+float(i)*17.0,.65)
	return root
