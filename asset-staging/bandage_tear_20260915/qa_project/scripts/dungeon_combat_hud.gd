...
func activate_slot(index: int) -> Dictionary:
 ...
 if get_tree().paused or player.safe_zone_mode or player.camping or player.is_paralyzed() or player.current_trap != null or player.is_timed_interacting() or player.combat_state != DungeonPlayer.CombatState.READY or player.is_bandage_motion_active():
 ...
 if not equip_slot.is_empty():
  ...
  result = player.use_consumable(id, bag)
 refresh()
 return result
...
func _draw() -> void:
 ...
 for i in warnings.size():
  var pos := Vector2(159+(i/5)*39,512+(i%5)*35)
 ...
 if not get_tree().paused and not (is_instance_valid(player.hud) and player.hud.bow_spread_reticle.visible):
