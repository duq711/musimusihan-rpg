...
func _show_work() -> void:
	dialogue_line_label.text = "돈을 벌고 싶다면 은신처 증류기에서 술을 만들어 오게. 정제 브랜디는 36, 압생트는 51크라운에 사지. 특급은 더 쳐준다네. 필요한 포도주·증류주·약초·빈 병은 여기서 계속 살 수 있네."
...
func _rebuild_stock_rows() -> void:
	...
	for item_value in item_ids:
		...
		var unlimited := ExpeditionSession.is_stock_unlimited(item_id)
		var stock_text := "상시 공급" if unlimited else "재고 %d" % quantity
		var row := _item_row_button(
			"%s    %s    %d 크라운" % [str(definition.get("name", item_id)), stock_text, price],
			"Stock_%s" % item_id,
			item_id
		)
		row.disabled = not unlimited and quantity <= 0
...
func _rebuild_bag_rows() -> void:
	...
	for item_value in item_ids:
		...
		var sellable := inventory.count_sellable_item(item_id)
		var protected := quantity - sellable
		var row_caption := "%s    보유 %d    개당 %d 크라운" % [str(definition.get("name", item_id)), quantity, price]
		if protected > 0:
			row_caption = "%s    판매 %d · 보호 %d    %d 크라운" % [str(definition.get("name", item_id)), sellable, protected, price]
		var row := _item_row_button(
			row_caption,
			"Bag_%s" % item_id,
			item_id
...
func _select_bag_item(item_id: String) -> void:
	...
	if inventory.count_sellable_item(item_id) == 0 and inventory.count_item(item_id) > 0:
		trade_status_label.text = "제작·개조 무기는 대장간에서 관리합니다. 이 무기는 판매되지 않습니다."


func _refresh_selection_details() -> void:
	buy_button.disabled = selected_buy_id.is_empty() or (not ExpeditionSession.is_stock_unlimited(selected_buy_id) and ExpeditionSession.get_stock_quantity(selected_buy_id) <= 0)
	sell_button.disabled = selected_sell_id.is_empty() or inventory.count_sellable_item(selected_sell_id) <= 0
	if selected_buy_id.is_empty():
		...
		if ExpeditionSession.is_stock_unlimited(selected_buy_id):
			buy_detail_label.text += "\n상시 공급 · 증류 재료를 계속 구매할 수 있습니다."
	if selected_sell_id.is_empty():
		...
		var sellable := inventory.count_sellable_item(selected_sell_id)
		var protected := inventory.count_item(selected_sell_id) - sellable
		if protected > 0:
			sell_detail_label.text = "%s\n판매 가능 %d · 제작/개조 보호 %d\n제작·개조 무기는 대장간에서 관리합니다.\n일반 무기만 개당 %d 크라운에 판매합니다." % [str(sell_definition.get("name", selected_sell_id)), sellable, protected, ExpeditionSession.get_sell_price(selected_sell_id)]
...
func _transaction_error(reason: String) -> String:
	match reason:
		...
		"protected_weapon":
			return "제작·개조 무기는 판매되지 않습니다. 대장간에서 관리하십시오."
		_:
