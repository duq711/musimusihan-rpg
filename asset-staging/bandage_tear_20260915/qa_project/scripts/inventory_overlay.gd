...
const HEALTH_CHROME := preload("res://scripts/health_chrome.gd")
const COMBINED_LOADOUT := preload("res://scripts/combined_loadout_panel.gd")
...
var health_chrome: HEALTH_CHROME
var combined_loadout_panel: COMBINED_LOADOUT
var container_return_button: Button
var health_tab_button: Button
var equipment_tab_button: Button
var _compact_health_tab_button: Button
var _compact_equipment_tab_button: Button
var _compact_close_button: Button
...
func _input(event: InputEvent) -> void:
	...
	if event is InputEventKey and event.pressed and not event.echo and event.keycode == KEY_ESCAPE:
		...
		elif health_tab_active and health_panel != null and health_panel.handle_cancel():
			get_viewport().set_input_as_handled()
	elif event is InputEventKey and event.pressed and not event.echo and health_tab_active:
		if item_detail_window.is_open() or quantity_dialog_root.visible or event.ctrl_pressed or event.alt_pressed or event.meta_pressed:
			return
		var keys := [KEY_1, KEY_2, KEY_3, KEY_4, KEY_5, KEY_6, KEY_7, KEY_8, KEY_9, KEY_0]
		var key_index := keys.find(event.keycode)
		if key_index >= 0 and key_index < combined_loadout_panel.quick_use_item_ids.size():
			_on_quick_use_requested(combined_loadout_panel.quick_use_item_ids[key_index])
			get_viewport().set_input_as_handled()
...
func open_inventory(model: ExpeditionInventory, status_provider: Callable = Callable()) -> void:
	...
	health_panel.reset_presentation()
...
func close() -> void:
	if item_detail_window != null and item_detail_window.is_open():
		item_detail_window.handle_cancel()
		return
	if health_tab_active and health_panel != null and health_panel.handle_cancel():
		return
...
func set_status(text_value: String) -> void:
	...
	combined_loadout_panel.set_status(text_value)
...
func _build_interface() -> void:
	...
	var navigation_items := ["종합정보", "장비·건강상태", "건강상태", "스킬", "지도", "임무"]
	var navigation_widths := [80, 148, 0, 56, 56, 56]
	...
	for tab_index in range(navigation_items.size()):
		...
		if tab_index in [1, 2]:
			...
			if tab_index == 2:
				...
				_compact_health_tab_button = tab_button
				tab_button.tooltip_text = "부위별 체력과 치료"
				tab_button.pressed.connect(open_health_tab)
			else:
				equipment_tab_button = tab_button
				_compact_equipment_tab_button = tab_button
				health_tab_button = tab_button
				_compact_health_tab_button = tab_button
				tab_button.tooltip_text = "장비·건강 상태 통합 화면"
	...
	_compact_close_button = close_button
...
func _build_inventory_panel() -> void:
	...
	var equipment_positions := {
		...
		"waist_pouch": Vector2(16, 202),
		"backpack": Vector2(256, 202)
	}
	var equipment_sizes := {
		"head": Vector2(88, 67),
		...
		"waist_pouch": Vector2(96, 52),
		"backpack": Vector2(96, 52)
	}
	...
	for socket_position in [Vector2(90, 46), Vector2(250, 46)]:
...
func _build_health_panel() -> void:
	health_chrome = HEALTH_CHROME.new()
	health_chrome.name = "HealthPageChrome"
	health_chrome.visible = false
	modal_root.add_child(health_chrome)
	health_chrome.build()
	health_chrome.equipment_requested.connect(open_equipment_tab)
	health_chrome.health_requested.connect(open_health_tab)
	health_chrome.close_requested.connect(close)
	...
	combined_loadout_panel = COMBINED_LOADOUT.new()
	combined_loadout_panel.name = "CombinedLoadoutPanel"
	combined_loadout_panel.texture_provider = _item_texture
	modal_root.add_child(combined_loadout_panel)
	combined_loadout_panel.build()
	combined_loadout_panel.inventory_pressed.connect(_on_inventory_slot_pressed)
	combined_loadout_panel.inventory_gui_input.connect(_on_inventory_slot_gui_input)
	combined_loadout_panel.equipment_pressed.connect(_on_equipment_slot_pressed)
	combined_loadout_panel.equipment_gui_input.connect(_on_equipment_slot_gui_input)
	combined_loadout_panel.quick_use_requested.connect(_on_quick_use_requested)
	container_return_button = _action_button("상자 보관함으로", Vector2(468, 65), Vector2(155, 28), Color(0.07, 0.06, 0.04))
	container_return_button.name = "ReturnToContainer"
	container_return_button.z_index = 20
	modal_root.add_child(container_return_button)
	container_return_button.pressed.connect(_return_to_container)
...
func _return_to_container() -> void:
	if loot_container == null:
		return
	_ensure_built()
	health_panel.reset_presentation()
...
func _on_quick_use_requested(item_id: String) -> void:
	...
	if str(definition.get("category", "")) != "consumable":
		return
	_on_health_consumable_requested(item_id)
...
func _apply_health_tab_layout() -> void:
	...
	health_chrome.visible = health_tab_active
	combined_loadout_panel.visible = health_tab_active
	container_return_button.visible = health_tab_active and loot_container != null
	equipment_root.visible = not health_tab_active
	inventory_root.visible = not health_tab_active
	status_footer.visible = not health_tab_active
	(modal_root.get_node("InventoryNavigationBar") as Control).visible = not health_tab_active
	(modal_root.get_node("InnerFrame") as Control).visible = not health_tab_active
	_compact_close_button.visible = not health_tab_active
	health_tab_button = health_chrome.tab_buttons.health if health_tab_active else _compact_health_tab_button
	equipment_tab_button = health_chrome.tab_buttons.equipment if health_tab_active else _compact_equipment_tab_button
	close_button = health_chrome.close_button if health_tab_active else _compact_close_button
...
func _fit_health_layout() -> void:
	...
	# Both tab states share the viewport fit so navigation remains reachable
	# before entering health view and after returning to equipment.
...
func _refresh() -> void:
	...
	_restore_selected_details()
	_refresh_open_item_details()
	combined_loadout_panel.refresh(inventory_model, selected_source, selected_index, selected_equipment_slot)
	combined_loadout_panel.set_status(_action_status_label.text)
	_apply_health_tab_layout()
...
func _refresh_tactical_summary() -> void:
	...
	var health := float(actor_status.get("health", BODY_HEALTH.MAX_HEALTH))
	var max_health := maxf(1.0, float(actor_status.get("max_health", BODY_HEALTH.MAX_HEALTH)))
	if inventory_model == null:
		_character_stat_values_label.text = "15\n15\n15\n15\n15\n\n%d / %d\n0.0 KG" % [roundi(health), roundi(max_health)]
		_quick_loadout_label.text = "장착 0/%d  ·  회수 0" % ExpeditionInventory.EQUIPMENT_ORDER.size()
		return
	...
	_quick_loadout_label.text = "장착 %d/%d  ·  회수 %d" % [equipped_count, ExpeditionInventory.EQUIPMENT_ORDER.size(), inventory_model.raid_loot_value()]
...
func refresh_status_readout() -> void:
	...
	if health_chrome != null:
		health_chrome.update_status(actor_status, survival)
...
func _actor_status_snapshot() -> Dictionary:
	var body := BODY_HEALTH.snapshot(ExpeditionSession.body_health, ExpeditionSession.active_conditions)
	var snapshot := {
		"health": body.health,
		"max_health": BODY_HEALTH.MAX_HEALTH,
		"body_health": body,
		"stamina": 100.0,
		"max_stamina": 100.0
	}
