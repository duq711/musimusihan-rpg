...
const EQUIPMENT_ORDER := ExpeditionInventory.EQUIPMENT_ORDER
const EQUIPMENT_NAMES := {"head": "머리", "body": "흉부", "weapon": "무기", "offhand": "보조 장비", "utility": "도구", "waist_pouch": "허리 파우치", "backpack": "배낭"}
const EQUIPMENT_RECTS := {
	"head": Rect2(658, 108, 67, 80),
	"body": Rect2(733, 108, 127, 125),
	"weapon": Rect2(658, 196, 67, 137),
	"offhand": Rect2(733, 241, 60, 92),
	"utility": Rect2(801, 241, 59, 92),
	"waist_pouch": Rect2(658, 341, 97, 61),
	"backpack": Rect2(763, 341, 97, 61),
}
const SECTIONS := [
...
var storage_equipment_labels: Dictionary = {}
...
func build() -> void:
	...
	for storage_slot: String in ["waist_pouch", "backpack"]:
		var at := Vector2(1055, 76) if storage_slot == "waist_pouch" else Vector2(983, 251)
		storage_equipment_labels[storage_slot] = _label(self, "", at, Vector2(184, 18) if storage_slot == "waist_pouch" else Vector2(256, 17), 11, MUTED)
		storage_equipment_labels[storage_slot].horizontal_alignment = HORIZONTAL_ALIGNMENT_RIGHT
...
func refresh(model: ExpeditionInventory, selected_source: String = "", selected_index: int = -1, selected_equipment_slot: String = "") -> void:
	...
	for slot: String in storage_equipment_labels:
		var equipped_id := str(model.equipment.get(slot, "")) if model != null else ""
		storage_equipment_labels[slot].text = ExpeditionInventory.get_item_name(equipped_id) if not equipped_id.is_empty() else "장착하지 않음"
