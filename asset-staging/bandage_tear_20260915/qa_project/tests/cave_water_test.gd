...
func _run() -> void:
	...
	for index in water.pools.size():
		...
		var face_cross := (vertices[faces[1]] - vertices[faces[0]]).cross(vertices[faces[2]] - vertices[faces[0]])
		_check(face_cross.dot(Vector3.UP) < 0.0, "clockwise water front faces must agree with upward normals for top and underside shading")
