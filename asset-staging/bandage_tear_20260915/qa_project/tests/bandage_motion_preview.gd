...
var output := "res://artifacts/visual_qa/bandage_forearm/low_tear_08"
...
func _run() -> void:
	...
	for i in count:
		...
		if picture.save_png(path.path_join(filename)) != OK:
			...
			quit(1)
			return
		frames.append({"file": filename, "time": i * step, "active": player.is_bandage_motion_active(), "winding": player.bandage_hands.winding, "torn": player.bandage_hands.torn, "hands_visible": player.bandage_hands.visible, "weapon_visible": player.weapon_pivot.visible})
