...
func _check_dungeon_hud_trial() -> void:
	...
	_check(panel.activate_slot(5).accepted and room.inventory.count_item("pilgrim_ration") == count - 1 and "hunger" not in panel.warnings, "Trial hotbar uses real food and removes warning")
	_f2_menu()
	room.run_feature("dungeon_combat_hud")
