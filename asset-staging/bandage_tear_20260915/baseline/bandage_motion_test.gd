...
func _run() -> void:
	...
	print("BANDAGE MOTION TEST %s: stow/lift/redraw, real consumption, 3 low wraps, continuous pose, pause, completion, interruption, isolation" % ("PASS" if failures.is_empty() else "FAIL"))
