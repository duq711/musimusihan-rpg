...
func set_time(seconds: float) -> void:
	...
	var leave := smoothstep(3.50, 4.15, motion_time)
	...
	# The three one-second passes and their finishing poses are unchanged.
	...
	var press := smoothstep(3.05, 3.30, motion_time)
	var finish_pull := smoothstep(3.30, 3.50, motion_time)
	...
	roll.visible = true
	cloth.visible = true
	strip.visible = motion_time < 3.40
	_build_wrap(maxf(0.06, winding))
	_build_strip(contact, roll_position - radial * 0.022, -right_basis.x)
