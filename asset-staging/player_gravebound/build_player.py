...
SOURCE_SIDE = ROOT/'concept-art/player_gravebound/concept_side.png'
...
shutil.copy2(SOURCE_SIDE,WORK/'gravebound_concept_side.png')
...
side_img=bpy.data.images.load(str(WORK/'gravebound_concept_side.png'));side_img.pack()
...
cowl=tube('Gravebound_InnerNeckCowl',[(274,512,58,.102,.001),(298,512,63,.108,0),(322,512,75,.121,0),(341,512,85,.132,0)],hoodmat,folds=.001,cap=False)
cowl_mod=cowl.modifiers.new('CowlClothThickness','SOLIDIFY');cowl_mod.thickness=.005
bpy.context.view_layer.objects.active=cowl;bpy.ops.object.modifier_apply(modifier=cowl_mod.name)
...
blended={}
def side_u_bounds(name,py):
 if 'InnerNeckCowl' in name:return (470,550)
 if 'AnatomicalHead' in name or name.endswith('_Eyes'):return (405,475)
 if 'PointHood' in name:return (405,580)
 if 'Mantle' in name:return (406,640)
 if 'Sleeve' in name:return (497,612)
 if 'Bracer' in name:return (480,585)
 if 'Glove' in name or 'Finger_' in name:return (464,535)
 if 'Trousers' in name:return (454,604)
 if 'BootCuff' in name:return (465,600)
 if 'Boot' in name:
  toe=max(0,min(1,(py-1310)/90));return (478-103*toe,594)
 if 'Pouch' in name:return (428,508)
 if 'Coat' in name:return (387,642)
 return (400,615)
for o in parts:
 ...
 depth_min=min(v.co.y for v in o.data.vertices);depth_max=max(v.co.y for v in o.data.vertices)
 for loop in o.data.loops:
  ...
  py_side=72+(1.78265-v.z)/(1.78265-.00767)*(1467-72)
  x0,x1=side_u_bounds(o.name,py_side)
  depth_t=max(0,min(1,(v.y-depth_min)/max(.001,depth_max-depth_min)))
  if 'InnerNeckCowl' in o.name:
   # Sample only folded hood cloth above the clasp, on every cowl face.
   u=max(0,min(1,(v.x+.115)/.23));q=max(0,min(1,(v.z-1.423)/.086))
   side_uv.data[loop.index].uv=((470+80*u)/1024,1-(282-87*q)/1536)
  else:side_uv.data[loop.index].uv=((x0+(x1-x0)*depth_t)/1024,1-py_side/1536)
 ...
 if key not in blended:
  ...
  uv=nt.nodes.new('ShaderNodeUVMap');uv.uv_map='SideProjection'
  tex=nt.nodes.new('ShaderNodeTexImage');tex.image=side_img;nt.links.new(uv.outputs[0],tex.inputs['Vector']);side_color=tex.outputs['Color']
...
bpy.ops.object.mode_set(mode='EDIT');bpy.ops.mesh.select_all(action='SELECT');bpy.ops.uv.smart_project(angle_limit=1.15,island_margin=.008,area_weight=.45);bpy.ops.object.mode_set(mode='OBJECT')
...
scene.render.engine='CYCLES';scene.cycles.samples=1;scene.render.bake.use_clear=True;scene.render.bake.margin=3
# Bake the shared atlas in one mesh operation. Sequential object dilation was
# overwriting neighbouring islands (confirmed against the dark source sleeve).
bpy.ops.object.select_all(action='DESELECT');bake_parts=[]
for o in parts:
 copy=o.copy();copy.data=o.data.copy();copy.parent=None;copy.matrix_world=o.matrix_world.copy();scene.collection.objects.link(copy);copy.select_set(True);bake_parts.append(copy)
bpy.context.view_layer.objects.active=bake_parts[0];bpy.ops.object.join();bake_mesh=bpy.context.object
bpy.ops.object.bake(type='EMIT')
bake_data=bake_mesh.data;bpy.data.objects.remove(bake_mesh,do_unlink=True);bpy.data.meshes.remove(bake_data)
...
report={'revision':5,'height_m':max(v.z for v in allv)-min(v.z for v in allv),'bounds_blender':[[min(v[i] for v in allv) for i in range(3)],[max(v[i] for v in allv) for i in range(3)]],'mesh_count':len(parts),'triangles':sum(sum(len(p.vertices)-2 for p in o.data.polygons) for o in parts),'mesh_names':[o.name for o in parts],'glb_bytes':(OUT/'gravebound_player.glb').stat().st_size,'uv_channels':['Atlas'],'atlas_size':[4096,4096],'atlas_padding_pixels':3,'atlas_bake_mode':'single combined mesh','front_axis_godot':'-Z','source_concept':str(SOURCE),'source_concept_back':str(SOURCE_BACK),'source_concept_side':str(SOURCE_SIDE)}
