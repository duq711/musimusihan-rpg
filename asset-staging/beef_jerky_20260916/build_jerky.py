"""Procedural Mac Blender authoring; supplied SBSAR textures, closed meshes.
...
# Settle the irregular pieces under gravity, keeping the result as static meshes.
# The closed authored surface drives a small-margin convex hull, not a box.
pile=[]
bpy.ops.mesh.primitive_plane_add(size=2, location=(0,0,-.0002))
physics_floor=bpy.context.object
bpy.ops.rigidbody.object_add()
physics_floor.rigid_body.type='PASSIVE'
physics_floor.rigid_body.collision_shape='MESH'
physics_floor.rigid_body.use_margin=True
physics_floor.rigid_body.collision_margin=.0001
placements=[(-.012,-.010,18),(.012,.010,-41),(-.009,.008,35),(.0,-.006,-20),(.027,-.002,13),(-.010,.0,75),(-.014,-.007,-12),(.007,-.010,21)]
for i,(source,(px,py,rz)) in enumerate(zip(pieces,placements)):
    ...
    obj.location=(px,py,.035+i*.04)
    obj.rotation_euler=(math.radians((i%3-1)*5),math.radians((i%2*2-1)*5),math.radians(rz))
    bpy.ops.rigidbody.object_add()
    rb=obj.rigid_body;rb.mass=.016;rb.collision_shape='CONVEX_HULL';rb.friction=.85;rb.restitution=0
    rb.use_margin=True;rb.collision_margin=.00015;rb.linear_damping=.55;rb.angular_damping=.7
    pile.append(obj)
world=scene.rigidbody_world
world.substeps_per_frame=12;world.solver_iterations=40
world.point_cache.frame_start=1;world.point_cache.frame_end=240
for frame in range(1,241):scene.frame_set(frame)
deps=bpy.context.evaluated_depsgraph_get()
settled=[obj.evaluated_get(deps).matrix_world.copy() for obj in pile]
for obj,matrix in zip(pile,settled):
    bpy.ops.object.select_all(action='DESELECT');obj.select_set(True);bpy.context.view_layer.objects.active=obj
    bpy.ops.rigidbody.object_remove();obj.matrix_world=matrix
bpy.data.objects.remove(physics_floor,do_unlink=True)
scene.frame_set(1)
...
cam.data.ortho_scale=.40;aim(cam,(0,0,.025))
