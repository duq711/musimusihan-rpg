"""Mac-only non-destructive reverse tail: horizontal continuation, hidden reset."""
...
cut=data['clips']['left_reverse']; rows=cut['tracks']['sword']
...
# First control retains incoming rightward grip speed; zero end tangent.
c1=g0+velocity*(.09/3); c1.x=min(end_g.x,max(g0.x,c1.x))
c1.y=g0.y;c1.z=g0.z
bpy.ops.object.select_all(action='SELECT');bpy.ops.object.delete(use_global=False)
controls={n:rig.make_control('Reverse_'+n) for n in ['sword','grip','shoulder','elbow','wrist']}
...
assert all(data['clips'][k]==v for k,v in original['clips'].items() if k!='left_reverse')
assert cut['tracks']['shield']==original['clips']['left_reverse']['tracks']['shield']
assert all(r==s for r,s in zip(rows,original['clips']['left_reverse']['tracks']['sword']) if r['time_seconds']<=.71)
scene=bpy.context.scene;scene.render.fps=120;scene.frame_end=172
scene['description']='Preserved reverse through contact .71s; same horizontal swing continues right through .80s; lower and reorient outside view, then lift without wrist turn.'
out=ROOT/'candidate_01';out.mkdir(exist_ok=True)
(out/'motion_manifest.json').write_text(json.dumps(data,separators=(',',':')))
bpy.ops.wm.save_as_mainfile(filepath=str(out/'Reverse_Followthrough.blend'))
...
(out/'qa.json').write_text(json.dumps(qa,indent=2)+'\n');print('REVERSE FINISH AUTHOR PASS',json.dumps(qa))
