...
func _run() -> void:
	...
	print("CAVE ART DETAILS COUNTS: rocks=%d boards=%d wall=%d variants=%d batches=%d" % [state.rock_count, state.board_count, state.get("wall_outcrop_count", 0), state.variant_count, state.multimesh_count])
	...
	for batch_data: Dictionary in details.call("get_render_batches"):
		submitted[str(batch_data.name)] = batch_data
	...
	for batch_node in batches:
		...
		_check(multimesh != null and multimesh.instance_count > 0, "each dressing batch must render real mesh instances")
		if multimesh == null:
			continue
		var mesh := multimesh.mesh
		var submission: Dictionary = submitted.get(str(batch.name), {})
		_check(not submission.is_empty() and submission.get("transforms", []).size() == multimesh.instance_count, "each renderer batch must receive the verified number of CPU placement transforms")
	...
	_check(instances == int(state.rock_count) + int(state.board_count), "reported rock and board counts must match actual MultiMesh instances")
