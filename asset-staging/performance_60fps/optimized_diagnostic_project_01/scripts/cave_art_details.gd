...
func _create_multimeshes() -> void:
	for key: String in _batches:
		...
		instance.material_override = _wood_material if int(batch.variant) >= ROCK_VARIANTS else _rock_material
...
func get_state_snapshot() -> Dictionary:
	return {"ready": _ready_to_view, "rock_count": _rock_count, "board_count": _board_count,
		...
		"variant_count": _meshes.size(), "multimesh_count": _batches.size(),
...
func get_render_batches() -> Array[Dictionary]:
	...
	for key: String in _batches:
		var batch: Dictionary = _batches[key]
		result.append({"name": "GroundedDetritus_" + key, "variant": int(batch.variant),
			"transforms": batch.transforms.duplicate(true)})
	return result
