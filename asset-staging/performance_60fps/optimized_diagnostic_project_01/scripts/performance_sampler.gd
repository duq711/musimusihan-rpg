...
func record(wall_ms: float) -> Dictionary:
	...
	for viewport in viewports:
		...
		var counters: Dictionary = {"cpu_ms": cpu, "gpu_ms": gpu, "rendering": rendering, "width": viewport.get_visible_rect().size.x, "height": viewport.get_visible_rect().size.y, "draw_calls": 0, "primitives": 0, "objects": 0}
...
func summary() -> Dictionary:
	...
	for path in samples[0].viewports:
		...
		for key in samples[0].viewports[path]:
			if key in ["width", "height", "rendering"]:
				continue
			var values: Array[float] = []
			for frame in samples:
