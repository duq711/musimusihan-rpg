...
func _test_catalog_and_non_reset_open() -> void:
	...
	for id: String in STAT_IDS:
		...
		if sliders.has(id) and spins.has(id):
			_check(sliders[id].min_value == (1.0 if id == "health" else 0.0) and sliders[id].max_value == 100.0, "slider range must match live safe limits: " + id)
...
func _test_bounds_and_isolation() -> void:
	...
	for id: String in STAT_IDS:
		...
		_check(room.set_test_stat(id, 500.0) and _value(id) == 100.0, "high input must clamp to maximum: " + id)
		for other: String in STAT_IDS:
			if other != id:
				_check(_value(other) == others[other], "editing %s must not change %s" % [id, other])
	_check(room.player.health == 100.0 and room.player.combat_state != DungeonPlayer.CombatState.DEAD, "health range changes must not trigger death flow")
...
func _test_guardrails_and_reset() -> void:
	...
	await process_frame
	_check(room.player.health == 100.0 and room.player.stamina == 100.0 and ExpeditionSession.stress == 0.0, "recovery must discard stale pending health/stress edits after applying the actual recovery")
	...
	_check(room.player.health == 100.0 and room.player.stamina == 100.0 and ExpeditionSession.hunger == 100.0 and ExpeditionSession.thirst == 100.0 and ExpeditionSession.stress == 0.0, "reset must restore the normal fresh trial values")
	room.run_feature("survival_controls")
	for id: String in STAT_IDS:
