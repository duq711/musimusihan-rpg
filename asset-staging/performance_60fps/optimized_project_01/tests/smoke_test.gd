...
func _run() -> void:
	...
	# A held shield blocks only the front arc, spends stamina, and still takes chip damage.
	...
	_check(player.health < health_before_block and player.health > health_before_block - 20.0, "shield must reduce frontal health damage")
