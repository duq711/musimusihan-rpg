...
func _run() -> void:
	...
	var arch_infills := 0
	var ceiling_closures := 0
	for node in (hideout.region_nodes["central"] as Node3D).get_children():
		var infill_kind := str(node.get_meta("structural_infill", ""))
		if infill_kind.is_empty():
			continue
		arch_infills += int(infill_kind == "ArchUpperMasonryInfill")
		ceiling_closures += int(infill_kind == "CeilingWallJointClosure")
		_check(node.find_child("BondedMasonryBlocks", true, false) != null, "upper facade uses the production masonry shared with the test-room gallery")
		_check(node.find_children("*", "CollisionShape3D", true, false).is_empty(), "upper facade infill must leave original passage physics unchanged")
	_check(arch_infills == 6 and ceiling_closures == 4, "production central hall must close all six arch headers and four ceiling joins")
