...
func update_stress(current: float, maximum := 100.0, stage := "") -> void:
	...
	stress_bar.add_theme_stylebox_override("fill", _panel_style(accent.darkened(0.15), accent, 1))
	stress_label.text = "스트레스 %d / %d · %s" % [roundi(value), roundi(limit), caption]
	# The label sits on the filled bar; a pale neutral remains readable even
	# when the extreme-stress fill is bright red.
	stress_label.add_theme_color_override("font_color", Color(0.98, 0.94, 0.98))
...
func update_survival(hunger: float, thirst: float, maximum := 100.0) -> void:
	...
	if is_instance_valid(hunger_label):
		hunger_label.text = "포만감 %d / %d" % [roundi(hunger), roundi(maximum)]
		hunger_label.add_theme_color_override("font_color", _need_color(hunger, Color(0.94, 0.84, 0.64)))
	if is_instance_valid(thirst_label):
		thirst_label.text = "수분 %d / %d" % [roundi(thirst), roundi(maximum)]
		thirst_label.add_theme_color_override("font_color", _need_color(thirst, Color(0.67, 0.88, 0.91)))
...
func update_conditions(conditions: Dictionary, drain_multiplier := 1.0) -> void:
	...
	if conditions.is_empty():
		condition_label.text = "상태 · 안정"
		condition_label.add_theme_color_override("font_color", Color(0.55, 0.62, 0.61))
		return
	...
	condition_label.text = "상태 · %s · 소모 ×%.2f" % [" / ".join(entries), drain_multiplier]
	condition_label.add_theme_color_override("font_color", Color(0.91, 0.42, 0.31))


func update_weapon_state(text_value: String, color := Color(0.72, 0.77, 0.78)) -> void:
	state_label.text = text_value
...
func update_archery(equipped: bool, arrows: int, draw_ratio: float, drawing: bool, cooldown: float, spread_degrees: float = 0.0, camera_fov_degrees: float = 76.0) -> void:
	...
	arrow_count_label.add_theme_color_override("font_color", Color(0.91, 0.77, 0.5) if arrows > 0 else Color(0.95, 0.36, 0.23))
	...
	bow_accuracy_label.add_theme_color_override("font_color", accuracy_color)
