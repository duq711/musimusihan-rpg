...
const MAX_UPPER_ARM_LENGTH := 0.95
...
func _pose_hand(hand: Node3D, side: int) -> void:
	...
	for index in 4:
		...
		arm.call("set_finger_curl", index, -0.14 - amount * 0.07 - tapping * 0.09, -0.26 - tapping * 0.12)
	arm.call("set_finger_curl", 4, -0.22 - sin(beat * 0.83) * 0.13 * fidget_weight, -0.28 - sin(beat + 0.4) * 0.16 * fidget_weight)
...
func _pose_sleeve(hand: Node3D, side: int) -> void:
	...
	(_arms[side] as Node3D).call("fit_arm", upper_start, elbow)
