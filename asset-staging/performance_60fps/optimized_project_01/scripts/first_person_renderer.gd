...
func sync_view() -> void:
	if not is_instance_valid(source_camera) or not source_camera.is_inside_tree():
		return
	var active := _source_viewport.get_camera_3d() == source_camera and source_camera.is_visible_in_tree()
	...
	var view_size := Vector2i(_source_viewport.get_visible_rect().size)
	...
	_source_viewport.scaling_3d_mode = Viewport.SCALING_3D_MODE_FSR2 if budget_scale < _original_scaling_scale else _original_scaling_mode
	viewport.size = RESOLUTION_BUDGET.equipment_size_for(view_size)
