...
func _build_workshop() -> void:
	...
	_add_static_box(region, "BrokenWorkbenchCollision", Vector3(11.2, 0.62, -7.15), Vector3(4.6, 1.22, 1.25), wood_material, false)
	for index in 4:
		_add_visual_box(region, "WorkbenchTopPlank_%d" % index, Vector3(11.2, 1.13, -7.15 + (float(index) - 1.5) * 0.30), Vector3(4.60, 0.22, 0.288), wood_material)
	_add_visual_box(region, "WorkbenchLowerStretcher", Vector3(11.2, 0.35, -7.15), Vector3(3.70, 0.18, 0.20), wood_material)
	for x in [9.4, 13.0]:
		for side in [-1.0, 1.0]:
			var leg := _add_visual_box(region, "WorkbenchLeg_%s_%s" % [str(x), str(side)], Vector3(x, 0.47, -7.15 + side * 0.42), Vector3(0.25, 0.95, 0.24), wood_material, Vector3(side * -6.0, 0.0, 4.0 if x < 11.0 else -5.0))
			DungeonLootChest._add_rivet(leg, Vector3(0, 0.24, side * 0.128), Vector3.BACK * side, metal_material)
		_add_visual_box(region, "WorkbenchLegBrace_%s" % str(x), Vector3(x, 0.39, -7.15), Vector3(0.20, 0.14, 1.02), wood_material)
	for side in [-1.0, 1.0]:
		_add_visual_box(region, "WorkbenchUpperStretcher_%s" % str(side), Vector3(11.2, 0.96, -7.15 + side * 0.48), Vector3(3.97, 0.14, 0.12), wood_material)
	_add_visual_box(region, "Whetstone", Vector3(10.0, 1.32, -7.12), Vector3(0.72, 0.22, 0.34), stone_material, Vector3(0.0, 14.0, 0.0))
	_build_rusty_vice(region, Vector3(12.65, 1.35, -7.15))
...
func _build_hearth(parent: Node3D) -> void:
	...
	var vessel_iron := AGED_SURFACES.pitted_iron(Color(0.14, 0.15, 0.15), 4.0)
	vessel_iron.metallic_specular = 0.16
	var pot := HEARTH_VISUAL.create_vessel(0.36, 0.48, vessel_iron, true)
	pot.name = "CookingPot"
	pot.position = Vector3(1.28, 0.42, 2.0)
	parent.add_child(pot)
	HEARTH_VISUAL.add_pot_bail(pot, 0.36, 0.48, vessel_iron)
	for index in 3:
		var angle := TAU * float(index) / 3.0
		var upper := Vector3(cos(angle) * 0.21, -0.18, sin(angle) * 0.21)
		var lower := Vector3(cos(angle) * 0.25, -0.395, sin(angle) * 0.25)
		HEARTH_VISUAL.add_curved_handle(pot, "ForgedPotFoot", PackedVector3Array([upper, lower]), 0.024, vessel_iron)
	var cup := HEARTH_VISUAL.create_vessel(0.105, 0.36, vessel_iron, false)
	cup.name = "SingleCup"
	cup.position = Vector3(1.58, 0.28, 2.72)
	parent.add_child(cup)
	HEARTH_VISUAL.add_cup_handle(cup, 0.105, 0.36, vessel_iron)
