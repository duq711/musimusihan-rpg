...
func _build_inventory_panel() -> void:
	...
	var clear_button := _action_button("선택 해제", Vector2(266, 518), Vector2(110, 29), Color(0.052, 0.054, 0.051))
	clear_button.name = "ClearSelectionButton"
	clear_button.pressed.connect(_on_clear_selection_pressed)
	inventory_root.add_child(clear_button)
...
func _item_context(source: String, index: int, equipment_slot: String) -> Dictionary:
	if source == "inventory" and inventory_model != null and index >= 0 and index < inventory_model.slots.size():
		...
		return {"source": source, "index": index, "id": str(stack.get("id", "")), "quantity": int(stack.get("quantity", 1))}
	...
	if source == "equipment" and inventory_model != null and inventory_model.equipment.has(equipment_slot):
		...
		return {"source": source, "index": index, "equipment_slot": equipment_slot, "id": item_id, "quantity": 1 if not item_id.is_empty() else 0}
	return {}
...
func _move_inventory_to_container(index: int, requested_quantity := 999999, notify := true) -> int:
	...
	var movable := mini(quantity, _container_capacity_for(item_id))
	if movable <= 0:
		return 0
	...
	if removed_quantity <= 0:
		return 0
	var remainder := loot_container.store_item(item_id, removed_quantity, false, true)
	if remainder > 0:
		...
		var restore_failure := inventory_model.add_item(item_id, remainder, false)
		if restore_failure > 0:
			loot_container.store_item(item_id, restore_failure, false, true)
...
func _move_container_to_inventory(index: int, requested_quantity := 999999, notify := true) -> int:
	...
	var quantity := mini(int(stack.get("quantity", 0)), requested_quantity)
	var movable := mini(quantity, _inventory_capacity_for(item_id))
	if movable <= 0:
		return 0
	...
	if removed_quantity <= 0:
		return 0
	var remainder := inventory_model.add_item(item_id, removed_quantity, false)
	if remainder > 0:
		var restore_failure := loot_container.store_item(item_id, remainder, false, source_identified)
		if restore_failure > 0:
			inventory_model.add_item(item_id, restore_failure, false)
	var moved := removed_quantity - remainder
	if notify and moved > 0:
		inventory_model.changed.emit()
		loot_container.changed.emit()
...
func _inventory_capacity_for(item_id: String) -> int:
	if inventory_model == null:
		return 0
	var definition := ExpeditionInventory.get_item_definition(item_id)
...
func _container_capacity_for(item_id: String) -> int:
	if loot_container == null:
		return 0
	var definition := ExpeditionInventory.get_item_definition(item_id)
	if definition.is_empty():
	...
	for stack in loot_container.items:
		if str(stack.get("id", "")) == item_id and bool(stack.get("identified", true)):
			capacity += maxi(0, stack_max - int(stack.get("quantity", 0)))
