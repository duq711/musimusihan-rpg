...
func _build_ghost(at: Vector3) -> void:
	...
	_ghost_material.albedo_color = Color(0.12, 0.135, 0.14, 0.0)
	_ghost_material.vertex_color_use_as_albedo = true
	_ghost_material.cull_mode = BaseMaterial3D.CULL_DISABLED
	var shroud := preload("res://scripts/dark_fantasy_effect_visual.gd").create_shroud()
	_add_ghost_mesh("RaggedHoodedShroud", shroud, Vector3.ZERO, Vector3.ONE)
