...
func _inspect_first_person_materials(player: DungeonPlayer) -> void:
	...
	var rigs: Array[Node3D] = [player.weapon_arm, player.shield_arm, player.torch_arm, player.chest_hands._arms[-1], player.chest_hands._arms[1]]
	for rig in rigs:
		_check(is_instance_valid(rig) and rig.get_script().resource_path == "res://scripts/player_arm_visual.gd", "all carried and chest arms must use the shared production arm rig")
...
func _contains_import(node: Node) -> bool:
	if node.scene_file_path == MODEL_PATH:
		return true
	for child in node.get_children():
		if _contains_import(child):
			return true
	return false
