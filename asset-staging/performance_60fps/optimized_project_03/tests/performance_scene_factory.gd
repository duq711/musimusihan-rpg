...
class Motion extends Node:
	...
	var starting_yaw := 0.0
	var maximum_yaw_change := 0.0
	var distance_travelled := 0.0
	var last_position := Vector3.ZERO
	var attack_requests := 0
	func _ready() -> void:
		process_physics_priority = -100
		starting_yaw = player.rotation.y
	...
	func _physics_process(delta: float) -> void:
		...
		# Camera local rotation belongs to production recoil and is reset every
		# frame. Actual mouse look rotates the player body, so steer that yaw.
		player.rotation.y = starting_yaw + sin(elapsed * 0.7) * 0.22
		maximum_yaw_change = maxf(maximum_yaw_change, absf(player.rotation.y - starting_yaw))
