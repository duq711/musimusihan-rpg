...
func _test_catalog_and_item_contract() -> void:
	_check(SpellCatalog.SPELL_ORDER == EXPECTED_SPELL_ORDER, "spell catalog must preserve the authored fire, water, ice, stone, and healing order")
...
func _test_merchant_and_dungeon_spawns() -> void:
	...
	_check(game.loot_chests.size() == 2, "the dungeon must retain two deterministic authored loot chests")
	if game.loot_chests.size() == 2:
		var entrance_ids := _container_item_ids(game.loot_chests[0].container)
		var reliquary_ids := _container_item_ids(game.loot_chests[1].container)
		for item_id in ["weathered_staff", "water_spellbook", "stone_spellbook"]:
			_check(entrance_ids.has(item_id), "the entrance supply chest must spawn %s" % item_id)
		for item_id in ["fire_spellbook", "ice_spellbook", "healing_spellbook"]:
			_check(reliquary_ids.has(item_id), "the deep reliquary chest must spawn %s" % item_id)
		var all_spawn_ids := entrance_ids.duplicate()
		all_spawn_ids.append_array(reliquary_ids)
		for item_id in MAGIC_ITEM_IDS:
			_check(all_spawn_ids.has(item_id), "%s must be obtainable from the authored dungeon chests" % item_id)
	game.queue_free()
	await process_frame
