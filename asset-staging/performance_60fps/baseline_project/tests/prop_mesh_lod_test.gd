...
func _run() -> void:
	...
	var original := skull.mesh as ArrayMesh
	if OS.get_environment("BAKE_PROP_LODS") == "1":
		_bake(original, skull)
...
func _bake(original: ArrayMesh, skull: MeshInstance3D) -> void:
	...
	baked.set_meta("source_bounds", SKULL.source_bounds())
	baked.set_meta("source_piece_count", skull.get_meta("anatomical_source_pieces"))
