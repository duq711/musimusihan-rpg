...
func _test_runtime_pause_and_loot() -> void:
	...
	var entrance: LootContainer = game.loot_chests[0].container
	var loot_ids: Array[String] = []
	for stack in entrance.items:
		loot_ids.append(str(stack.id))
	_check(loot_ids.has("hunting_bow") and loot_ids.has("wooden_arrow"), "dungeon entrance chest must provide bow and arrows")
