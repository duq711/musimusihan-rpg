...
func configure(title_text: String, item_stacks: Array, subtitle_text := "검은 성물실", capacity := 20) -> LootContainer:
	...
	for value in item_stacks:
		...
		store_item(str(stack.get("id", "")), int(stack.get("quantity", 1)), false, false)
	return self


func store_item(item_id: String, quantity := 1, notify := true, identified := true) -> int:
	...
	if stack_max > 1:
		for stack in items:
			...
			if str(stack.get("id", "")) != item_id or bool(stack.get("identified", true)) != identified:
				continue
	...
	while remaining > 0 and items.size() < max_slots:
		...
		items.append({"id": item_id, "quantity": moved, "identified": identified})
		remaining -= moved
