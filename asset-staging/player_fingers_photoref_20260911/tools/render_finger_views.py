"""Render twenty separate actual left-hand finger views without saving the blend.

Run with Blender --background --threads 2 --python this_file -- --source BLEND
--output-dir NEW_DIRECTORY. Defaults: five digits, dorsal/palmar/radial/ulnar,
768x1152, 16 CPU Cycles samples. For a quick check use --digits index --samples 4
...
VIEWS = ("dorsal", "palmar", "radial", "ulnar")
...
def digit_frame(holder, rig, skin, ids, digit, thumb_orientation=None):
    ...
    orientation_source = "Neutral distal rig dorsal axis"
    if digit == "thumb":
        assert thumb_orientation, "Supply the source thumb rotation report for baseline cameras"
        dorsal = Vector(thumb_orientation["new_dorsal_native"])
        dorsal = (dorsal - axis * dorsal.dot(axis)).normalized()
        orientation_source = "Authored thumb roll report, projected perpendicular to displayed finger axis"
    ...
    frames = {}
    # All views stay upright at exactly the same magnification. The left hand's
    # +across direction faces its radial (thumb) side for the four long digits;
    # this direction rotates with the authored thumb's intrinsic dorsal frame.
    for view, outward in (("dorsal", dorsal), ("palmar", -dorsal),
                          ("radial", across), ("ulnar", -across)):
        ...
        frames[digit + "_" + view] = {
            ...
            "display_orientation": "Finger tip upward; no image rotation or compositing",
        }
    return frames
...
def main():
    ...
    parser.add_argument("--thumb-orientation-report", type=Path,
                        help="Baseline only: defaults to thumb_rotation_report.json beside source blend")
    ...
    if args.camera_manifest:
        ...
        assert set(reference["frames"]) == {d + "_" + v for d in DIGITS for v in VIEWS}, "Reference must contain all twenty views"
        frames = reference["frames"]
    else:
        orientation_path = (args.thumb_orientation_report or source.parent / "thumb_rotation_report.json").resolve()
        thumb_orientation = json.loads(orientation_path.read_text())["hands"]["left"]
        for digit in DIGITS:
            frames.update(digit_frame(holder, rig, skin, selections[digit], digit, thumb_orientation))
    ...
    report = {
        ...
        "thumb_orientation_report": str(orientation_path) if orientation_path else None,
        "thumb_orientation_report_sha256": sha(orientation_path) if orientation_path else None,
        ...
        "renders": {}, "scope": "One real digit and one of four intrinsic faces per separate PNG, neutral source, static face extraction only; tip upright in every image",
    }
