...
for name,points in new.items():
 if 'Hand' in name:assert signature(points)==signature(old[name]),name
  ...
  def section(ri):
   h=.935+(1.443-.935)*ri/80
   q=[p for p in points if abs(p[1]-h)<1e-5];assert q
   bounds=[(min(p[k] for p in q),max(p[k] for p in q)) for k in (0,2)]
   return [sum(bounds[0])/2,h,sum(bounds[1])/2]
  directions=[]
  for indices in ((10,17,24),(48,56,64)):
   a,m,b=[section(i) for i in indices]
   t=(m[1]-a[1])/(b[1]-a[1])
   assert max(abs(m[k]-(a[k]*(1-t)+b[k]*t)) for k in range(3))<1e-5,(name,'wavy limb segment')
   d=[b[k]-a[k] for k in range(3)];length=math.sqrt(sum(x*x for x in d));directions.append([x/length for x in d])
  angle=math.degrees(math.acos(max(-1.,min(1.,sum(a*b for a,b in zip(*directions))))))
  assert 6<angle<22,(name,'elbow bend angle',angle)
  print(name,'distinct straight upper/forearm axes, elbow angle',round(angle,2),'degrees PASS')
...
print('PASS: both complete hands and lower sleeves restored, relaxed elbow axes verified, 24 body meshes and FP sources preserved.')
