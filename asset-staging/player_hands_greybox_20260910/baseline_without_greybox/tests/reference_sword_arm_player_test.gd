...
func _run() -> void:
	...
	_check(clips.size() == 8, "All eight analytical clip inputs must decode before cache substitution.")
	if clips.size() != 8:
		quit(1)
		return
...
func _test_combat_sampling(player: DungeonPlayer, fps: int) -> void:
	for variant: String in DATA.ATTACK_CLIPS:
		...
		for frame in range(fps * 2):
			...
			if phase in ["windup", "active", "recovery"] and not player._reference_pose_handoff_active:
		...
		_check(seen_active and player.combat_state == DungeonPlayer.CombatState.READY, "Actual attack must traverse ACTIVE and finish at every sampled rate.")
	# Test the exact hit key through the full composed player path.
	_neutralize(player)
	player.combat_state = DungeonPlayer.CombatState.ACTIVE
	player.sword_attack_variant = "overhead"
