...
func _test_actual_hand_contacts_and_interrupts() -> void:
	...
	_check(player.get_first_person_motion_snapshot().visible_arm_count == 2 and not player.shield_pivot.visible, "unequipped shield and stowed torch must not leave duplicate arms in the sword-only view")
	_check_relaxed_wrist_visible(player, player.left_support_arm)
	...
	_check_relaxed_wrist_visible(player, player.right_relaxed_arm)
