...
func _run() -> void:
	_check(REFERENCE.is_available(), "Actual Windows-authored motion must load; fallback curves are not a completed delivery: " + REFERENCE.get_load_error())
	...
	for failure in failures:
		push_error(failure)
	print("REFERENCE SWORD MOTION %s: Windows motion delivery and production playback checks, physical run/air handoffs, guard ownership and composed solo/paired clash recovery" % ("PASS" if failures.is_empty() else "FAIL"))
...
func _test_clips_and_combat_clocks() -> void:
	...
	for variant: String in REFERENCE.ATTACK_CLIPS:
		...
		for paired in [false, true]:
			...
			_check(MOTION.sword("active", hit, 0.0, false, paired, variant).is_equal_approx(contact), "Real sword clock must reach the Windows-authored contact pose: " + variant)
...
func _test_guard_lowering_ownership() -> void:
	...
	for frame in range(24):
		...
		expected.origin += movement.position * 0.65
