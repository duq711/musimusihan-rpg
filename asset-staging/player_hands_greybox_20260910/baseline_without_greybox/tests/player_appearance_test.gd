...
func _inspect_first_person_materials(player: DungeonPlayer) -> void:
	...
	for part in _meshes(player.player_body):
		source_meshes.append(part.mesh)
	# Check each production slot's intended rig, including stowed rigs. Accepting
	# whichever script happens to occupy a slot would hide equipment regressions.
	_check(player.weapon_arm == (player._sword_weapon_arm if player._equipped_weapon_type() == "melee" else player._legacy_weapon_arm), "equipped melee must select its anatomical sword hand; other weapons must select the original shared rig")
	_inspect_skinned_arm(player._sword_weapon_arm, 1)
	...
	for rig in rigs:
		...
		_check(correct_rig, "other weapons, torch, supporting hands and chest hands must retain the original full-body arm rig")
		if not correct_rig:
			continue
		var declared_sources: Array = rig.get_source_meshes()
		var rendered_parts := _meshes(rig)
		_check(not rendered_parts.is_empty() and declared_sources.size() == rendered_parts.size(), "each shared arm must render its source model parts")
		_check(rig.finger_joints.size() == 5 and not rig.hand_meshes.is_empty() and not rig.arm_meshes.is_empty(), "each shared arm must include the glove, sleeve and five articulated digits")
		for digit: Array in rig.finger_joints:
			_check(digit.size() == 2 and digit[0] is Node3D and digit[1] is Node3D and (digit[1] as Node3D).get_parent() == digit[0], "each original digit must retain both linked proximal and distal joints")
		for part in rendered_parts:
			_check(source_meshes.has(part.mesh) and declared_sources.has(part.mesh), "actual rendered arm geometry must be a mesh from the full-body GLB: " + str(part.name))
			_check(part.skin == null, "the preserved source-part rig must not acquire an unrelated skeletal skin")
			for surface_index in range(part.mesh.get_surface_count()):
				var source_material := part.mesh.surface_get_material(surface_index) as BaseMaterial3D
				var actual_material := part.get_active_material(surface_index) as BaseMaterial3D
				_check(actual_material == source_material and actual_material != null and actual_material.albedo_texture != null, "actual arm surfaces must use the full-body atlas and material resources: " + str(part.name))
				_check(actual_material != null and actual_material.shading_mode == BaseMaterial3D.SHADING_MODE_PER_PIXEL and not actual_material.emission_enabled, "original articulated arm materials must remain lit by the scene")
