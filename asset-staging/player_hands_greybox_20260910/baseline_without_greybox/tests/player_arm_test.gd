...
func _run() -> void:
	...
	if failures.is_empty():
		print("PLAYER ARM PASS: full-body GLB/atlas sharing, articulated grip and wrist-preserving arm fit, actual combat/bow/chest preview paths, trial hits and contacts, F2/repeat/reset and expedition restoration")
		quit(0)
	else:
...
func _test_shared_geometry_and_joints() -> void:
	...
	for side in [-1, 1]:
		...
		_check(parts.size() == source_refs.size() and parts.size() >= 13, "each actual arm must include the source sleeve, bracer, glove and ten finger segments")
		for part in parts:
			_check(original_meshes.has(part.mesh) and source_refs.has(part.mesh), "rendered arm parts must share actual full-body mesh resources")
			for surface in range(part.mesh.get_surface_count()):
				var material := part.get_active_material(surface) as BaseMaterial3D
				_check(material == part.mesh.surface_get_material(surface) and material != null and material.albedo_texture != null, "arm surfaces must use the original lit atlas material")
		_check(rig.find_children("*", "CollisionObject3D", true, false).is_empty(), "articulated visuals must not add combat or interaction collisions")
		rig.reset_pose()
		var glove: MeshInstance3D = rig.hand_meshes[0]
		var glove_bounds: AABB = glove.transform * glove.mesh.get_aabb()
		var knuckle_average := 0.0
		for digit in 4:
			knuckle_average += (rig.finger_joints[digit][0] as Node3D).position.z / 4.0
		_check(glove_bounds.position.z >= knuckle_average - 0.014, "fingerless glove must stop at the articulated knuckles instead of covering hooked fingers")
		_check(glove_bounds.position.z <= knuckle_average + 0.014, "glove must still join the finger roots without exposing an empty palm gap")
		for digit in 5:
			var joint: Node3D = rig.finger_joints[digit][0]
			_check(glove_bounds.has_point(joint.position), "actual articulated skin roots must enter the shared glove instead of floating above its palm")
		var open_parts := _part_transforms(rig.hand_meshes)
		var open_joints := _joint_transforms(rig)
		rig.set_grip(1.0)
		_check(_part_transforms(rig.hand_meshes) != open_parts and _joint_transforms(rig) != open_joints, "gripping must bend rendered source fingers through actual joints")
		...
		var held_parts := _part_transforms(rig.hand_meshes)
		...
		_check(_part_transforms(rig.arm_meshes) != arm_before, "fitting an arm must move its actual sleeve and bracer meshes")
		_check(_part_transforms(rig.hand_meshes) == held_parts and rig.global_transform.is_equal_approx(wrist_before) and stage.global_transform.is_equal_approx(parent_before), "shoulder and elbow fitting must never move the hand or carried weapon")
		_check(rig.get_source_meshes() == source_refs, "posing must retain original source resources")
		rig.set_arm_visible(false)
		for part: MeshInstance3D in rig.arm_meshes:
			_check(not part.visible, "arm stowing must hide the sleeve and bracer")
		for part: MeshInstance3D in rig.hand_meshes:
			_check(part.visible, "arm-only hiding must preserve hand visibility for close gestures")
		rig.set_arm_visible(true)
		rig.reset_pose()
		_check(_joint_transforms(rig) == open_joints and rig.global_transform.is_equal_approx(wrist_before), "reset must restore open finger joints without moving the wrist")
		rig.set_finger_curl(0, -0.35, -0.60)
		_check(_joint_transforms(rig) != open_joints and _part_transforms(rig.hand_meshes) != open_parts, "an individual finger curl must affect the actual source geometry")
		rig.set_string_draw(1.0, 0.0)
		var hooked := _part_transforms(rig.hand_meshes)
		rig.set_string_draw(1.0, 1.0)
		_check(_part_transforms(rig.hand_meshes) != hooked and rig.global_transform.is_equal_approx(wrist_before), "releasing the bowstring must open actual fingers while preserving the held nock's wrist frame")
		rig.set_relaxed_pose(0.0)
		var relaxed := _part_transforms(rig.hand_meshes)
		rig.set_relaxed_pose(1.0)
		_check(_part_transforms(rig.hand_meshes) != relaxed and rig.get_source_meshes() == source_refs, "relaxed and assisting open palms must animate the same player mesh resources")
	stage.queue_free()
	await process_frame
	_check(ExpeditionSession.capture_snapshot() == original and Input.mouse_mode == mouse_before, "arm construction and poses must not alter expedition or cursor state")
