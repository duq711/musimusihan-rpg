...
func _exercise_shield_impact(room: Node3D) -> void:
	...
	_check(is_equal_approx(health - player.health, ceilf(attacker.attack_damage * 0.18)) and is_equal_approx(stamina - player.stamina, attacker.attack_damage * 1.18), "actual sustained guard must apply shield chip damage and one real stamina cost")
