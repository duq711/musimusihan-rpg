...
static func inspect_detail_rendering(geometry: Node) -> Dictionary:
	...
	for submission: Dictionary in details.call("get_render_batches"):
		var visual := details.get_node_or_null(NodePath(str(submission.name))) as Node3D
		if visual == null:
			mismatches += 1
			continue
		var audit := preload("res://tests/cave_detail_submission_checks.gd").inspect(submission, visual, cache, true)
		count += int(audit.instances)
		mismatches += int(audit.mismatches)
