...
func _test_catalog_and_ingredient_availability() -> void:
	...
	_check(ExpeditionSession.capture_snapshot().size() == 10, "immediately eaten camp meals must not add persistent cooking fields")
...
func _test_all_actual_recipes(capped: bool) -> void:
	for id: String in EXPECTED:
		...
		_check(is_equal_approx(player.health, minf(100.0, float(before.health) + float(recipe.health))) and is_equal_approx(player.stamina, minf(100.0, float(before.stamina) + float(recipe.stamina))), "eating must apply exact health/stamina recovery with caps: " + id)
...
func _test_real_scene_controls_and_supplies() -> void:
	...
	for enemy in get_nodes_in_group("enemy"):
		enemy.set_physics_process(false)
	var entrance: DungeonLootChest = game.loot_chests[0]
