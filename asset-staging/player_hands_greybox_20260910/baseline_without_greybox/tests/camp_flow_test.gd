...
func _run() -> void:
	...
	_key(KEY_C)
	_check(game.camp.is_open() and game.game_mode == game.GameMode.CAMPING and paused, "C must open the real camp plan and pause the dungeon")
	...
	_check(bag.slots == slots_before, "opening the plan must not spend any supplies")
	...
	_check(bag.count_item("camp_kit") == 0 and game.camp.warmth == 2, "first rest must spend one kit and one warmth exactly once")
	...
	_check(bool(game.open_camp().accepted), "a second supplied camp must reopen")
	...
	game._resume_game()
	bag.add_item("camp_kit", 1)
	_check(bool(game.open_camp().accepted), "post-pause camp must reopen normally")
	...
	bag.add_item("camp_kit", 1)
	_check(bool(game.open_camp().accepted), "post-hit camp must reopen when safe")
	game.camp.start_action("rest")
	...
	_check(not bool(game.open_camp().accepted) and bag.slots == unsafe_slots, "nearby live enemies must reject placement without spending")
	enemy.position = enemy_position
	_check(bool(game.open_camp().accepted), "safe camp must reopen after threat moves away")
	game.camp.start_action("rest")
	...
	if failures.is_empty():
		print("CAMP FLOW TEST PASS: real C and action UI, paused plan/live rest, player locks, atomic costs, one survival clock, rewards, inventory/pause/focus/damage/approach cleanup")
		quit(0)
	else:
