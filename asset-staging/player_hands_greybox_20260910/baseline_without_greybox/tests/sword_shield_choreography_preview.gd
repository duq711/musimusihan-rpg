...
static func action_summary(fixture: Dictionary) -> Dictionary:
	...
	if ATTACKS.has(id):
		...
		valid = valid and bool(fixture.action_result.get("blocked", false)) and not bool(fixture.action_result.get("parried", false)) and float(fixture.max_impact) > 0.0 and is_equal_approx(health_lost, ceilf(18 * 0.18)) and is_equal_approx(stamina_spent, 18 * 1.18)
	else:
