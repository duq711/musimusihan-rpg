...
frames = root / 'godot-game/artifacts/visual_qa/bandage_forearm/wrap_only_04'
manifest = json.loads((frames/'manifest.json').read_text())
assert len(manifest['frames']) == 140 and manifest['state_and_sources_preserved']
video = frames/'bandage_wrap_only.mp4'
...
with av.open(str(video), 'w', options={'movflags': '+faststart'}) as container:
    ...
    for frame in manifest['frames']:
        ...
        draw.text((24,20), '붕대 감기 · 펼치기 제거', font=font, fill='#f1eee7')
...
assert decoded == 140
print(f'{video}\n140 decoded frames / 30fps / {video.stat().st_size} bytes')
