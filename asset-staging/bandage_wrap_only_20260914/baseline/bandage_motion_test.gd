...
func _run() -> void:
	...
	for i in 371:
		...
		if i == 44:
			_check(player.bandage_hands.strip.visible and not player.bandage_hands.cloth.visible and is_zero_approx(player.bandage_hands.winding), "prepare and unfold the bandage before applying it to the forearm")
		...
		if i > 122 and i < 303:
			...
			if i == 123: first_working_basis = player.bandage_hands.left_arm.basis
