...
static func inspect_bound_materials(fixture: Dictionary) -> Dictionary:
	...
	return {"passed": passed and materials.size() >= 5 and surfaces >= 9, "material_count": materials.size(), "rendered_surface_count": surfaces, "minimum_bound_texture_size": [minimum_size.x, minimum_size.y], "binding_source": "Live MeshInstance3D active materials; source hashes cover GLBs, extracted PNGs and texture import settings"}
