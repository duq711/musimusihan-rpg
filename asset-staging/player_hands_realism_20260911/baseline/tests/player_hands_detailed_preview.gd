...
func _run() -> void:
	...
	for pose_id: String in POSE_IDS:
		...
		await _capture(viewport, output_path, pose_id, inspection)
		if pose_id == "free_hands":
			...
			if not player.set_hands_visual_profile("greybox"):
				...
				await _capture(viewport, output_path, "free_hands_greybox", {"pose": "free_hands", "profile": "greybox", "same_pose_and_camera": exact_pose, "greybox": player.get_hands_greybox_snapshot(), "passed": exact_pose and player.hands_greybox_enabled})
...
func _capture(viewport: SubViewport, output_path: String, pose_id: String, inspection: Dictionary) -> void:
