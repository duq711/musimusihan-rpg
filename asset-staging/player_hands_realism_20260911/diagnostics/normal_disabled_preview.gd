...
func _run() -> void:
	...
	var image_name := "open_normal_disabled.png"
	...
	var report := {
		...
		"single_change": "normal_enabled=false on in-memory material copies only",
		"normal_disabled_surface_count": surface_count, "material_names": changed_names,
	...
	print("REALISM NORMAL DIAGNOSTIC %s: %s" % ["PASS" if failures.is_empty() else "FAIL", output])
