...
func _build_equipped_torch(iron: StandardMaterial3D) -> void:
	...
	# Local shaft-width comparison; cage, flame and global torch transform stay fixed.
	for part in torch_model.find_children("*", "MeshInstance3D", true, false):
		if str(part.name).begins_with("CharredHandle") or str(part.name).begins_with("LeatherWrap"):
			part.scale.x *= TORCH_GRIP.SHAFT_RADIUS_RATIO
			part.scale.z *= TORCH_GRIP.SHAFT_RADIUS_RATIO
...
func _update_viewmodel(delta: float) -> void:
	...
	if is_instance_valid(torch_pivot):
		...
		var torch_target := MOTION.pose(TORCH_GRIP.CARRY_POSITION, Vector3(-25, 8, 13)) if held else MOTION.pose(Vector3(-0.78, -1.55, -0.38), Vector3(-8, 10, -4))
