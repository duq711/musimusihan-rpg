...
var output := "res://artifacts/visual_qa/bandage_forearm"
...
func _run() -> void:
	...
	var count := 10 if sparse else 160
	var step := 0.48 if sparse else 1.0 / 30.0
	for i in count:
		...
		if picture.save_png(path.path_join(filename)) != OK:
			...
			quit(1)
			return
		frames.append({"file": filename, "time": i * step, "active": player.is_bandage_motion_active(), "winding": player.bandage_hands.winding})
