		...
		for frame in range(fps * 2):
			...
			if phase == "windup":
				...
				if wrist_turn:
					...
					_check(_near_pose(player.weapon_pivot.transform, entry_pose), "Reverse and overhead WINDUP must retain the held sword at %dHz/%s." % [fps, variant])
...
func _test_zero_time_handoffs(player: DungeonPlayer) -> void:
	for source: String in ["run", "air", "land", "attack"]:
		...
		player.cancel_sword_attack()
		_check(bool(player.begin_sword_attack("right_diagonal").get("accepted", false)), "Basic wrist-turn entry must accept an actual prior pose: " + source)
		player._update_viewmodel(0.0)
		_check(_near_pose(sword, player.weapon_pivot.transform), "Basic wrist turn at zero time must retain the previous actual sword: " + source)
		_check_arm_equal(_actual_arm(player), before, "basic wrist turn t=0 from " + source)
		_check_geometry(player, "basic wrist turn zero-time handoff from " + source)
		player.cancel_sword_attack()
