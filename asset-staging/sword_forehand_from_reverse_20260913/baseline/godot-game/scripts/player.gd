...
func _update_viewmodel(delta: float) -> void:
	...
	if reference_enabled:
		...
		if _uses_direct_sword_entry(phase):
			# Lay the forehand blade around the current grip before cutting.
			# The grip never rises during charge; the other cuts retain their entry.
			var entry_pose := _sword_entry_pose
			var entry_arm := _sword_entry_arm
			if sword_attack_variant == "right_diagonal":
		...
		if not (_uses_direct_sword_entry(phase) and phase == "windup" and sword_attack_variant == "right_diagonal"):
