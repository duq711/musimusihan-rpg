...
func _exercise_attack(player: DungeonPlayer, variant: String, hold: float, stowed: bool) -> void:
	...
	print("DIRECT ENTRY ", context, " held_pose_mm=", windup_drift * 1000.0, " windup_deg=", windup_angle, " entry_mm=", entry_drift * 1000.0, " arm_length_mm=", arm_length_error * 1000.0, " authored_mm=", source_position_error * 1000.0, " authored_deg=", source_angle_error, " charge=", expected_charge, " spends=", spent_count)
...
func _finish() -> void:
	...
	print("SWORD DIRECT ENTRY %s: all cuts retain the held blade and arm through WINDUP, release/charge/cost clocks, 100ms authored rejoin and expedition preservation; target damage is covered by sword_attack_variants" % ("PASS" if failures.is_empty() else "FAIL"))
