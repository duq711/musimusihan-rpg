...
func set_time(seconds: float) -> void:
	...
	# Viewmodel shoulder anchors stay outside the frustum, at the same
	# depth as the upper sleeves instead of far behind the raised hands.
	left_arm.fit_arm(to_global(Vector3(-0.34, -0.26, -0.18)), to_global(_elbow))
	...
	# Aim the winding forearm back toward the body and down to the elbow.
	# Mirroring the receiving arm's axis raised the elbow above the wrist
	# and stretched the upper sleeve across the foreground.
	var palm_z := (-_axis + Vector3(0.0, -0.10, 0.08)).normalized()
	var palm_y := (_normal - palm_z * _normal.dot(palm_z)).normalized()
	...
	# The wrapper origin is a grip anchor, not the imported anatomical wrist.
	# Fit the sleeve from its real wrist pivot so the cuff and hand meet.
	...
	var right_elbow := to_local(wrist_world) + palm_z * 0.27
	right_arm.fit_arm(to_global(Vector3(0.34, -0.26, -0.18)), to_global(right_elbow))
