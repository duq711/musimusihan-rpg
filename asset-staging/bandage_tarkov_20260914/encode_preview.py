...
frames = root / 'godot-game/artifacts/visual_qa/bandage_forearm/tarkov_03'
manifest = json.loads((frames/'manifest.json').read_text())
assert len(manifest['frames']) == 202 and manifest['state_and_sources_preserved']
video = frames/'bandage_tarkov.mp4'
...
with av.open(str(video), 'w', options={'movflags': '+faststart'}) as container:
    ...
    for frame in manifest['frames']:
        ...
        draw.text((24,20), '붕대 감기 · 새 참고 동작 반영', font=font, fill='#f1eee7')
...
assert decoded == 202
print(f'{video}\n202 decoded frames / 30fps / {video.stat().st_size} bytes')
