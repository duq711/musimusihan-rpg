...
const SWORD_FOREHAND_GRIP := preload("res://scripts/sword_forehand_grip.gd")
...
var _sword_entry_contact_pose := Transform3D.IDENTITY
...
func request_jump() -> Dictionary:
	...
	var reason := ""
	if camping or combat_state == CombatState.DEAD:
		reason = "unavailable"
...
func advance_movement(delta: float, movement_input: Vector2, sprint_requested: bool = false) -> void:
	...
	if current_trap != null or is_timed_interacting():
	...
	var sprinting := not is_timed_interacting() and not bow_drawing and not _is_flail_busy() and sprint_requested and movement.y < -0.15 and stamina > 0.0
...
func begin_sword_attack(variant: String = "") -> Dictionary:
	...
	if _sword_direct_entry:
		...
		_sword_entry_shield_pose = shield_pivot.transform if is_instance_valid(shield_pivot) else Transform3D.IDENTITY
...
func _update_viewmodel(delta: float) -> void:
	...
	if reference_enabled and sword_attack_variant == "right_diagonal" and phase == "active" and _uses_direct_sword_entry(phase):
		# Use the contact as one entry destination, with the same presentation
		# offsets as the sweep it will join. No outward preparation waypoint.
		_sword_entry_contact_pose = REFERENCE_MOTION.retimed_attack("active", get_melee_hit_time(), attack_charge, sword_attack_variant, _sword_attack_coordinated)
		_sword_entry_contact_pose.origin += movement.position + Vector3(_motion_look_sway.x, _motion_look_sway.y, 0) + Vector3(0.035, -0.27, 0.12) * raising
		_sword_entry_contact_pose.basis = _sword_entry_contact_pose.basis * Basis.from_euler(movement.rotation) * Basis.from_euler(Vector3(0.32, 0, -0.12) * raising)
	...
	if is_instance_valid(shield_pivot):
		...
		if reference_enabled:
			if sword_attack_variant == "right_diagonal" and _uses_direct_sword_entry(phase):
				# A quick release skips WINDUP's authored shield motion too.
				# Join from the actually held shield instead of jumping to its end.
				var shield_entry := 0.0 if phase == "windup" else _direct_sword_entry_progress()
				shield_target = _sword_entry_shield_pose.interpolate_with(shield_target, shield_entry)
	...
	if reference_enabled:
		...
		target = _apply_forehand_grip_direction(target, phase)
...
func _advance_reference_pose_handoff(enabled: bool, phase: String, guard_owned: bool, delta: float) -> void:
	...
	if _reference_pose_handoff_active:
		...
		# This ends before the minimum 0.22s windup. The active blade path and
		# gameplay hit time are therefore never delayed by a presentation blend.
...
func _apply_forehand_grip_direction(pivot: Transform3D, phase: String) -> Transform3D:
	# Clash/guard handoffs already contain the actual corrected held pose.
	if sword_attack_variant != "right_diagonal" or blocking or _sword_clash_recovering:
		return pivot
	if phase == "active" and is_zero_approx(state_time) and _uses_direct_sword_entry(phase):
		return pivot
	var recovery := lerpf(0.47, 0.68, clampf(attack_charge, 0.0, 1.0))
	var turn := SWORD_FOREHAND_GRIP.angle(phase, state_time, get_melee_hit_time(), recovery)
	if turn == 0.0:
		return pivot
	var corrected := SWORD_FOREHAND_GRIP.apply(pivot, turn)
	if phase == "active" and _uses_direct_sword_entry(phase):
		# The old moving source rotates almost half a turn before contact.
		# Aim at the already-corrected contact once, without adding that source
		# rotation and a second animated half-turn to the visible entry.
		corrected = SWORD_FOREHAND_GRIP.enter(_sword_entry_pose, _sword_entry_contact_pose, _direct_sword_entry_progress())
	if not _reference_arm_target.is_empty():
		# Keep the existing shoulder and elbow hint. Only the held wrist moves;
		# the normal fixed-length solver fits the original glove and sleeve.
		_reference_arm_target["raw_sword"] = corrected
		_reference_arm_target["wrist"] = corrected * SWORD_LONG_GRIP.REST_WRIST
		_reference_arm_target["exact_sample"] = false
		_reference_arm_target.erase("fitted_pose")
	return corrected
...
func _sword_direct_entry_duration() -> float:
	# The reused reverse follow-through starts beyond the right screen edge.
	# Join the forehand at contact so entry stays connected to the held sword.
	return get_melee_hit_time() if sword_attack_variant == "right_diagonal" else SWORD_DIRECT_ENTRY_SECONDS
...
func begin_finger_joint_review() -> bool:
	...
	for arm: Node3D in [left_support_arm, right_relaxed_arm]:
		if not bool(arm.call("get_joint_snapshot").get("corrective_available", false)):
