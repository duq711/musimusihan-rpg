...
- 2026-09-13 최신 사용자 지시: 정베기의 멈칫하는 대기와 추가 뒤집기를 없앤다. 검·방패와 양손 상태의 짧은 LMB release는 다음 전투 틱에 바로 ACTIVE로 전환하며 기존 최소 대기 0.22초를 제거한다. 누르기 유지는 WINDUP과 기존 0.24초 차지 시작·1.24초 자동 공격을 따른다. 역베기·내려베기·비연결 단독 검의 최소 준비 0.22초는 유지한다.
- 현재 구현: 시작 자세에서 방향을 맞춘 타격 자세까지 한 번의 최단 보간으로 검 방향을 연결하며 별도의 180도 뒤집기를 겹치지 않는다. 진입의 바깥 후퇴를 없애고 현재 파지점에서 0.145초의 타격 파지점까지 직접 연결하며, 타격 이후 기존 궤적·회수를 유지한다. 방패도 공격 시작 때의 실제 자세에서 연결한다. 타격 0.145초·활성 0.30초, 피해·기력·방패 규칙과 원본 manifest·GLB·역베기는 보존한다. 기존 `주무장 1번 · 방패 수납` 항목에서 정베기 즉시 베기·파지를 비교한다. 관련 자동 검사 8종과 실제 Vulkan/embedded 양손·방패 촬영을 완료했다. 실제 진입의 직접 파지 경로·한 번의 최단 회전·손 연결·대기 복귀를 확인했고 기존 역베기는 총 182 PNG가 변경 전과 같았다. 기존 자연 검격 교차 시험은 4개 assertion에서 계속 실패하며, 회수의 화면 밖 구간도 남아 있다. 사용자 미학적 승인을 뜻하지 않으며 변경 전 자료·검사·한계는 [검증 기록](../../asset-staging/sword_forehand_immediate_cut_20260913/validation_summary.json)에 보존한다.
