...
- 2026-09-13 사용자 후속 지시: 양손검 공격 전에 한 번 치켜드는 준비 동작을 없애고 현재 파지에서 바로 베기를 시작한다. 방패 착용/수납은 같은 공격 연결과 속도를 사용하며 기존 차지·피해·기력 규칙을 유지한다. 이는 반려된 v4를 다시 적용하는 것이 아니며, 앞선 손 조형 문제 해결과 구분한다.
