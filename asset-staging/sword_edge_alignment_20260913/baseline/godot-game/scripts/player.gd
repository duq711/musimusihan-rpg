...
func _update_viewmodel(delta: float) -> void:
	...
	if reference_enabled:
		...
		if _uses_direct_sword_entry(phase):
			# Both endpoints already include carry/sway/equip offsets. Connect
			# the actual held pose to the moving cut, including its matching arm.
			var entry_progress := 0.0 if phase == "windup" else _direct_sword_entry_progress()
			target = _sword_entry_pose.interpolate_with(target, entry_progress)
			_reference_arm_target = _mix_reference_arms(_sword_entry_arm, _reference_arm_target, entry_progress)
