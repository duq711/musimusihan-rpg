...
func _test_combat_sampling(player: DungeonPlayer, fps: int) -> void:
	for variant: String in DATA.ATTACK_CLIPS:
		...
		_check(player._uses_direct_sword_entry("windup") and _near_pose(player._sword_entry_pose, entry_pose), "Actual start must capture the held sword instead of selecting the raised preparation key.")
		...
		for frame in range(fps * 2):
			...
			if phase == "windup":
				_check(player._uses_direct_sword_entry(phase) and _near_pose(player.weapon_pivot.transform, entry_pose), "WINDUP must retain the held sword without raising it at %dHz/%s." % [fps, variant])
				...
				_check(player._uses_direct_sword_entry(phase), "Only the initial 100ms of ACTIVE may blend from the held pose into the authored swing.")
				if is_zero_approx(player.state_time):
					seen_active_start = true
					_check(_near_pose(player.weapon_pivot.transform, entry_pose), "ACTIVE t=0 must continue from the held sword without a jump: %dHz/%s" % [fps, variant])
	...
	# Test the exact hit key through the full composed player path.
	_neutralize(player)
	_check(bool(player.begin_sword_attack("overhead").get("accepted", false)), "Exact contact sampling must initialize the real attack-entry and coordinated timing state.")
	player.combat_state = DungeonPlayer.CombatState.ACTIVE
	player.state_time = 0.10
	player._update_viewmodel(0.0)
	var boundary_metadata := DATA.clip_metadata("overhead")
	var boundary_time := DATA.authored_attack_time(boundary_metadata.timing, "active", player.state_time, player.attack_charge, true)
	var boundary_sample := DATA.sample_right_arm("overhead", boundary_time)
	_check(not player._uses_direct_sword_entry("active") and _near_pose(player.weapon_pivot.transform, boundary_sample.raw_sword), "Exactly 100ms must restore the original blade pose before the clash/contact window.")
	_check_geometry(player, "exact 100ms authored entry boundary")
	player.state_time = player.get_melee_hit_time()
	player._update_viewmodel(0.0)
	var expected := DATA.sample_right_arm("overhead", 0.5)
	_check(is_equal_approx(player.state_time, 0.145) and not player._uses_direct_sword_entry("active"), "Exactly 145ms must reach the authored contact without the entry override.")
	_check(bool(player._joint_landmarks.sword.exact_authored_sample), "Shared real hit timestamp must preserve the exact authored key.")
	_check_arm_equal(_actual_arm(player), expected, "exact retimed contact")
