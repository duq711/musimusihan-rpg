...
var _torch_was_held := false
var _torch_draw_elapsed := 0.0
var _torch_draw_from := Transform3D.IDENTITY
...
func advance_movement(delta: float, movement_input: Vector2, sprint_requested: bool = false) -> void:
	...
	if _movement_grounded and planar_speed > WALK_SPEED:
		_movement_run_time += delta * clampf(planar_speed / SPRINT_SPEED, 0.0, 1.5)
...
func get_next_sword_attack_variant() -> String:
	# The established sword-only attack remains unchanged. The new cycle is
	# the coordinated sword-and-shield stance, including its supporting arm.
	if not _has_shield_equipped():
...
func begin_sword_attack(variant: String = "") -> Dictionary:
	...
	if not _has_shield_equipped() and selected != SWORD_ATTACK_VARIANTS[0]:
		return _sword_attack_failure("shield_required")
	sword_attack_variant = selected
	_sword_attack_had_shield = _has_shield_equipped()
	_sword_attack_uses_cycle = variant.is_empty() and sword_attack_mode == "cycle" and _sword_attack_had_shield
	attack_charge = 0.0
...
func get_melee_hit_time() -> float:
	return CHOREOGRAPHY.HIT_SECONDS if _has_shield_equipped() else ATTACK_HIT_TIME


func get_melee_active_duration() -> float:
	return CHOREOGRAPHY.ACTIVE_SECONDS if _has_shield_equipped() else 0.16
...
func _update_viewmodel(delta: float) -> void:
	...
	var target := MOTION.sword(phase, state_time, attack_charge, blocking, _has_shield_equipped(), sword_attack_variant)
	if paired_sword:
	...
	var reference_locomotion := reference_enabled and phase == "ready" and not reference_guard_owned and _has_reference_locomotion()
	if reference_locomotion:
		...
		var authored_amount := 1.0 if _movement_phase != "grounded" else clampf((_motion_speed - WALK_SPEED) / (SPRINT_SPEED - WALK_SPEED), 0.0, 1.0)
		movement.position *= 1.0 - authored_amount
		movement.rotation *= 1.0 - authored_amount
	else:
		_reference_locomotion_valid = false
	_reference_arm_target = _reference_right_arm_target(reference_enabled, reference_locomotion, phase, paired_sword, target)
	...
	if is_instance_valid(torch_pivot):
		...
		var torch_target := MOTION.pose(TORCH_GRIP.STOW_POSITION, TORCH_GRIP.STOW_ROTATION)
		if held:
			if not _torch_was_held:
				_torch_draw_elapsed = 0.0
				_torch_draw_from = torch_pivot.transform
			_torch_draw_elapsed = minf(TORCH_GRIP.DRAW_DURATION, _torch_draw_elapsed + maxf(delta, 0.0))
			var reveal := MOTION.pose(TORCH_GRIP.DRAW_POSITION, TORCH_GRIP.DRAW_ROTATION)
			...
			if _torch_draw_elapsed < TORCH_GRIP.DRAW_REVEAL_TIME:
				torch_target = MOTION.blend(_torch_draw_from, reveal, _torch_draw_elapsed / TORCH_GRIP.DRAW_REVEAL_TIME)
			else:
				torch_target = MOTION.blend(reveal, carry, (_torch_draw_elapsed - TORCH_GRIP.DRAW_REVEAL_TIME) / (TORCH_GRIP.DRAW_DURATION - TORCH_GRIP.DRAW_REVEAL_TIME))
			torch_target.origin += movement.position * 0.35
...
func _update_character_arms() -> void:
	...
	for arm: Node3D in [weapon_arm, shield_arm, torch_arm, left_support_arm, right_relaxed_arm]:
		...
		if arm == _sword_weapon_arm or (arm == shield_arm and role == "shield"):
