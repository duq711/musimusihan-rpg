...
func run():
	...
	output=ProjectSettings.globalize_path("res://artifacts/visual_qa/wooden_torch_shoulder_carry")
	...
	var elapsed := 0.0
	for sample in [0.18, 0.38, 0.65, 1.05]:
	...
	p.set_torch_enabled(false);p._update_viewmodel(2.0);p.viewmodel_renderer.sync_view();await capture(vp,"extinguished")
