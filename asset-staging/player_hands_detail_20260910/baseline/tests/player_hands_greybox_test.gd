...
func _test_player_runtime_and_preview() -> void:
	...
	for pose_id: String in PREVIEW.POSE_IDS:
		...
		if pose_id == "free_hands":
			...
			for side in [-1, 1]:
				_check(player.chest_hands._arms[side].get("greybox_visual").is_visible_in_tree(), "actual timed chest interaction must display both greybox hands")
...
func _test_room_lifecycle() -> void:
	...
	for iteration in 2:
		...
		for side in [-1, 1]:
			...
			_check(hand.global_position.distance_to(contact.origin) < 0.015 and player.chest_hands._arms[side].get("greybox_visual").is_visible_in_tree(), "both greybox hands must reach the real chest contact transforms")
