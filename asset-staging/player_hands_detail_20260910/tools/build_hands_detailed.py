"""Background-only detailed neutral-clay hand builder.

Mac execution explicitly authorized by the user on 2026-09-10 for this task.
Copies source objects, reduces topology before the armature, and writes new
outputs only. Original source files and existing Blender sessions are preserved.
"""
...
def main():
    ...
    parser.add_argument("--target-triangles", type=int, default=56000)
    ...
    preview.name = "Bilateral_Detailed_Review"
    ...
    palette = [gray("Detailed_Skin", .40), gray("Detailed_Glove", .155),
               gray("Detailed_Sleeve", .115), gray("Detailed_Nail", .46), gray("Detailed_Trim", .075)]
    palette[3].node_tree.nodes.get("Principled BSDF").inputs["Roughness"].default_value = .36
    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from hand_detail_geometry import refine_hand_set
    ...
    def paint_single_hand_surface(source_obj, clone, source_rig):
        """Represent the fingerless glove on the skin itself, with no second shell."""
        clone.data.materials.clear()
        clone.data.materials.append(palette[0])
        clone.data.materials.append(palette[1])
        group_names = {group.index: group.name for group in clone.vertex_groups}
        finger_axes = {}
        for finger in ("thumb", "index", "middle", "ring", "little"):
            # thumb0 is a metacarpal inside the palm, whereas the other bone0
            # landmarks begin at the knuckles. Use the actual thumb phalanx.
            start, end = ("1", "2") if finger == "thumb" else ("0", "1")
            base = source_rig.matrix_world @ source_rig.data.bones[finger + start].head_local
            first_joint = source_rig.matrix_world @ source_rig.data.bones[finger + end].head_local
            finger_axes[finger] = (base, first_joint - base)
        counts = {"glove_faces": 0, "exposed_finger_faces": 0}
        for polygon in clone.data.polygons:
            influence = {}
            center = Vector((0, 0, 0))
            for index in polygon.vertices:
                vertex = clone.data.vertices[index]
                center += source_obj.matrix_world @ vertex.co
                for group in vertex.groups:
                    name = group_names.get(group.group, "")
                    influence[name] = influence.get(name, 0) + group.weight
            center /= len(polygon.vertices)
            dominant = max(influence, key=influence.get, default="wrist")
            finger = next((name for name in finger_axes if dominant.startswith(name)), None)
            exposed = False
            if finger:
                base, axis = finger_axes[finger]
                exposed = (center - base).dot(axis) / axis.length_squared >= .50
            polygon.material_index = 0 if exposed else 1
            counts["exposed_finger_faces" if exposed else "glove_faces"] += 1
        if not all(counts.values()):
            raise RuntimeError("Single hand surface is missing glove or exposed-finger material region")
        return {"method": "one anatomical surface; wrist/palm and proximal half first phalanx use cuff grey",
                "finger_cut_fraction": .50, "thumb_landmarks": "thumb1 to thumb2; thumb0 is metacarpal", **counts}
    ...
    def export_glb(path, scene, objects):
        ...
        options = {"filepath": str(path), "export_format": "GLB", "use_selection": True,
                   "export_yup": True, "use_active_scene": True, "export_materials": "EXPORT", "export_skins": True,
    ...
    report = {"status": "building", "created_utc": datetime.now(timezone.utc).isoformat(),
              ...
              "scope": "Detailed neutral-clay bilateral hands with physical skin sculpt, nails and tailored trim",
              "preservation": "No mirroring, transform application or armature bake; source rigs retained",
              "hand_surface": "Single anatomical mesh; separate overlapping fingerless glove omitted from outputs only",
    ...
    for side in ("left", "right"):
        ...
        excluded = [o for o in originals if o.type == "MESH" and
                    "fingerlessleatherglove" in (o.name + " " + o.data.name).lower()]
        if len(excluded) != 1:
            raise RuntimeError(f"{side}: expected one separate fingerless glove shell")
        original_triangles = sum(mesh_signature(o)["triangles"] for o in originals if o.type == "MESH")
        before_triangles = sum(mesh_signature(o)["triangles"] for o in originals if o.type == "MESH" and o not in excluded)
        ...
        for obj in originals:
            ...
            clone.name = obj.name  # Preserve adapter prefixes; Blender may add a numeric suffix.
        ...
        for obj, clone in copies.items():
            ...
            if obj.type == "MESH":
                ...
                modifier = clone.modifiers.new("Detailed_Collapse", "DECIMATE")
                ...
                regions = None
                if "anatomicalhand" in (obj.name + " " + obj.data.name).lower():
                    regions = paint_single_hand_surface(obj, clone, rigs[0])
                else:
                    clone.data.materials.clear()
        ...
        nails, detail_features = refine_hand_set(generated, clone_rig, palette, side)
        generated.extend(nails)
        bpy.context.view_layer.update()
        for mesh_row in mesh_report:
            current = next(o for o in generated if o.name == mesh_row["output_name"])
            mesh_row["after_detail"] = mesh_signature(current)
        if rig_signature(clone_rig) != rig_before:
            raise RuntimeError(f"Detail generation changed rest skeleton: {side}")
        ...
        standalone = export_glb(output / f"{side}_hand_detailed.glb", output_scene, generated)
        ...
        if standalone["textures"]:
            raise RuntimeError(f"Detailed export unexpectedly contains textures: {side}")
        report["hands"][side] = {"source_aabb": native_bounds, "reduced_aabb": reduced_bounds,
                                 ...
                                 "original_total_triangles": original_triangles,
                                 "excluded_output_meshes": [{"source_name": o.name, "triangles": mesh_signature(o)["triangles"],
                                    "reason": "Overlapping glove shell replaced by regions on anatomical hand"} for o in excluded],
    ...
    activate(preview)
    report["combined"] = export_glb(output / "both_hands_detailed_preview.glb", preview, preview_objects)
    ...
    world = bpy.data.worlds.new("Detailed_Review_World")
    ...
    for name, offset, energy, size in (
        ("Key", (1, -1, 2), 65, 2), ("Fill", (-1, 1, 1), 38, 2),
        ("Rim", (0, 2, -1), 45, 1.5),
    ):
    ...
    views = {"dorsum": (0, 0, 1), "palm": (0, 0, -1), "side": (1, 0, 0),
             "closeup_dorsum": (0, -.18, 1), "closeup_palm": (0, .14, -1)}
    report["renders"] = {}
    for name, direction in views.items():
        ...
        if name == "side":
            ...
            bpy.context.view_layer.update()
        closeup = name.startswith("closeup")
        visible_objects = [o for o in preview_objects if o.type == "MESH" and
            ("anatomicalhand" in o.name.lower() or o.name.startswith("Nail_"))] if closeup else preview_objects
        view_bounds = bbox(visible_objects)
        view_center = Vector([(view_bounds["min"][i] + view_bounds["max"][i]) / 2 for i in range(3)])
        view_span = max(view_bounds["size"])
        camera_data.ortho_scale = view_span * (1.10 if closeup else 1.28)
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(output / "bilateral_hands_detailed.blend"))
    ...
    print("DETAILED_BUILD_COMPLETE", output)
