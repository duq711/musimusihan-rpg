...
frames = root / 'godot-game/artifacts/visual_qa/bandage_forearm'
manifest = json.loads((frames/'manifest.json').read_text())
assert len(manifest['frames']) == 160 and manifest['state_and_sources_preserved']
video = frames/'bandage_forearm.mp4'
...
with av.open(str(video), 'w', options={'movflags': '+faststart'}) as container:
    ...
    for frame in manifest['frames']:
        ...
        draw.text((24,20), '붕대 사용 · 왼팔 감기', font=font, fill='#f1eee7')
...
assert decoded == 160
print(f'{video}\n160 decoded frames / 30fps / {video.stat().st_size} bytes')
