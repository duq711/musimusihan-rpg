...
var pending_blacksmith_trial := ""
var pending_alchemy_trial := ""
var pending_cooking_trial := false
var pending_hideout_view := ""
...
func reset_loadout() -> void:
	if is_instance_valid(performance_overlay):
		performance_overlay.set_enabled(false)
	pending_cave_entry_room = ""
	pending_blacksmith_trial = ""
	pending_alchemy_trial = ""
	pending_cooking_trial = false
	pending_hideout_view = ""
...
func finish() -> void:
	...
	if is_instance_valid(performance_overlay):
		performance_overlay.set_enabled(false)
		performance_overlay.queue_free()
		performance_overlay = null
...
func toggle_performance_monitor() -> void:
	if not active:
		return
	if not is_instance_valid(performance_overlay):
		performance_overlay = PERFORMANCE_OVERLAY.new()
		performance_overlay.name = "TrialPerformanceMonitor"
		add_child(performance_overlay)
	performance_overlay.set_enabled(not performance_overlay.enabled)
...
func prepare_blacksmith_trial(station: String) -> bool:
	pending_blacksmith_trial = ""
	if not active or station not in ["", "forge", "grip", "blade", "rune"]:
		return false
	pending_blacksmith_trial = station
	if not station.is_empty():
		pending_hideout_view = ""
	return true


func consume_blacksmith_trial() -> String:
	var requested := pending_blacksmith_trial if active else ""
	pending_blacksmith_trial = ""
	return requested


func prepare_alchemy_trial(recipe_id: String) -> bool:
	pending_alchemy_trial = ""
	if not active:
		return false
	if not recipe_id.is_empty() and recipe_id not in preload("res://scripts/alchemy_catalog.gd").ordered_recipe_ids():
		return false
	pending_alchemy_trial = recipe_id
	if not recipe_id.is_empty():
		pending_hideout_view = ""
	return true


func consume_alchemy_trial() -> String:
	var requested := pending_alchemy_trial if active else ""
	pending_alchemy_trial = ""
	return requested


func prepare_cooking_trial(requested := true) -> bool:
	pending_cooking_trial = active and requested
	if pending_cooking_trial:
		pending_hideout_view = ""
	return active


func consume_cooking_trial() -> bool:
	var requested := active and pending_cooking_trial
	pending_cooking_trial = false
	return requested


func prepare_hideout_view(room_id: String) -> bool:
	pending_hideout_view = ""
	if not active:
		return false
	if room_id.is_empty():
		return true
	if HIDEOUT_RUIN_VIEWS.get_shot(room_id).is_empty():
		return false
	pending_cave_entry_room = ""
	pending_blacksmith_trial = ""
	pending_alchemy_trial = ""
	pending_cooking_trial = false
	pending_hideout_view = room_id
	return true


func consume_hideout_view() -> String:
	var requested := pending_hideout_view if active else ""
	pending_hideout_view = ""
	return requested
...
func return_to_room() -> void:
	...
	# The loading overlay always processes. Keep the departing scene paused so
	# its player cannot recover stress or advance survival before replacement.
...
func _cancel_scene_activities() -> void:
	...
	if scene.has_method("cancel_blacksmith"):
		scene.call("cancel_blacksmith")
	if scene.has_method("cancel_cooking"):
		scene.call("cancel_cooking")
	if scene.has_method("cancel_alchemy"):
		scene.call("cancel_alchemy")
