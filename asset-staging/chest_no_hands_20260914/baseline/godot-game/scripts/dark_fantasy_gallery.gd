...
var inspection_environment: Environment
var equipment_reflection: Sky
...
func select_object(id: String) -> bool:
	...
	object_id = id
	# Metal requires a reflected room, even against the neutral color backdrop.
	# Keep the existing diffuse lighting and use this only for the new equipment.
	var equipment := str(entries[index].get("factory_type", "")) == "sword_shield"
	if equipment and equipment_reflection == null:
		var room_light := ProceduralSkyMaterial.new()
		room_light.sky_top_color = Color(0.28, 0.30, 0.32)
		room_light.sky_horizon_color = Color(0.70, 0.69, 0.67)
		room_light.ground_bottom_color = Color(0.15, 0.15, 0.15)
		room_light.ground_horizon_color = Color(0.45, 0.44, 0.42)
		equipment_reflection = Sky.new()
		equipment_reflection.sky_material = room_light
	inspection_environment.sky = equipment_reflection if equipment else null
	inspection_environment.reflected_light_source = Environment.REFLECTION_SOURCE_SKY if equipment else Environment.REFLECTION_SOURCE_BG
...
func set_view(view_name: String) -> bool:
	...
	if object_root.has_meta("production_scene"):
		# Fit the visible profile, so axial sword views actually show the
		# guard and pommel instead of reserving space for the hidden blade length.
		var projected := AABB()
		for endpoint in 8:
			var point := camera.to_local(object_bounds.get_endpoint(endpoint))
			if endpoint == 0: projected = AABB(point, Vector3.ZERO)
			else: projected = projected.expand(point)
		camera.size = maxf(projected.size.x, projected.size.y) * 1.2 + 0.01
