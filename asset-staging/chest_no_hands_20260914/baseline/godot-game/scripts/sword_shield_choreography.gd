...
static func ready() -> Transform3D:
	if REFERENCE.has_track("idle"):
		return REFERENCE.sample("idle", 0.0)
	return LONG_GRIP.SOURCE_READY

static func keys(variant: String, charge: float) -> Array[Transform3D]:
	if REFERENCE.has_track(variant):
		var timing: Dictionary = REFERENCE.clip_metadata(variant).timing
		return [REFERENCE.sample(variant, timing.windup_seconds), REFERENCE.sample(variant, timing.windup_seconds + timing.hit_seconds), REFERENCE.sample(variant, timing.windup_seconds + timing.active_seconds)]
	...
	match variant:
		...
		_:
			...
			contact = held(Vector3(0.07,-0.22,-0.65), Vector3(-0.83,0.10,-0.512), 25)
			follow = held(Vector3(0.04,-0.30,-0.57), Vector3(-0.72,-0.66,-0.214476), 20)
...
static func sword(phase: String, elapsed: float, charge: float, variant: String) -> Transform3D:
	if phase == "ready":
		return ready()
	if phase in ["ready", "windup", "active", "recovery"] and REFERENCE.has_track(variant):
		return REFERENCE.retimed_attack(phase, elapsed, charge, variant, true)
	...
	match phase:
		...
		"active":
			...
			if variant == "right_diagonal":
				# The hand sweeps left and down through the real enemy blade before
				# its 90ms hit, then continues through contact without doubling back.
				crossing = held(Vector3(0.14,-0.18,-0.69),Vector3(-0.80,0.14,-0.59),20)
...
static func guard_sword(raise_progress: float) -> Transform3D:
	# The sword stays in the right hand, lowered beside the body during guard.
	var lowered := held(Vector3(0.48, -0.76, -0.48), Vector3(0.25, -0.92, -0.30), 10.0)
	return mix(ready(), lowered, raise_progress)
...
static func shield_guard() -> Transform3D:
	# Close, low framing: a wide upper arc along the bottom of the image.
	return pose(Vector3(0.0, -0.55, -0.43), Vector3(8.0, 0.0, 0.0))
...
static func shield(raise_progress: float, impact_remaining: float, phase: String, elapsed: float, charge: float, variant: String) -> Transform3D:
	...
	if REFERENCE.has_track("idle", "shield"):
		idle = REFERENCE.sample("idle", 0.0, "shield")
	var guard := shield_guard()
	...
	if phase in ["windup", "active", "recovery"] and REFERENCE.has_track(variant, "shield"):
		result = mix(REFERENCE.retimed_attack(phase, elapsed, charge, variant, true, "shield"), guard, raise_progress)
	# Bring the elbow inward while raising; both endpoint framings stay exact.
