...
const OPTIONAL_BASELINE := "../asset-staging/sword_shield_offscreen_20260914/baseline/godot-game/assets/animations/reference_sword_motion/motion_manifest.json"
...
func _test_attack(player: DungeonPlayer, variant: String, hold: float, stowed: bool) -> void:
	...
	player._update_viewmodel(0.0)
	if not stowed:
		_check(_near_pose(player.shield_pivot.transform, held_shield), context + ": opening at zero must start at the actual carried shield.")
	...
	for frame in 180:
		...
		if not stowed and player.combat_state in [DungeonPlayer.CombatState.WINDUP, DungeonPlayer.CombatState.ACTIVE, DungeonPlayer.CombatState.RECOVERY]:
			...
			_check(player.shield_pivot.is_visible_in_tree(), context + ": leftward clearance must keep the equipped shield visible, not stow it.")
			_check(shield.origin.y >= held_shield.origin.y - 0.015, context + ": the shield must clear sideways without dropping below carry.")
			_check(shield.origin.distance_to(previous_shield.origin) < 13.0 * delta + 0.0001 and shield.basis.get_rotation_quaternion().angle_to(previous_shield.basis.get_rotation_quaternion()) < 0.05, context + ": phase boundaries must not jump or twist the shield.")
			if player.combat_state != DungeonPlayer.CombatState.RECOVERY:
				_check(shield.origin.x <= previous_shield.origin.x + 0.00001, context + ": opening must continue left across charge and release without reversing.")
				if elapsed >= 0.12 - 0.000001:
					_check(_shield_is_left_of_view(player), context + ": every shield mesh bound must be entirely outside the left view plane before sword contact.")
			_check(not player.blocking and is_zero_approx(player.block_time), context + ": moving the shield aside must not silently enable damage blocking or just guard.")
...
func _check_source_preservation() -> void:
	...
	for clip: String in before_clips:
		_check(after_clips.has(clip) and after_clips[clip] == before_clips[clip], "Shield anticipation must preserve every sword clip exactly: " + clip)
...
func _test_handoffs(fixture: Dictionary, stowed: bool) -> void:
	...
	for elapsed: float in [0.075, 0.145]:
		for guard in [false, true]:
			...
			if not stowed:
				_check(_near_pose(shield, player.shield_pivot.transform), "Cancelling an incomplete shield opening must blend from its visible pose.")
		...
		if clashed:
			...
			if not stowed:
				_check(_near_pose(shield, player.shield_pivot.transform), "Clash recovery must start at the exact visible shield, including before clearance completes.")
...
func _check_geometry(player: DungeonPlayer, stowed: bool, context: String) -> void:
	...
	if not stowed:
		_check(float(player._hand_contacts.get("shield", {}).get("error", 1.0)) < 0.00001, context + ": the real left hand must remain attached to the shield grip while opening and returning.")
	if stowed:
...
func _shield_is_left_of_view(player: DungeonPlayer) -> bool:
	var found := false
	var projection := player.camera.get_camera_projection()
	for mesh: MeshInstance3D in player.shield_model.find_children("*", "MeshInstance3D", true, false):
		if mesh.mesh == null: continue
		found = true
		var bounds := mesh.get_aabb()
		for corner in 8:
			var point := player.camera.to_local(mesh.to_global(bounds.get_endpoint(corner)))
			var clip := projection * Vector4(point.x, point.y, point.z, 1.0)
			# Homogeneous left-plane test also handles bounds near the camera.
			if clip.x + clip.w >= -0.001: return false
	return found
