...
func _test_recoil_and_true_aim() -> void:
	...
	# The authored release pose, breathing clock and look sway also move the
	# bow. Isolate the actual recoil at the same presentation time and pose.
	var weak_timer := player._bow_recoil
	player._bow_recoil = 0.0
	player._update_viewmodel(0.0)
	var weak_resting_position := player.weapon_pivot.position
	var weak_kick := weak_position.distance_to(weak_resting_position)
	player._bow_recoil = weak_timer
	player._update_viewmodel(0.0)
	_check(player.weapon_pivot.position.is_equal_approx(weak_position), "sampling the same-pose recoil baseline must preserve the actual weak release")
	_check(weak_kick > 0.005, "weak release must produce visible weapon kick independent of its authored release pose")
	...
	player._update_viewmodel(1.0)
	var full_position := player.weapon_pivot.position
	var full_camera_rotation := player.camera.rotation.length()
	var full_timer := player._bow_recoil
	player._bow_recoil = 0.0
	player._update_viewmodel(0.0)
	var full_resting_position := player.weapon_pivot.position
	var full_kick := full_position.distance_to(full_resting_position)
	player._bow_recoil = full_timer
	player._update_viewmodel(0.0)
	_check(player.weapon_pivot.position.is_equal_approx(full_position), "sampling the same-pose recoil baseline must preserve the actual full release")
	_check(full_kick > 0.0 and full_kick < weak_kick, "full draw must have visibly smaller actual weapon recoil than weak draw at the same respective release poses")
	_check(full_camera_rotation > 0.0 and full_camera_rotation < weak_camera_rotation, "full draw must have less camera recoil than weak draw")
	...
	var recovered_position := player.weapon_pivot.position
	# Reevaluate the zero-recoil pose without advancing any animation time.
	# A stale kick left by the physics update must not survive this comparison.
	player._update_viewmodel(0.0)
	_check(player.weapon_pivot.position.is_equal_approx(recovered_position), "weapon recoil must recover to the matching zero-recoil pose without a residual displacement")
	print("BOW RECOIL MEASUREMENTS: old absolute weak=%.6f full=%.6f recovery=%.6f; same-pose kick weak=%.6f full=%.6f recovered=%.6f" % [weak_position.distance_to(resting_position), full_position.distance_to(resting_position), recovered_position.distance_to(resting_position), weak_kick, full_kick, recovered_position.distance_to(player.weapon_pivot.position)])
