...
func _run() -> void:
	...
	if available:
		...
		for clip: String in CLIPS:
			result[clip + "_recovery"] = _test_recovery_preservation(clip)
			result[clip + "_followthrough"] = _test_followthrough(clip)
...
func _test_recovery_preservation(clip: String) -> Dictionary:
	...
	var peak_step := 0.0
	var previous := MOTION.sample(clip, 0.8)
	var peak_visible_step := 0.0
	for index in range(1, 621):
		var pose := MOTION.sample(clip, 0.8 + float(index) * 0.001)
...
func _test_optional_preservation() -> String:
	...
	for clip_name: String in original_clips:
		if clip_name != "left_reverse":
			_check(current_clips.get(clip_name) == original_clips[clip_name], "Forehand reversal must preserve the original clip exactly, especially the completed forehand: " + clip_name)
...
func _outside_blade(pose: Transform3D) -> bool:
	...
	for i in 8:
		...
		right = right and point.x > -vertical * (1280.0 / 720.0) * point.z
		below = below and point.y < vertical * point.z
	return left or right or below


func _test_followthrough(clip: String) -> Dictionary:
	var direction := -1.0 if clip == "right_diagonal" else 1.0
	var contact := MOTION.sample(clip, CONTACT_TIME)
	var previous := contact
	var max_angle := 0.0
	var max_height_axis_error := 0.0
	for index in range(1, 91):
		var pose := MOTION.sample(clip, CONTACT_TIME + float(index) * 0.001)
		...
		for point: Vector3 in [GRIP.GRIP_CENTER, BLADE_POINTS.center, BLADE_POINTS.tip]:
			_check(direction * ((pose * point).x - (previous * point).x) >= -0.0001, "After contact the grip, blade center and tip must continue in the cutting direction, without reversing inside the view.")
		previous = pose
	...
	_check(_outside_blade(previous), "The full blade must pass beyond the corresponding viewport edge before recovery.")
	for index in range(401):
		var time := 0.8 + float(index) * 0.001
		_check(_outside_blade(MOTION.sample(clip, time)), "The blade must stay outside the view during lowered reorientation; time=" + str(time))
	var ready := MOTION.sample("idle", 0.0)
	for index in range(221):
		var pose := MOTION.sample(clip, 1.20 + float(index) * 0.001)
