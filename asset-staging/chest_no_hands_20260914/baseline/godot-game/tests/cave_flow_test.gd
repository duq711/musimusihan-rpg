...
func _run() -> void:
	...
	if not ResourceLoader.exists(CAVE_PATH):
		_finish()
		return
	print("CAVE FLOW CHECK: actual hideout map entry", " at ", Time.get_ticks_msec(), " ms")
	await _test_hideout_entry()
	await _clear_current_scene()
	print("CAVE FLOW CHECK: isolated repeated entry, death and F2 restoration", " at ", Time.get_ticks_msec(), " ms")
	await _test_sandbox_roundtrip()
	await _clear_current_scene()
	print("CAVE FLOW CHECK: every authored landmark entry", " at ", Time.get_ticks_msec(), " ms")
...
func _test_sandbox_roundtrip() -> void:
	...
	_check(room.panel_open and paused and room.player.health == DungeonPlayer.MAX_HEALTH, "F2 must return to a healthy player and paused test menu")
	...
	_check(cave.get_instance_id() != cave_id and cave.enemies_alive == initial_enemy_count and preload("res://tests/loot_test_helpers.gd").valid_count(cave), "reselecting the cave must create fresh encounters and a newly selected valid loot setup")
	...
	if not await _wait_for_scene(ROOM_PATH):
		return
	room = current_scene as Node3D
	_check(room.panel_open and room.player.health == DungeonPlayer.MAX_HEALTH and room.inventory == trial_inventory, "F2 after cave death must restore a playable test room without touching the original journey")
	room.reset_room()
...
func _test_landmark_entries() -> void:
	...
	for zone in zones:
		print("CAVE FLOW CHECK: landmark ", zone, " at ", Time.get_ticks_msec(), " ms")
		...
		_check(cave.enemies_alive == 6 and preload("res://tests/loot_test_helpers.gd").valid_count(cave) and cave.get_dungeon_info().chambers == 18, "landmark visits must contain the complete real rebuilt mine and gameplay")
