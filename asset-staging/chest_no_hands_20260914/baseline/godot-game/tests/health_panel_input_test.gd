...
func _check_combined_bounds(viewport: SubViewport, overlay: InventoryOverlay) -> void:
	...
	_check(loadout.inventory_buttons.size() == 30 and loadout.equipment_buttons.size() == 5 and loadout.quick_use_buttons.size() == 10, "Combined page must expose all real bag, equipment and quick slots.")
