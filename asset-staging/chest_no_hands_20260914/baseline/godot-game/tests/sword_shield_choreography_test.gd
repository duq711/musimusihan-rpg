...
func _audit_actual_arms(player: DungeonPlayer, context: String) -> void:
	...
	if sword_marker != null and contacts.has("sword"):
		...
		# Reference cuts intentionally leave the frame during preparation/follow-through.
		_check(not player.camera.is_position_behind(sword_marker.global_position), "Sword must remain in front of the camera near plane: " + context)
		if player.combat_state == DungeonPlayer.CombatState.READY and player._shield_raise_progress <= 0.001:
			_check(normalized.x > 0.65 and normalized.x < 0.94 and normalized.y > 0.8 and normalized.y < 1.15, "Reference idle grip must return to lower right: " + context)
	for role: String in ["shield"]:
		...
		# Lowered carry intentionally leaves the hand below the image; only the
		# raised protective grip must satisfy the former on-screen requirement.
		elif player.combat_state == DungeonPlayer.CombatState.READY and player._shield_raise_progress >= 0.999:
			var rim := player.camera.unproject_position(player.shield_model.to_global(Vector3(0, .427, 0))) / player.camera.get_viewport().get_visible_rect().size
			_check(rim.x > .35 and rim.x < .60 and rim.y > .60 and rim.y < .95, "Shield guard/recoil must preserve the low central rim: " + context)
