...
func _run() -> void:
	...
	room.run_feature("wall_equipment")
	_check(room.player.health == DungeonPlayer.MAX_HEALTH and room.player.position.distance_to(Vector3(8.0, 1.0, -15.3)) < 0.02, "wall trial must restore health and starting distance when reselected")
	...
	for iteration in range(2):
		...
		_check(cave.cave_geometry != null and cave.enemies_alive == 6 and preload("res://tests/loot_test_helpers.gd").valid_count(cave), "entry contact trial must retain real mine geometry and encounters")
