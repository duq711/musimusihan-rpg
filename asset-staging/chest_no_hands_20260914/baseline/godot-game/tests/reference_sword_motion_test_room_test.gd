...
const FEATURES := {"motion_shield_stow": "shield_stow","motion_sword_draw": "draw","motion_sword_run": "run", "motion_sword_jump": "jump", "motion_sword_sequence": "sequence"}
const STEP := 1.0 / 60.0
...
func _run() -> void:
	...
	for feature_id: String in FEATURES:
		...
		for iteration in range(2):
			...
			if family == "shield_stow":
				...
				_check(not player.shield_pivot.visible and not player._has_shield_equipped() and player.weapon_arm.visible and player.sword_support_arm.visible and player._sword_support_progress() == 1.0, "test-room stow must hide the shield and leave both hands gripping the sword")
				await _exercise_two_hand_cuts(room)
			elif family == "draw":
				_check(player._equipment_draw_active(), "draw trial must still be playing the production equip clip after 0.5 seconds")
				await _drive_frames(player, 72)
				_check(not player._equipment_draw_active() and player.weapon_pivot.basis.y.dot(Vector3.UP) > .97, "draw must settle to upright carry and release pose ownership")
	...
	print("REFERENCE SWORD MOTION TEST ROOM %s: five playable fixtures, two-hand basic/reverse target hits and grip, actual run/jump/landing/collision, repeated F2, reset and original expedition restoration" % ("PASS" if failures.is_empty() else "FAIL"))
...
func _exercise_two_hand_cuts(room: Node3D) -> void:
	var player: DungeonPlayer = room.player
	var target: DungeonEnemy = room.arm_motion_target
	_check(is_instance_valid(target) and room.enemies_alive == 1, "stow review must create one real cut target")
	if not is_instance_valid(target): return
	_check(target.position.is_equal_approx(room.TWO_HAND_CUT_TARGET) and not target.is_physics_processing(), "two-hand target must be stationary at the documented two-metre distance")
	# A real target hit translates the camera briefly through production shake.
	# Preserve the player's aim, rather than rejecting that visible hit feedback.
	var look := Vector2(player.rotation.y, player._pitch)
	var head_basis := player.head.basis
	var camera_basis := player.camera.basis
	for variant: String in ["right_diagonal", "left_reverse"]:
		var health := target.health
		var stamina := player.stamina
		var cost := player.get_melee_stamina_cost(0.0)
		var start_pose := player.weapon_pivot.transform
		var maximum_motion := 0.0
		var phases := {}
		var attack := player.begin_sword_attack()
		_check(bool(attack.get("accepted", false)) and str(attack.get("variant", "")) == variant, "repeated LMB must review the production basic then reverse cut: " + variant)
		if not bool(attack.get("accepted", false)): return
		_check(player.combat_state == DungeonPlayer.CombatState.WINDUP, "begin must record the real pre-release state even when the next tick starts the cut: " + variant)
		phases[player.combat_state] = true
		_check(is_equal_approx(player.get_melee_hit_time(), .145) and is_equal_approx(player.get_melee_active_duration(), .30), "two-hand review must retain the shared real hit and active clocks")
		player.attack_release_requested = true
		for frame in range(120):
			await physics_frame
			player.advance_action_timers(STEP)
			...
			if frame == 0:
				_check(player.combat_state == (DungeonPlayer.CombatState.ACTIVE if variant == "right_diagonal" else DungeonPlayer.CombatState.WINDUP), "the first release tick must start forehand immediately and retain reverse's existing windup")
			player._update_viewmodel(STEP)
			player._resolve_active_attack()
			phases[player.combat_state] = true
			maximum_motion = maxf(maximum_motion, player.weapon_pivot.position.distance_to(start_pose.origin))
			var snapshot := player.get_first_person_motion_snapshot()
			var support: Dictionary = snapshot.hand_contacts.get("sword_support", {})
			_check(player.weapon_arm.is_visible_in_tree() and player.sword_support_arm.is_visible_in_tree() and float(support.get("grip_error", 1.0)) < 0.00001, "both actual hands must keep their sword grip throughout " + variant)
			_check(not player._has_shield_equipped() and not player.shield_pivot.visible, "cut review must retain its stowed shield: " + variant)
			_check(Vector2(player.rotation.y, player._pitch).is_equal_approx(look) and player.head.basis.is_equal_approx(head_basis) and player.camera.basis.is_equal_approx(camera_basis), "cut review must preserve the player's aim through normal hit feedback: " + variant)
			if player.combat_state == DungeonPlayer.CombatState.READY: break
		for phase in [DungeonPlayer.CombatState.WINDUP, DungeonPlayer.CombatState.ACTIVE, DungeonPlayer.CombatState.RECOVERY, DungeonPlayer.CombatState.READY]:
			_check(phases.has(phase), "cut review must traverse the real action and finish: %s phase=%d" % [variant, phase])
		_check(maximum_motion > 0.1 and target.health < health, "review must animate the real sword and damage the real target: " + variant)
		_check(is_equal_approx(stamina - player.stamina, cost), "review must charge one normal attack cost: " + variant)
	_check(bool(player.begin_sword_attack().get("accepted", false)), "two-hand review must allow the next real cut before F2 cancels it")
