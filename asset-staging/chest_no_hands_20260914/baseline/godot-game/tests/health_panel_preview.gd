...
static func inspect_fixture(viewport: SubViewport, fixture: Dictionary, shot: Dictionary) -> Dictionary:
	...
	if loadout.inventory_buttons.size() != 30 or loadout.equipment_buttons.size() != 5 or loadout.quick_use_buttons.size() != 10:
		failures.append("Combined loadout must expose the actual five equipment slots, thirty storage cells and ten quick-use controls.")
...
static func collect_hashes() -> Dictionary:
	...
	for path in ["res://assets/fonts/Cinzel-Variable.ttf", "res://assets/ui/health_armor_icons.svg"]:
		hashes[path] = FileAccess.get_sha256(path)
	return hashes
