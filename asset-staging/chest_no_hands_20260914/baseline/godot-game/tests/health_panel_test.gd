...
func _run() -> void:
	...
	_check(loadout.inventory_buttons.size() == 30 and loadout.equipment_buttons.size() == 5 and loadout.quick_use_buttons.size() == 10 and loadout.quick_use_item_ids.size() == 10, "Combined loadout must expose thirty real bag cells, five equipment slots and ten quick-use slots.")
	...
	for failure in failures:
		push_error(failure)
	print("HEALTH PANEL TEST %s: simultaneous actual health/loadout, 30 bag cells, five equipped slots, ten quick slots, treatment signals, conditions, silhouette input, shared navigation and responsive bounds" % ("PASS" if failures.is_empty() else "FAIL"))
