...
func _test_phases_and_completion() -> void:
	...
	for side in [-1, 1]:
		...
		_check(skeleton != null and rig.visual_profile == "original" and rig.get_meta("supplied_hand", false), "ordinary chest interaction must use the supplied hands without a review toggle")
		if skeleton == null: continue
		for digit: String in ["thumb", "index", "middle", "ring", "little"]:
			for joint in 3:
				var bone := skeleton.find_bone(digit + str(joint))
				if bone >= 0: fingers.append({"skeleton": skeleton, "bone": bone})
	_check(fingers.size() == 30, "both supplied hands must expose all thirty actual finger bones during chest work")
	var finger_poses: Array[Transform3D] = []
	for finger: Dictionary in fingers:
		finger_poses.append((finger.skeleton as Skeleton3D).get_bone_pose(int(finger.bone)))
	...
	for index in fingers.size():
		...
		finger_moved = finger_moved or (finger.skeleton as Skeleton3D).get_bone_pose(int(finger.bone)) != finger_poses[index]
	_check(finger_moved, "fidgeting must animate the supplied hand's real skeletal finger joints")
...
func _test_front_lift_framing() -> void:
	...
	for hand: Node3D in [player.chest_hands.left_hand, player.chest_hands.right_hand]:
		...
		var adapter := arm.get("detailed_visual") as Node3D
		var upper := adapter.get("_upper_arm") as Node3D
		var imported_rest: Transform3D = adapter.get("_part_rests")[upper.get_instance_id()]
		# The imported sleeve's shoulder endpoint is z=0.60 in adapter rest
		# space. Follow its actual source-to-mesh frame and current fitted node.
		var local_shoulder := imported_rest.affine_inverse() * Vector3(0.0, 0.0, 0.60)
		var actual_shoulder := upper.to_global(local_shoulder)
