...
func _run() -> void:
	...
	source_paths.append("res://scripts/hud.gd")
	...
	var guard_hud := OS.get_environment("SWORD_SHIELD_QA_GUARD_HUD") == "1" and not hand_study
	var shots: Array = ["study_30", "study_70", "study_110", "study_140"] if hand_study else POSES.duplicate()
	if guard_hud: shots.append("exhausted")
	for pose_id in shots:
		...
		var valid := configure_pose(fixture, "guard" if hand_study else ("idle" if pose_id == "exhausted" else pose_id))
		...
		if guard_hud:
			...
			player.block_time = 0.4 if pose_id == "guard" else 0.1
			if pose_id == "exhausted": player.stamina = 0.0
			player.advance_combat_state(0.01, pose_id in ["guard", "impact", "exhausted"])
			if pose_id == "exhausted":
				PREVIEW._sample_view(player, 0.4)
				valid = valid and not player.blocking and is_zero_approx(player.block_time) and is_zero_approx(player._shield_raise_progress)
			hud.update_health(player.health, player.MAX_HEALTH)
			hud.update_stamina(player.stamina, player.MAX_STAMINA)
		player.viewmodel_renderer.sync_view()
		for frame in 12: await process_frame
		var guard_result := {}
		if guard_hud and pose_id in ["guard", "impact"]:
			guard_result = player.receive_attack(18.0, player.global_position - player.global_basis.z * 2.0, "bleeding")
			valid = valid and bool(guard_result.blocked) and is_zero_approx(float(guard_result.damage)) and bool(guard_result.parried) == (pose_id == "impact")
			await process_frame
		...
		if guard_hud:
			...
			result["blocking"] = player.blocking
			result["state_caption"] = player.hud.state_label.text
			result["event_caption"] = player.hud.event_label.text
	...
	print("SWORD SHIELD PREVIEW %s: %s; %s" % ["PASS" if failures.is_empty() else "FAIL", "four diagnostic hand rotations" if hand_study else ("five production guard HUD states" if guard_hud else "four production poses"), output])
