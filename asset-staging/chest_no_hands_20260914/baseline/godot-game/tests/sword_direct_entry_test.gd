...
func _exercise_attack(player: DungeonPlayer, variant: String, hold: float, stowed: bool) -> void:
	...
	_check(is_equal_approx(entry_duration, 0.10), context + ": all cuts must share the same 100ms held-to-authored entry")
	...
	var before_entry_boundary: Dictionary = {}
	...
	for frame in 420:
		...
		# Accumulated simulation time can reach 0.099999999... at the nominal
		# checkpoint. Check strict ownership on each side without rewriting the
		# player's time, and verify the actual blade/hands stay continuous.
		if player.combat_state == DungeonPlayer.CombatState.ACTIVE:
			if absf(player.state_time - (0.10 - ENTRY_BOUNDARY_EPSILON)) < 0.0000001:
				before_entry_boundary = actual
				_check(player._uses_direct_sword_entry("active"), context + ": the sample just before 100ms must still use direct entry")
			if absf(player.state_time - (0.10 + ENTRY_BOUNDARY_EPSILON)) < 0.0000001:
				after_entry_boundary_seen = true
				_check(not player._uses_direct_sword_entry("active"), context + ": the sample just after 100ms must use the authored pose")
				_check(not before_entry_boundary.is_empty(), context + ": the entry boundary must include both real-clock samples")
				if not before_entry_boundary.is_empty():
					_check(_point_drift(before_entry_boundary, actual) < 0.0001 and _angle(before_entry_boundary.pose, actual.pose) < 0.06, context + ": blade, grip and actual arms must remain continuous across the 100ms boundary")
	...
	_check(wrist_error < 0.00003, context + ": the actual sleeve wrist must follow the final authored sword and original hand")
	...
	_check(not before_entry_boundary.is_empty() and after_entry_boundary_seen, context + ": the real combat ticks must bracket the shared 100ms entry boundary")
	_check(checkpoint_100ms_seen and rejoin_seen and contact_seen and preserved_samples > 10 and source_position_error < 0.00003 and source_angle_error < 0.06, context + ": all cuts must retain their exact authored pose from 100ms through contact and recovery")
...
func _finish() -> void:
	...
	print("SWORD DIRECT ENTRY %s: immediate forehand release, held charge, shared 100ms entry, exact authored contact/recovery, actual wrist and arm lengths, cost clocks and expedition preservation; target damage is covered by sword_attack_variants" % ("PASS" if failures.is_empty() else "FAIL"))
