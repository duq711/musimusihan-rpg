...
func _test_unstarted_journey_round_trip() -> void:
	...
	_check(snapshot.size() == 11 and snapshot.has("stress") and snapshot.has("body_health"), "the snapshot must include all eleven mutable expedition fields, including body health")
...
func _test_camping_supplies_round_trip() -> void:
	...
	_check(ExpeditionSession.capture_snapshot() == snapshot and snapshot.size() == 11, "scene-local camping must preserve the eleven expedition fields without persisting temporary effects")


func _test_body_health_round_trip() -> void:
	ExpeditionSession.begin_new_journey()
	var original := ExpeditionSession.get_inventory()
	ExpeditionSession.body_health.parts.left_arm = 0.0
	ExpeditionSession.body_health.parts.thorax = 61.5
	ExpeditionSession.body_health.selected_part = "left_arm"
	ExpeditionSession.apply_condition("fracture", 87.0, "left_arm")
	ExpeditionSession.apply_condition("bleeding", 23.0, "right_leg")
	var snapshot := ExpeditionSession.capture_snapshot()
	ExpeditionSession.body_health.parts.thorax = 1.0
	ExpeditionSession.body_health.condition_parts.fracture.append("right_leg")
	_check(snapshot.body_health.parts.thorax == 61.5 and snapshot.body_health.condition_parts.fracture == ["left_arm"], "body part values and nested condition locations must be copied at capture time")
	ExpeditionSession.begin_new_journey()
	_check(ExpeditionSession.body_health.parts.left_arm == 60.0 and ExpeditionSession.body_health.selected_part == "" and ExpeditionSession.body_health.condition_parts.is_empty(), "new trial journey must reset body parts, selected treatment and injury locations independently")
	ExpeditionSession.body_health.parts.head = 0.0
	ExpeditionSession.restore_snapshot(snapshot)
	_check(ExpeditionSession.get_inventory() == original and ExpeditionSession.capture_snapshot() == snapshot, "trial death and treatment changes must restore the exact original body and inventory reference")
	ExpeditionSession.body_health.parts.left_arm = 1.0
	ExpeditionSession.body_health.condition_parts.bleeding.clear()
	_check(snapshot.body_health.parts.left_arm == 0.0 and snapshot.body_health.condition_parts.bleeding == ["right_leg"], "restored body dictionaries must not mutate their reusable snapshot")
	ExpeditionSession.restore_snapshot(snapshot)
	_check(ExpeditionSession.capture_snapshot() == snapshot, "body health snapshots must remain reusable after restoration and surgery")
