...
func _test_planning_and_actual_actions() -> void:
	for id: String in ["rest", "meal", "treat"]:
		...
		_check(is_equal_approx(player.health, 30.0) and is_equal_approx(player.stamina, 20.0) and completions.is_empty(), "partial progress must not apply completion healing or stamina")
		...
		var expected_health := 30.0 + float(CAMP.ACTIONS[id].health)
...
func _test_atomic_resources_and_no_effect() -> void:
	for failure: String in ["no_kit", "meal_supply", "bandage", "no_effect", "invalid_action", "late_transaction"]:
