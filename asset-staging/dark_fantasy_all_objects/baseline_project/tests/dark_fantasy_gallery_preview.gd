...
func _run() -> void:
	...
	for entry in objects:
		...
		var record := {"source_path": source_path, "source_sha256": FileAccess.get_sha256(source_path) if FileAccess.file_exists(source_path) else "", "id": id, "title": entry.title, "factory": entry.get("factory", ""), "images": [], "bounds": {"position": _vector(gallery.object_bounds.position), "size": _vector(gallery.object_bounds.size)}}
	...
	var sources := {}
	for script in DirAccess.get_files_at("res://scripts"):
		if script.ends_with(".gd"):
			var path := "res://scripts/" + script
			sources[path] = FileAccess.get_sha256(path)
	sources["res://tests/dark_fantasy_gallery_preview.gd"] = FileAccess.get_sha256("res://tests/dark_fantasy_gallery_preview.gd")
