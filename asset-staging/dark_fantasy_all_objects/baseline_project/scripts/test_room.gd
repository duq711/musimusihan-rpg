...
func _restock_trial_supplies(supplies: Dictionary) -> int:
	...
	while inventory.slots.size() + required_slots > ExpeditionInventory.MAX_SLOTS:
...
func _spawn_archery_accuracy_target() -> void:
	var board := _add_static_box("ArcheryAccuracyTarget", Vector3(0, 2.5, -3), Vector3(5.4, 5, 0.25), _material(Color(0.22, 0.135, 0.07), 0.94, 0.0))
	...
	for index in range(radii.size()):
		...
		marking.mesh = disc
		marking.material_override = _material(colors[index], 0.97, 0.0)
