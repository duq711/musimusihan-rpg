...
func _build_viewmodel() -> void:
	...
	_add_gauntlet(weapon_pivot, Vector3(0.0, -0.24, 0.015), dark_steel, leather, false)
...
func _build_shield(iron: StandardMaterial3D, leather: StandardMaterial3D) -> void:
	...
	_add_gauntlet(shield_pivot, Vector3(-0.17, -0.38, -0.08), iron, leather, true)
...
func _build_equipped_torch(iron: StandardMaterial3D) -> void:
	...
	_add_gauntlet(
		...
		iron,
		_material(Color(0.12, 0.045, 0.018), 0.94, 0.02),
		false
	)
...
func _add_gauntlet(
	parent: Node3D,
	origin: Vector3,
	iron: StandardMaterial3D,
	leather: StandardMaterial3D,
	mirrored: bool
) -> void:
	# Match the full-body wanderer's padded sleeves, strapped leather bracers
	# and fingerless gloves. Existing hand pivots and combat contacts stay shared.
	var side := -1.0 if mirrored else 1.0
	var glove_material := PLAYER_APPEARANCE.leather_material()
	var cloth := PLAYER_APPEARANCE.cloth_material()
	var skin := PLAYER_APPEARANCE.skin_material()
	var forearm := _cylinder_mesh(0.090, 0.12, 0.42, cloth)
	forearm.name = "LeatherForearm"
	forearm.position = origin + Vector3(0.11 * side, -0.18, 0.055)
	forearm.rotation.z = deg_to_rad(18.0 * side)
	parent.add_child(forearm)

	var cuff := _cylinder_mesh(0.117, 0.103, 0.26, glove_material)
	cuff.name = "BlackIronCuff"
	cuff.set_meta("appearance_material", "worn_leather_bracer")
	cuff.position = origin + Vector3(0.063 * side, -0.07, 0.035)
	cuff.rotation.z = deg_to_rad(16.0 * side)
	parent.add_child(cuff)
	for index in 3:
		var strap := _cylinder_mesh(0.122 - index * 0.005, 0.119 - index * 0.005, 0.018, leather)
		strap.name = "BracerStrap%d" % index
		strap.position = cuff.position + Vector3(0.0, -0.09 + index * 0.085, 0.0)
		strap.rotation.z = cuff.rotation.z
		parent.add_child(strap)
		var clasp := _sphere_mesh(0.5, iron)
		clasp.name = "BracerRivet%d" % index
		clasp.position = strap.position + Vector3(0.0, 0.0, -0.119)
		clasp.scale = Vector3(0.008, 0.008, 0.004)
		parent.add_child(clasp)

	var hand := _sphere_mesh(0.5, glove_material)
	hand.name = "GauntletPalm"
	hand.position = origin + Vector3(-0.015 * side, 0.07, 0.025)
	hand.scale = Vector3(0.115, 0.15, 0.105)
	parent.add_child(hand)
	for index in range(4):
		var knuckle := _sphere_mesh(0.5, glove_material)
		knuckle.name = "Knuckle%02d" % index
		knuckle.position = hand.position + Vector3((float(index) - 1.5) * 0.034, 0.07, -0.055)
		knuckle.scale = Vector3(0.031, 0.037, 0.028)
		parent.add_child(knuckle)
		var finger := _sphere_mesh(0.5, skin)
		finger.name = "ExposedFingertip%02d" % index
		finger.position = knuckle.position + Vector3(0.0, -0.014, -0.023)
		finger.scale = Vector3(0.028, 0.030, 0.029)
