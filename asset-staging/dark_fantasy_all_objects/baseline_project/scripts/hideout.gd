...
enum HideoutMode { RUNNING, MAP, INVENTORY, PAUSED, RESTING, TRANSITIONING }
...
func _build_materials() -> void:
	stone_material = _textured_material(Color(0.64, 0.69, 0.68), WALL_TEXTURE, 0.91, 0.015, 0.72)
	floor_material = _textured_material(Color(0.55, 0.62, 0.61), FLOOR_TEXTURE, 0.33, 0.025, 0.84)
	wood_material = _textured_material(Color(0.53, 0.38, 0.25), OAK_TEXTURE, 0.92, 0.01, 2.8)
	metal_material = _textured_material(Color(0.32, 0.34, 0.33), IRON_TEXTURE, 0.51, 0.74, 3.6)
	dark_material = _material(Color(0.017, 0.021, 0.021), 0.96, 0.01)
	straw_material = _material(Color(0.36, 0.27, 0.13), 0.98, 0.0)
	cloth_material = _material(Color(0.17, 0.16, 0.13), 0.98, 0.0)
	moss_material = _material(Color(0.075, 0.13, 0.09), 0.97, 0.0)
	bone_material = _material(Color(0.46, 0.43, 0.34), 0.91, 0.01)
	ember_material = _material(Color(0.11, 0.025, 0.008), 0.82, 0.02)
	ember_material.emission_enabled = true
	ember_material.emission = Color(0.85, 0.10, 0.018)
	ember_material.emission_energy_multiplier = 2.7
...
func _build_world() -> void:
	...
	_add_water_drips(Vector3(0.0, 5.0, -0.8), Vector3(5.8, 0.1, 7.5), 34)
...
func _build_environment() -> void:
	...
	environment.background_color = Color(0.003, 0.007, 0.009)
	environment.ambient_light_source = Environment.AMBIENT_SOURCE_COLOR
	environment.ambient_light_color = Color(0.075, 0.13, 0.15)
...
func _build_central_hall() -> void:
	...
	saint_light.position = Vector3(0.0, 3.55, -5.8)
	saint_light.omni_range = 5.4
	saint_light.light_color = Color(0.17, 0.48, 0.58)
	saint_light.light_energy = 1.05
...
func _build_sleeping_cell() -> void:
	...
	_add_static_box(region, "BedFrame", Vector3(-12.6, 0.28, 6.7), Vector3(3.7, 0.42, 1.65), wood_material)
	_add_visual_box(region, "StrawMattress", Vector3(-12.6, 0.57, 6.7), Vector3(3.45, 0.28, 1.45), straw_material)
	_add_visual_box(region, "ThreadbareBlanket", Vector3(-12.9, 0.75, 6.68), Vector3(2.2, 0.08, 1.5), cloth_material, Vector3(0.0, 0.0, -2.0))
	_add_visual_box(region, "RolledPillow", Vector3(-14.05, 0.77, 6.7), Vector3(0.48, 0.25, 1.08), cloth_material)
	...
	if chest != null:
		...
		_apply_material_to_prefix(chest, ["ChestBody", "FrontPlank", "BackPlank", "LidSlat"], wood_material)
		_apply_material_to_prefix(chest, ["LidBand", "Corner", "Lock", "Hinge"], metal_material)
...
func _build_storage_room() -> void:
	...
	_add_visual_box(region, "EmptySack", Vector3(9.5, 0.14, 3.05), Vector3(1.1, 0.26, 0.72), straw_material, Vector3(0.0, 18.0, 5.0))
...
func _build_workshop() -> void:
	...
	_add_static_box(region, "BrokenWorkbenchCollision", Vector3(11.2, 0.62, -7.15), Vector3(4.6, 1.22, 1.25), wood_material)
	for x in [9.4, 13.0]:
		_add_visual_box(region, "WorkbenchLeg", Vector3(x, 0.47, -7.15), Vector3(0.32, 0.95, 0.78), wood_material, Vector3(0.0, 0.0, 4.0 if x < 11.0 else -5.0))
	_add_visual_box(region, "Whetstone", Vector3(10.0, 1.32, -7.12), Vector3(0.72, 0.22, 0.34), stone_material, Vector3(0.0, 14.0, 0.0))
	_add_visual_box(region, "RustyVice", Vector3(12.65, 1.35, -7.15), Vector3(0.44, 0.42, 0.56), metal_material)
	_add_interactable(region, "WorkbenchInteraction", "workbench", "부서진 작업대 점검", Vector3(11.2, 0.0, -6.9), Vector3(4.8, 2.1, 1.9), Vector3(0.0, 1.0, 0.0), 0.7)
	_build_weapon_rack(region, Vector3(14.9, 0.0, -2.8))
	_add_rubble_cluster(region, Vector3(9.0, 0.02, -3.0), 0.56)
	_add_wall_torch(region, Vector3(11.9, 2.45, -7.82), 0.0, Color(0.78, 0.31, 0.11), 0.58)
...
func _build_flooded_store() -> void:
	...
	_add_moss_patch(region, Vector3(-15.03, 1.1, -4.9), Vector2(2.2, 1.4), 90.0)
	...
	_add_water_drips(Vector3(-11.8, 3.95, -5.0), Vector3(2.2, 0.1, 1.9), 18, region)
...
func _build_sealed_ossuary() -> void:
	...
	ossuary_light.position = Vector3(0.0, 1.1, -17.4)
	ossuary_light.light_color = Color(0.12, 0.62, 0.57)
	ossuary_light.light_energy = 0.72
...
func _build_drainage_tunnel() -> void:
	...
	drain_light.position = Vector3(21.7, 1.15, -5.0)
	drain_light.light_color = Color(0.10, 0.40, 0.46)
	drain_light.light_energy = 0.82
...
func _build_broken_saint(parent: Node3D) -> void:
	...
	_add_cylinder_visual(statue, "SaintRobe", Vector3(0.0, 1.85, 0.0), 0.58, 0.82, 1.75, stone_material)
	_add_visual_box(statue, "BrokenShoulders", Vector3(0.0, 2.68, 0.0), Vector3(1.55, 0.42, 0.72), stone_material, Vector3(0.0, 0.0, -4.0))
	_add_visual_box(statue, "SaintArmL", Vector3(-0.72, 2.05, 0.0), Vector3(0.30, 1.32, 0.38), stone_material, Vector3(0.0, 0.0, -12.0))
	_add_visual_box(statue, "SaintArmR", Vector3(0.72, 1.92, 0.0), Vector3(0.30, 1.16, 0.38), stone_material, Vector3(0.0, 0.0, 16.0))
...
func _build_hearth(parent: Node3D) -> void:
	...
	_add_cylinder_visual(hearth_root, "AshBed", Vector3(0.0, 0.12, 0.0), 0.70, 0.70, 0.12, dark_material)
	for offset in [Vector3(-0.28, 0.28, 0.0), Vector3(0.28, 0.28, 0.0), Vector3(0.0, 0.31, -0.26)]:
		_add_visual_box(hearth_root, "EmberLog", offset, Vector3(0.72, 0.16, 0.18), ember_material, Vector3(0.0, 35.0 * offset.x, 0.0))
	...
	hearth_light.position = Vector3(0.0, 0.72, 0.0)
	...
	_add_interactable(hearth_root, "HearthInteraction", "hearth", "화로 손보기", Vector3.ZERO, Vector3(2.1, 1.65, 2.1), Vector3(0.0, 0.82, 0.0), 0.35)
	_add_cylinder_visual(parent, "CookingPot", Vector3(1.28, 0.42, 2.0), 0.36, 0.29, 0.48, metal_material)
	_add_visual_box(parent, "SingleCup", Vector3(1.58, 0.28, 2.72), Vector3(0.23, 0.36, 0.23), metal_material)
	_add_interactable(parent, "CookingPotInteraction", "meal", "식어 가는 냄비 확인", Vector3(1.25, 0.0, 2.1), Vector3(1.4, 1.4, 1.4), Vector3(0.0, 0.7, 0.0), 0.0)


func _build_small_stool(parent: Node3D, stool_position: Vector3) -> void:
	_add_cylinder_visual(parent, "StoolSeat", stool_position + Vector3(0.0, 0.54, 0.0), 0.52, 0.52, 0.16, wood_material)
	for offset in [Vector3(-0.28, 0.25, -0.22), Vector3(0.28, 0.25, -0.22), Vector3(0.0, 0.25, 0.30)]:
		_add_visual_box(parent, "StoolLeg", stool_position + offset, Vector3(0.16, 0.52, 0.16), wood_material)
...
func _build_shelf(parent: Node3D, shelf_position: Vector3, yaw_degrees: float, width: float) -> void:
	...
	for x in [-width * 0.43, width * 0.43]:
		_add_visual_box(shelf, "ShelfPost", Vector3(x, 1.25, 0.0), Vector3(0.22, 2.5, 0.42), wood_material)
	for y in [0.35, 1.25, 2.15]:
		_add_visual_box(shelf, "ShelfBoard", Vector3(0.0, y, 0.0), Vector3(width, 0.16, 0.78), wood_material, Vector3(0.0, 0.0, 1.5 if y > 1.0 else -1.0))
...
func _add_water_drips(origin: Vector3, extents: Vector3, amount: int, parent_override: Node3D = null) -> void:
	...
	drips.scale_amount_max = 0.72
	var quad := QuadMesh.new()
	quad.size = Vector2(0.009, 0.055)
	var material := _material(Color(0.42, 0.72, 0.76, 0.58), 0.08, 0.0)
	material.transparency = BaseMaterial3D.TRANSPARENCY_ALPHA
	material.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED
	material.billboard_mode = BaseMaterial3D.BILLBOARD_ENABLED
	quad.material = material
...
func _add_rubble_cluster(parent: Node3D, center: Vector3, scale_value: float) -> void:
	...
	for index in range(offsets.size()):
		...
		_add_visual_box(parent, "BrokenStone", center + offsets[index] * scale_value, size_value, stone_material, Vector3(float(index * 9 % 17), float(index * 31 % 83), float(index * 7 % 14)))


func _add_crate(parent: Node3D, position_value: Vector3, size_value: Vector3) -> void:
	_add_visual_box(parent, "EmptyCrate", position_value + Vector3(0.0, size_value.y * 0.5, 0.0), size_value, wood_material, Vector3(0.0, float(_piece_index * 17 % 21) - 10.0, 0.0))
	for y_sign in [-1.0, 1.0]:
		_add_visual_box(parent, "CrateBand", position_value + Vector3(0.0, size_value.y * 0.5 + y_sign * size_value.y * 0.29, size_value.z * 0.51), Vector3(size_value.x * 1.04, 0.08, 0.05), metal_material)


func _add_barrel(parent: Node3D, position_value: Vector3, radius: float, height: float) -> void:
	_add_cylinder_visual(parent, "EmptyBarrel", position_value + Vector3(0.0, height * 0.5, 0.0), radius * 0.9, radius, height, wood_material, 18)
	for y in [height * 0.22, height * 0.78]:
		var ring := MeshInstance3D.new()
		ring.name = "BarrelIronBand"
		var torus := TorusMesh.new()
		torus.inner_radius = radius * 0.87
		torus.outer_radius = radius * 0.96
		torus.rings = 18
		torus.ring_segments = 6
		ring.mesh = torus
		ring.material_override = metal_material
		ring.position = position_value + Vector3(0.0, y, 0.0)
		ring.visibility_range_end = 28.0
		parent.add_child(ring)


func _add_bucket(parent: Node3D, position_value: Vector3, scale_value: float) -> void:
	_add_cylinder_visual(parent, "RainBucket", position_value + Vector3(0.0, scale_value * 0.45, 0.0), scale_value * 0.44, scale_value * 0.34, scale_value * 0.9, metal_material, 16)
	var water_mesh := PlaneMesh.new()
	water_mesh.size = Vector2.ONE * scale_value * 0.72
	var water := MeshInstance3D.new()
	water.name = "CaughtRainWater"
	water.mesh = water_mesh
	water.material_override = water_material
	water.position = position_value + Vector3(0.0, scale_value * 0.89, 0.0)
...
func _add_hanging_cloth(parent: Node3D, position_value: Vector3, size_value: Vector2, color_value: Color) -> void:
	...
	var cloth_mat := _material(color_value, 0.99, 0.0)
	_add_visual_box(cloth_root, "CloakCloth", Vector3(0.0, -size_value.y * 0.5, 0.0), Vector3(size_value.x, size_value.y, 0.055), cloth_mat, Vector3(0.0, 0.0, 2.5))
...
func _add_hanging_herbs(parent: Node3D, position_value: Vector3) -> void:
	...
	for index in range(5):
		...
		_add_cylinder_visual(herb_root, "DriedHerbBundle", Vector3(x, -0.72, 0.0), 0.13, 0.04, 0.42, moss_material, 7)
...
func _add_candle_cluster(parent: Node3D, base_position: Vector3, count: int) -> void:
	for index in range(count):
		...
		_add_cylinder_visual(parent, "UsedCandle", base_position + offset + Vector3(0.0, height * 0.5, 0.0), 0.035, 0.045, height, bone_material, 8)
...
func _add_moss_patch(parent: Node3D, position_value: Vector3, size_value: Vector2, yaw_degrees: float) -> void:
	...
	patch.mesh = quad
	patch.material_override = moss_material
...
func _add_iron_gate(parent: Node3D, gate_position: Vector3, width: float, height: float, along_x: bool) -> void:
	...
	for index in range(bar_count):
		...
		_add_cylinder_visual(gate, "GateBar", position_value, 0.055, 0.055, height, metal_material, 8)
	for y in [0.45, height - 0.45]:
		var size_value := Vector3(0.15, 0.13, width) if along_x else Vector3(width, 0.13, 0.15)
...
func _add_niche_wall(parent: Node3D, origin: Vector3, columns: int, rows: int, yaw_degrees := 0.0) -> void:
	...
	for row in range(rows):
		for column in range(columns):
			...
			_add_visual_box(root_node, "NicheVoid", Vector3(x, y, 0.03), Vector3(0.78, 0.78, 0.10), dark_material)
			for side in [-1.0, 1.0]:
				_add_visual_box(root_node, "NicheFrame", Vector3(x + side * 0.46, y, -0.04), Vector3(0.11, 0.95, 0.18), stone_material)
			_add_visual_box(root_node, "NicheLintel", Vector3(x, y + 0.46, -0.04), Vector3(1.02, 0.11, 0.18), stone_material)
			if (row + column) % 3 != 1:
...
func _add_skull(parent: Node3D, position_value: Vector3, scale_value: float) -> void:
	...
	sphere.height = scale_value * 1.7
	sphere.radial_segments = 8
	sphere.rings = 5
	skull.mesh = sphere
	skull.material_override = bone_material
	skull.position = position_value
	skull.scale = Vector3(0.84, 1.0, 0.72)
	skull.visibility_range_end = 20.0
	parent.add_child(skull)
	_add_visual_box(parent, "SkullJaw", position_value + Vector3(0.0, -scale_value * 0.78, 0.0), Vector3(scale_value * 1.18, scale_value * 0.52, scale_value * 0.72), bone_material)
...
func _on_hideout_interaction(action_id: String, _player_node: Node) -> void:
	match action_id:
		...
		"hearth":
			_toggle_hearth()
		"meal":
			hideout_hud.show_event("묽은 뿌리채소 수프가 아직 미지근합니다 · 누군가 살아가는 흔적입니다", 2.4)
		"wash":
			hideout_hud.show_event("차가운 빗물로 피와 먼지를 씻었습니다", 1.8)
		"workbench":
			hideout_hud.show_event("도구가 부족합니다 · 대장장이를 영입하면 수리 작업대가 열립니다", 2.4)
		"storage":
...
func _animate_water() -> void:
	...
	for index in range(water_surfaces.size()):
		...
		if is_instance_valid(water):
			water.position.y = 0.04 + sin(elapsed * 0.72 + float(index) * 1.9) * 0.004
