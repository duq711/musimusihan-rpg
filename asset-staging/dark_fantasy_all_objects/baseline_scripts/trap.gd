...
func _build_visuals() -> void:
	plate_material = _material(Color(0.12, 0.135, 0.13), 0.76, 0.46)
	plate_material.albedo_texture = PITTED_IRON_TEXTURE
	plate_material.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS_ANISOTROPIC
	plate_material.uv1_triplanar = true
	plate_material.uv1_triplanar_sharpness = 32.0
	plate_material.uv1_scale = Vector3.ONE * 2.8
	rune_material = _material(Color(0.25, 0.86, 0.76, 0.94), 0.28, 0.10)
	rune_material.emission_enabled = true
	rune_material.emission = Color(0.08, 0.8, 0.64)
	rune_material.emission_energy_multiplier = 3.3
	...
	var iron := _material(Color(0.16, 0.17, 0.165), 0.43, 0.86)
	iron.albedo_texture = PITTED_IRON_TEXTURE
	iron.texture_filter = BaseMaterial3D.TEXTURE_FILTER_LINEAR_WITH_MIPMAPS_ANISOTROPIC
	iron.uv1_triplanar = true
	iron.uv1_triplanar_sharpness = 32.0
	iron.uv1_scale = Vector3.ONE * 4.0
...
func _trigger(victim: DungeonPlayer, cause: String) -> void:
	...
	if is_instance_valid(victim):
		victim.receive_environment_damage(damage, cause, inflicted_condition)
