...
func _build_visual() -> void:
	...
	match element:
		"ice":
			...
			ice_mesh.size = Vector3(visual_radius * 1.25, visual_radius * 2.8, visual_radius * 1.25)
			core.mesh = ice_mesh
			core.rotation = Vector3(deg_to_rad(90), 0, deg_to_rad(45))
		"stone":
			var stone_mesh := SphereMesh.new()
			stone_mesh.radius = visual_radius
			stone_mesh.height = visual_radius * 1.75
			stone_mesh.radial_segments = 7
			stone_mesh.rings = 4
			core.mesh = stone_mesh
			core.scale = Vector3(1.1, 0.88, 1.0)
		_:
			...
			core.mesh = sphere
			if element == "water":
				core.scale = Vector3(0.78, 0.78, 1.45)
	core.material_override = _projectile_material(_spell_color, element != "stone", 3.2)
	_visual_root.add_child(core)

	if element != "stone":
		var halo := MeshInstance3D.new()
		halo.name = "ElementHalo"
		var halo_mesh := SphereMesh.new()
		halo_mesh.radius = visual_radius * 1.22
		halo_mesh.height = visual_radius * 2.44
		halo_mesh.radial_segments = 10
		halo_mesh.rings = 6
		halo.mesh = halo_mesh
		halo.material_override = _projectile_material(Color(_spell_color, 0.15), true, 1.4)
		_visual_root.add_child(halo)

	var light := OmniLight3D.new()
	light.name = "ElementLight"
	light.light_color = _spell_color
	light.light_energy = 1.1 if element != "stone" else 0.35
