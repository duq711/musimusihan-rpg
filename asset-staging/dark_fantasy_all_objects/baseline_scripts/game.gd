...
func _spawn_loot_chests() -> void:
	...
	_spawn_loot_chest(
		"봉인된 성물 상자",
		Vector3(4.0, 0.0, -11.2),
		[
			{"id": "silver_chalice", "quantity": 1},
			{"id": "rune_fragment", "quantity": 2},
			{"id": "reliquary", "quantity": 1},
			{"id": "healing_draught", "quantity": 1},
...
func _spawn_loot_chest(title_text: String, spawn_position: Vector3, item_stacks: Array, subtitle_text: String) -> void:
...
func _add_static_box(
	...
	if show_visual:
		var mesh_instance := _visual_box(box_size, material)
...
func _add_gothic_arch(center: Vector3, half_width: float, rise: float) -> void:
	...
	for index in range(segment_count):
		var angle := PI * float(index) / float(segment_count - 1)
		var block := _visual_box(Vector3(0.51, 0.31, 0.62), stone_material)
		block.name = "GothicVoussoir"
		block.position = center + Vector3(cos(angle) * half_width, sin(angle) * rise, 0.0)
...
func _add_hanging_chain(chain_position: Vector3, length: float) -> void:
	...
	for index in range(link_count):
		...
		link.mesh = torus
		link.material_override = metal_material
...
func _add_rubble_cluster(cluster_position: Vector3, scale_value: float) -> void:
	...
	for index in range(offsets.size()):
		...
		var rubble := _visual_box(rubble_size, stone_material)
...
func _add_wet_patch(patch_position: Vector3, patch_size: Vector2, yaw_degrees: float) -> void:
	var patch_material := _material(Color(0.015, 0.055, 0.062, 0.48), 0.08, 0.12)
