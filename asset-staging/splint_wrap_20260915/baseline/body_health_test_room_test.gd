...
func _check_splint_trial() -> void:
	...
	_check(room.player.splint_hands.active and room.inventory.count_item("splint") == 2, "trial must use one real splint")
	_check(not ExpeditionSession.condition_affects_part("fracture", "left_arm"), "trial must cure actual arm fracture")
	_f2_menu()
	_check(not room.player.splint_hands.active, "F2 must clear splint presentation")
	room.run_feature("splint_forearm")
	_check(room.player.splint_hands.active and room.inventory.count_item("splint") == 2, "repeat must restock and replay")
	_f2_menu()
