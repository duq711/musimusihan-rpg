...
func set_time(seconds: float) -> void:
	...
	var leave := smoothstep(3.50, DURATION, elapsed)
	var shift := Vector3(0.0, -0.55, 0.12) * leave
	# Escape from Tarkov reference Q9hG7zeQ3xM, 50–56.5s:
	# Start at the established winding pose: three one-second passes,
	# then pull down and return. The former 2.05s preparation is removed.
	...
	var working := 1.0 - smoothstep(WRAP_END, 3.45, elapsed)
	...
	var press := smoothstep(3.05, 3.30, elapsed)
	var finish_pull := smoothstep(3.30, 3.50, elapsed)
