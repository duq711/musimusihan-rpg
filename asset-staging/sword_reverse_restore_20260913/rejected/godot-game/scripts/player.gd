...
const SWORD_WRIST_TURN_SECONDS := 0.14
const SWORD_REVERSE_ENTRY_SECONDS := 0.20
...
func _update_viewmodel(delta: float) -> void:
	...
	if reference_enabled:
		...
		if _uses_direct_sword_entry(phase):
			# Lay either cutting edge before the sweep. The reverse cut also
			# moves across the body to its left-side start without raising the grip.
			var entry_pose := _sword_entry_pose
			var entry_arm := _sword_entry_arm
			if sword_attack_variant in ["right_diagonal", "left_reverse"]:
				if sword_attack_variant == "left_reverse":
					entry_pose = _sword_reverse_entry_pose(state_time / SWORD_REVERSE_ENTRY_SECONDS if phase == "windup" else 1.0)
				else:
					entry_pose = _sword_wrist_turn_pose(state_time / SWORD_WRIST_TURN_SECONDS if phase == "windup" else 1.0)
				entry_arm = _sword_entry_arm.duplicate(true)
				entry_arm["raw_sword"] = entry_pose
				entry_arm["wrist"] = entry_pose * SWORD_LONG_GRIP.REST_WRIST
				entry_arm["exact_sample"] = false
				entry_arm.erase("fitted_pose")
		...
		# The wrist turn already starts from the actual captured pose. A second
		# origin/slerp handoff would move its pinned grip away from the pivot.
		if not (_uses_direct_sword_entry(phase) and phase == "windup" and sword_attack_variant in ["right_diagonal", "left_reverse"]):
...
func _uses_direct_sword_entry(phase: String) -> bool:
	return _sword_direct_entry and not blocking and (phase == "windup" or (phase == "active" and state_time < SWORD_DIRECT_ENTRY_SECONDS))
...
func _direct_sword_entry_progress() -> float:
	...
	return MOTION.smooth_phase(state_time / SWORD_DIRECT_ENTRY_SECONDS)


func _sword_wrist_turn_pose(progress: float) -> Transform3D:
	var metadata := REFERENCE_MOTION.clip_metadata(sword_attack_variant)
	var preparation := REFERENCE_MOTION.sample(sword_attack_variant, float(metadata.timing.windup_seconds))
	var orientation := _sword_entry_pose.basis.get_rotation_quaternion().slerp(preparation.basis.get_rotation_quaternion(), MOTION.smooth_phase(progress))
	var turned := Basis(orientation)
	var grip := _sword_entry_pose * SWORD_LONG_GRIP.GRIP_CENTER
	return Transform3D(turned, grip - turned * SWORD_LONG_GRIP.GRIP_CENTER)


func _sword_reverse_entry_pose(progress: float) -> Transform3D:
	var turned := _sword_wrist_turn_pose(progress * SWORD_REVERSE_ENTRY_SECONDS / SWORD_WRIST_TURN_SECONDS)
	var metadata := REFERENCE_MOTION.clip_metadata("left_reverse")
	var preparation := REFERENCE_MOTION.sample("left_reverse", float(metadata.timing.windup_seconds))
	var ready := REFERENCE_MOTION.sample("idle", 0.0)
	var grip_offset := preparation * SWORD_LONG_GRIP.GRIP_CENTER - ready * SWORD_LONG_GRIP.GRIP_CENTER
	turned.origin += grip_offset * MOTION.smooth_phase(progress)
	return turned
