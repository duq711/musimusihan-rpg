...
func _test_combat_sampling(player: DungeonPlayer, fps: int) -> void:
	for variant: String in DATA.ATTACK_CLIPS:
		...
		_check(player._uses_direct_sword_entry("windup") and _near_pose(player._sword_entry_pose, entry_pose), "Actual start must capture the visible sword before any wrist turn or swing.")
		...
		var reverse_entry := variant == "left_reverse"
		var entry_grip := entry_pose * ARM.GRIP_CENTER
		var preparation := DATA.sample(variant, float(metadata.timing.windup_seconds))
		var authored_idle := DATA.sample("idle", 0.0)
		var reverse_grip_delta := preparation * ARM.GRIP_CENTER - authored_idle * ARM.GRIP_CENTER
		var reverse_projection := 0.0
		var seen_reverse_endpoint := false
		var last_windup_pose := entry_pose
		var last_windup_arm := entry_arm
		var seen_laid_pose := false
		...
		for frame in range(fps * 2):
			...
			if phase == "windup":
				...
				if wrist_turn:
					var actual_grip := player.weapon_pivot.transform * ARM.GRIP_CENTER
					_check(actual_grip.distance_to(entry_grip) < 0.00002, "The basic cut must rotate around the captured grip without moving or raising it at %dHz." % fps)
					if player.state_time >= player.SWORD_WRIST_TURN_SECONDS:
						seen_laid_pose = true
						_check(player.weapon_pivot.basis.is_equal_approx(preparation.basis), "The completed wrist turn must use the existing preparation orientation at %dHz." % fps)
				elif reverse_entry:
					var actual_grip := player.weapon_pivot.transform * ARM.GRIP_CENTER
					var grip_offset := actual_grip - entry_grip
					# This fixture's arbitrary preparation need not move along X.
					# Check its line and endpoint without copying the easing helper.
					if reverse_grip_delta.length_squared() > 0.00000001:
						var projection := grip_offset.dot(reverse_grip_delta) / reverse_grip_delta.length_squared()
						_check(projection >= reverse_projection - 0.00002 and projection <= 1.00002, "Analytical reverse preparation must move monotonically toward its authored grip offset at %dHz." % fps)
						_check(grip_offset.distance_to(reverse_grip_delta * projection) < 0.00002, "Analytical reverse preparation must stay on the authored grip translation line at %dHz." % fps)
						reverse_projection = projection
					if player.state_time >= player.SWORD_WRIST_TURN_SECONDS - 0.0000001:
						seen_laid_pose = true
						_check(player.weapon_pivot.basis.is_equal_approx(preparation.basis), "Reverse wrist orientation must finish before its lateral preparation at %dHz." % fps)
					if player.state_time >= player.SWORD_REVERSE_ENTRY_SECONDS - 0.0000001:
						seen_reverse_endpoint = true
						_check(grip_offset.distance_to(reverse_grip_delta) < 0.00002, "Reverse preparation must finish at the authored grip offset by 200ms and retain it until release at %dHz." % fps)
				else:
					_check(_near_pose(player.weapon_pivot.transform, entry_pose), "Overhead WINDUP must retain the held sword at %dHz/%s." % [fps, variant])
					_check_arm_equal(_actual_arm(player), entry_arm, "held WINDUP arm %dHz/%s" % [fps, variant])
				last_windup_pose = player.weapon_pivot.transform
				last_windup_arm = _actual_arm(player)
			elif phase == "active" and player.state_time < 0.10:
				_check(player._uses_direct_sword_entry(phase), "Only the initial 100ms of ACTIVE may blend from the final preparation into the authored swing.")
				if is_zero_approx(player.state_time):
					seen_active_start = true
					_check(_near_pose(player.weapon_pivot.transform, last_windup_pose), "ACTIVE t=0 must continue the final windup sword without a jump: %dHz/%s" % [fps, variant])
					_check_arm_equal(_actual_arm(player), last_windup_arm, "ACTIVE zero-time arm %dHz/%s" % [fps, variant])
			elif phase in ["active", "recovery"] and not player._reference_pose_handoff_active:
				_check(not player._uses_direct_sword_entry(phase), "From ACTIVE 100ms onward, blade crossings, contact and recovery must use the original authored path.")
		...
		_check(not wrist_turn or seen_laid_pose, "The basic cut must complete its 140ms wrist turn before release at every sampled rate.")
		_check(not reverse_entry or (seen_laid_pose and seen_reverse_endpoint), "Reverse preparation must complete its 140ms wrist turn and 200ms grip movement before release at every sampled rate.")
	...
	for variant: String in DATA.ATTACK_CLIPS:
		...
		_check(not player._uses_direct_sword_entry("active") and _near_pose(player.weapon_pivot.transform, boundary_sample.raw_sword), "Exactly 100ms must restore the original blade pose before the clash/contact window: " + variant)
		_check_geometry(player, "exact 100ms authored entry boundary " + variant)
...
func _test_zero_time_handoffs(player: DungeonPlayer) -> void:
	for source: String in ["run", "air", "land", "attack"]:
		...
		for variant: String in ["right_diagonal", "left_reverse"]:
			_check(bool(player.begin_sword_attack(variant).get("accepted", false)), "Wrist-turn entry must accept an actual prior pose: " + source + "/" + variant)
			player._update_viewmodel(0.0)
			_check(_near_pose(sword, player.weapon_pivot.transform), "Wrist turn at zero time must retain the previous actual sword: " + source + "/" + variant)
			_check_arm_equal(_actual_arm(player), before, "wrist turn t=0 from " + source + "/" + variant)
			_check_geometry(player, "wrist turn zero-time handoff from " + source + "/" + variant)
			player.cancel_sword_attack()
