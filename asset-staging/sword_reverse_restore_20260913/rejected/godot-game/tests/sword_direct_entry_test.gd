...
func _exercise_attack(player: DungeonPlayer, variant: String, hold: float, stowed: bool) -> void:
	...
	var authored_idle := REFERENCE.sample("idle", 0.0)
	var reverse_grip_delta := prep * ARM.GRIP_CENTER - authored_idle * ARM.GRIP_CENTER
	if reverse_entry:
		_check(is_equal_approx(player.SWORD_REVERSE_ENTRY_SECONDS, 0.20), context + ": reverse preparation must finish within the existing 220ms windup")
		_check(reverse_grip_delta.x < -0.001 and absf(reverse_grip_delta.y) < 0.00003 and absf(reverse_grip_delta.z) < 0.00003, context + ": the authored reverse preparation must move the grip left at the same height and depth")
	var elapsed := 0.0
	var spent_count := 0
	...
	var reverse_grip_yz_error := 0.0
	var reverse_rightward_step := 0.0
	var reverse_endpoint_error := 0.0
	var reverse_checkpoint_seen := false
	var reverse_pose: Dictionary = {}
	var reverse_hold_drift := 0.0
	var entry_drift := INF
	var entry_angle := INF
	...
	for frame in 420:
		...
		elif reverse_entry and player.combat_state == DungeonPlayer.CombatState.WINDUP:
			for checkpoint: float in [player.SWORD_WRIST_TURN_SECONDS, player.SWORD_REVERSE_ENTRY_SECONDS]:
		...
		if player.combat_state == DungeonPlayer.CombatState.WINDUP:
			...
			if reverse_entry:
				var grip_offset: Vector3 = actual.grip - start.grip
				reverse_grip_yz_error = maxf(reverse_grip_yz_error, Vector2(grip_offset.y, grip_offset.z).length())
				reverse_rightward_step = maxf(reverse_rightward_step, (actual.grip as Vector3).x - (previous.grip as Vector3).x)
				...
				if player.state_time >= player.SWORD_REVERSE_ENTRY_SECONDS - 0.0000001:
					reverse_checkpoint_seen = reverse_checkpoint_seen or absf(player.state_time - player.SWORD_REVERSE_ENTRY_SECONDS) < 0.000001
					reverse_endpoint_error = maxf(reverse_endpoint_error, grip_offset.distance_to(reverse_grip_delta))
					if reverse_pose.is_empty(): reverse_pose = actual
					reverse_hold_drift = maxf(reverse_hold_drift, _point_drift(reverse_pose, actual))
	...
	if wrist_turn:
		...
		_check(reverse_grip_yz_error < 0.001 and reverse_rightward_step < 0.001, context + ": the actual reverse grip must travel monotonically left without changing height or depth")
		_check(turn_checkpoint_seen and turn_endpoint_error < 0.06, context + ": the reverse wrist must reach its authored preparation orientation at 140ms")
		_check(reverse_checkpoint_seen and not reverse_pose.is_empty() and reverse_endpoint_error < 0.001, context + ": at 200ms the actual grip must complete the authored idle-to-reverse preparation offset from the clicked grip")
		_check(reverse_hold_drift < 0.001, context + ": the completed reverse preparation must remain held until release")
	else:
		_check(windup_drift < 0.001 and windup_angle < 0.06, context + ": overhead must retain its existing held preparation")
	...
	if reverse_entry:
		print("REVERSE PREPARATION ", context, " yz_mm=", reverse_grip_yz_error * 1000.0, " rightward_mm=", reverse_rightward_step * 1000.0, " endpoint_mm=", reverse_endpoint_error * 1000.0, " held_mm=", reverse_hold_drift * 1000.0)
...
func _finish() -> void:
	...
	print("SWORD DIRECT ENTRY %s: pinned basic wrist turn, reverse lateral preparation, retained overhead hold, release/charge/cost clocks, unchanged authored rejoin and expedition preservation; target damage is covered by sword_attack_variants" % ("PASS" if failures.is_empty() else "FAIL"))
