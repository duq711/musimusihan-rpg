...
- 2026-09-13 이전 수정 지시: 우→좌 기본 베기는 날로 베는 궤적과 자연스러운 마무리·복귀로 다듬고, 좌→우 역베기의 검 궤적은 보존하며 손 접촉만 고친다. 이 중 역베기 궤적 보존 지시는 아래 최신 요청으로 대체한다. 원본 손 모델의 엄지·왼손 모양은 이번 모션 변경에서 별도로 수정하지 않는다.
- 2026-09-13 최신 사용자 확정: 현재 `right_diagonal`은 완성된 기본 베기로 승인됐다. 현재 파지 위치에서 손목으로 검을 눕힌 뒤 날부터 우→좌로 베는 이 모션을 기준으로, `left_reverse`도 유사한 궤적을 방향만 반대로 잇는다. 승인된 기본 베기 클립과 검·손 모델 원형은 보존한다. 방패 착용/수납은 같은 공격 연결과 속도를 사용하며 기존 차지·피해·기력·방패 방어 규칙을 유지한다.
- 이번 구현 판단: Mac에서 승인된 기본 베기 활성 구간의 검 길이축·날면·손 궤적을 X좌표 부호를 바꾼 좌우 대칭으로 대응시켜 역베기를 제작하되, 음수 배율 없이 올바른 회전으로 모델 원형을 유지한다. 역베기는 손잡이 높이·깊이를 유지한 채 0.20초 동안 왼쪽 준비 위치로 이동하며, 동시에 첫 0.14초에 손목으로 검을 눕힌다. 활성 첫 0.10초는 같은 시각의 제작 모션으로 연결하며 마지막 회수는 원래 대기 자세로 돌아온다. 기본 베기의 준비·궤적과 내려베기 데이터, 원본 손 모델은 유지한다. 기존 최소 입력 0.22초·차지 판정 기준 0.24초, 활성 구간 기준 타격 0.145초·활성 길이 0.30초는 기존 튜닝값을 유지한다.
- 구현·검증 상태: 이번 역베기 변경의 관련 자동 검사 9종과 실제 Vulkan/embedded 촬영을 완료했다. 승인된 기본 베기는 두 상태 총 182 PNG에서 이전 승인 촬영과 바이트 단위로 일치한다. 기존 `주무장 1번 · 방패 수납`에서 같은 궤적의 양방향 베기를 검·방패와 양손 상태로 비교한다. 요약 기록은 `../../asset-staging/sword_reverse_match_20260913/`, 세부 절차·결과는 [TEST_ROOM.md](../TEST_ROOM.md)에 기록한다. 이전 날 정렬 변경의 검사·촬영과 자연 검격 교차 실패는 `../../asset-staging/sword_edge_alignment_20260913/validation_summary.json`에 보존하며 이번 변경의 검증 결과로 대신하지 않는다.
