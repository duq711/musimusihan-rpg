...
func _update_viewmodel(delta: float) -> void:
	...
	if is_instance_valid(shield_pivot):
		...
		if paired_sword and _sword_attack_had_shield and not blocking and phase in ["windup", "active", "recovery"]:
			# Keep the actual carried shield throughout charge, strike and finish.
			# This changes presentation only; blocking and stamina still own defense.
			shield_pivot.transform = _sword_entry_shield_pose
