...
staging = Path('asset-staging/sword_shield_left_clear_20260914')
before = root / 'shield_hold_reverse_finish_shield'
after = root / 'shield_left_clear'
out = root / 'shield_left_clear_comparison'
...
report = {}
video = out / 'shield_left_clear.mp4'
...
for seq, name in zip(new['sequences'], ['정베기', '역베기', '내려베기']):
    ...
    clear = next(f for f in frames if f['shield_transform']['position'][0] < held[0] - .24)
    ...
    for repeats, speed in [(1, '정상 속도'), (2, '0.5배속')]:
        for i in range(91):
            ...
            for side, title in enumerate(['수정 전 · 방패 고정', '수정 후 · 방패 먼저 왼쪽으로']):
...
result = {'captures': report, 'video': str(video.resolve()), 'fps': 60, 'encoded_frames': encoded, 'duration_seconds': encoded / 60, 'presentation': 'Actual Godot before/after frames, normal then labelled half speed. No pose retouching, mirroring or frame interpolation.'}
