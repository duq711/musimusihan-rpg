...
const OPTIONAL_BASELINE := "../asset-staging/sword_forehand_mirror_reverse_20260913/baseline/godot-game/assets/animations/reference_sword_motion/motion_manifest.json"
...
func _run() -> void:
	...
	var result := {"contract": "same_time_spatial_mirror_of_reverse; shared_right_idle_endpoints", "aesthetic_quality_evaluated": false, "baseline": _test_optional_preservation()}
	if available:
...
func _test_pose_correspondence() -> Dictionary:
	...
	for index in range(181):
		var time := 0.62 + 0.18 * float(index) / 180.0
	...
	_check(grip_error < 0.00003 and axis_error < 0.00002, "Every active forehand sample must mirror the reverse at the SAME time, retaining height/depth and face direction.")
...
func _test_recovery_preservation() -> Dictionary:
	...
	for index in range(1, 621):
		...
		peak_step = maxf(peak_step, (pose * GRIP.GRIP_CENTER).distance_to(previous * GRIP.GRIP_CENTER))
		...
		previous = pose
	_check(peak_step < 0.015, "The full recovery must connect the mirrored follow-through to the right idle continuously.")
	return {"idle_and_arm_endpoints_checked": true, "samples": 621, "max_1ms_grip_step_m": peak_step}
