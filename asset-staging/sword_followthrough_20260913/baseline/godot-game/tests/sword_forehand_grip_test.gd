...
func _test_attack(player: DungeonPlayer, variant: String, hold: float, stowed: bool) -> void:
	...
	if variant == "right_diagonal":
		_check(mirror_samples > 3 and maximum_grip_error < 0.00005 and maximum_axis_error < 0.00005, context + ": every post-entry ACTIVE sample must match the same-phase reverse grip and basis under proper camera-X reflection.")
...
func _check_source_preservation() -> void:
	...
	for clip: String in before_clips:
		if clip == "right_diagonal": continue
		_check(after_clips.has(clip) and after_clips[clip] == before_clips[clip], "Only right_diagonal may change; preserve the original clip exactly: " + clip)
