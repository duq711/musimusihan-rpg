...
func _fit_sword_support() -> void:
	...
	for iteration in 5:
		...
		wrist = contact - orientation * SWORD_SHIELD_ARM.LONG_SWORD_GRIP_CENTER
	sword_support_arm.global_transform = Transform3D(orientation, wrist)
	var actual := sword_support_arm.to_global(SWORD_SHIELD_ARM.LONG_SWORD_GRIP_CENTER)
	_hand_contacts["sword_support"] = {"target": contact, "actual": actual, "error": actual.distance_to(contact)}
	fitted = REFERENCE_ARM.solve(shoulder, hint, wrist)
	var elbow: Vector3 = fitted.elbow
	shoulder = fitted.shoulder
	sword_support_arm.call("set_sword_grip_surface", weapon_pivot.global_transform.affine_inverse() * Transform3D(orientation, target - orientation * SWORD_SHIELD_ARM.LONG_SWORD_GRIP_CENTER))
