...
const GRIP_CENTER := Vector3(0.0, -0.019, -0.0825)
# Support-only palm fit; the shared shield hand and its source scale stay intact.
const LONG_SWORD_GRIP_CENTER := Vector3(0.0, -0.011, -0.074)
const DISTAL_LENGTH := {"little": 0.0176, "ring": 0.02104, "middle": 0.022, "index": 0.02024, "thumb": 0.025}
...
func _shield_grip_contact(digit: String, along_grip: float, tension: float) -> Dictionary:
	...
	var angle := deg_to_rad(-4.0)
	var v := clampf((point.x + 0.18) * sin(angle) + (point.y - 0.015) * cos(angle) + (0.003 if is_thumb else 0.0), -0.049, 0.049)
	# All four shortened fingers wrap the same quadrant of the real grip.
	# Contact angles lie in face interiors, never on ambiguous 18-gon vertices.
	# Their distal pads form one row; the middle finger never crosses to the
	# opposite side or disappears under a separate glove contact.
	var phi := deg_to_rad(260.0 if is_thumb else 0.0 if digit == "index" else 40.0)
	...
	elif digit == "little": clearance += 0.0015
...
func _set_combat_grip(role: String, tension: float, thumb_amount: float) -> void:
	...
	for digit: String in DIGITS:
		...
		if role == "sword" and long_grip_surface and digit != "thumb" and strength >= 0.75:
			_solve_long_sword_digit(digit, strength)
		if role == "shield":
	...
	if role == "shield":
		# Refit the common finger row after thumb opposition has deformed the
		# shared palm weights. Each target remains the same physical surface.
		for digit: String in ["little", "ring", "middle", "index"]:
			_solve_shield_power_digit(digit, profile[digit] * amount, amount)
...
func _grip_pad_target(role: String, digit: String, along_grip: float, tension: float) -> Vector3:
	if digit == "thumb":
		...
		if role == "sword": return _project_sword_surface(GRIP_CENTER + Vector3(_geometry_side * 0.054, -0.019625835, 0.001716968), tension, 0.0004)
...
func _project_sword_surface(nominal: Vector3, tension: float, pad_thickness := 0.0) -> Vector3:
	...
	point.y = clampf(point.y, -0.310 if long_grip_surface else -0.1634, -0.0332)
	...
	var source_y := -0.026 + (point.y + (0.144 if long_grip_surface and point.y < -0.17 else 0.0) + 0.026) / 0.60
	...
	surface += normal * ((lerpf(0.0008, 0.0003, tension) if long_grip_surface else lerpf(0.0020, 0.0012, tension)) + pad_thickness)
...
func _solve_shield_power_digit(digit: String, seed: Vector3, strength: float) -> void:
	...
	var angles := seed
	# A position-only three-joint solve can unfold DIP while meeting the
	# target. Keep the terminal joint curled and solve MCP/PIP around it so
	# all four fingers present their broad pads in the same closed grip.
	var distal_curl := lower.z
	if digit == "little": distal_curl = -0.95
	if digit in ["ring", "middle"]: distal_curl = -1.10
	angles.z = distal_curl * closure
	var lateral := 0.0
	var root_bone := skeleton.find_bone(digit + "0")
	var contact := _shield_grip_contact(digit, skeleton.get_bone_global_pose(root_bone).origin.x, grip_amount)
	var target := _digit_pad(digit).lerp(contact.target, closure)
	_apply_digit_angles(digit, angles, lateral)
	if strength > 0.0:
		for iteration in 35:
			for joint in [1, 0]:
				var frame := skeleton.get_bone_global_pose(skeleton.find_bone(digit + str(joint)))
				var correction := _hinge_correction(frame.origin, frame.basis.x.normalized(), _digit_pad(digit), target)
				angles[joint] = clampf(angles[joint] + clampf(correction, -0.22, 0.22), lower[joint], 0.0)
				_apply_digit_angles(digit, angles, lateral)
			var reference := skeleton.get_bone_global_pose(skeleton.get_bone_parent(root_bone)) * skeleton.get_bone_rest(root_bone)
			var correction := _hinge_correction(reference.origin, reference.basis.z.normalized(), _digit_pad(digit), target)
			lateral = clampf(lateral + clampf(correction, -0.10, 0.10), -0.22, 0.22)
			_apply_digit_angles(digit, angles, lateral)
	_digit_contacts[digit] = {"target": target, "angles": angles, "lateral": lateral, "opposition": 0.0, "lower_limits": lower, "upper_limits": Vector3.ZERO, "lateral_limit": 0.22, "patch": digit, "contact_kind": "strap", "surface_kind": "padded_18gon", "surface": contact.surface, "inward": contact.inward, "uv": contact.uv}
...
func _solve_digit_contact(digit: String, target: Vector3, seed: Vector3, strength: float) -> void:
	...
	var angles := seed
	var terminal_thumb := is_thumb and _grip_role == "sword"
	# A stable IP fold presents the actual thumb pad to the oval hilt. With
	# the shorter thumb, a position-only solve can leave IP straight and put
	# the side of the digit on the leather even when its endpoint is exact.
	if terminal_thumb: angles.z = -1.25 * smoothstep(0.0, 0.75, strength)
	var lateral := 0.0
	_apply_digit_angles(digit, angles, lateral, opposition)
	if strength > 0.0:
		for iteration in (30 if terminal_thumb else 20):
			for joint in ([1, 0] if terminal_thumb else [2, 1, 0]):
				var bone := skeleton.find_bone(digit + str(joint))
...
func _solve_long_sword_digit(digit: String, strength: float) -> void:
	# Distal-only CCD can place the fingertip while its middle phalanx floats
	# above the extended hilt. Let both actual skin pads slide on that surface.
	var contact: Dictionary = _digit_contacts[digit]
	var seed: Vector3 = contact.angles
	var angles := seed
	var lateral: float = contact.lateral
	var patches: Array[String] = [digit, digit + "_middle"]
	if digit == "middle": patches.append("middle_proximal")
	for iteration in 28:
		for joint in [2, 1, 0]:
			var bone := skeleton.find_bone(digit + str(joint))
			var frame := skeleton.get_bone_global_pose(bone)
			var correction := 0.0
			var weight := 0.0
			for patch: String in patches:
				if joint == 2 and patch != digit: continue
				if joint == 1 and patch == "middle_proximal": continue
				var pad := _digit_pad(patch)
				var target := _project_sword_surface(pad, strength)
				correction += _hinge_correction(frame.origin, frame.basis.x.normalized(), pad, target)
				weight += 1.0
			angles[joint] = clampf(angles[joint] + clampf(correction / weight, -0.16, 0.16), contact.lower_limits[joint], contact.upper_limits[joint])
			_apply_digit_angles(digit, angles, lateral)
	angles = seed.lerp(angles, smoothstep(0.75, 0.89, strength))
	_apply_digit_angles(digit, angles, lateral)
	contact.angles = angles
	contact.target = _project_sword_surface(_digit_pad(digit), strength)
	contact.surface_kind = "extended_hilt_skin_patches"
