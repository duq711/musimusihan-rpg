...
func _run() -> void:
	...
	sandbox.begin()
	for sequence_id: String in (["shield_stow", "run", "jump", "right_diagonal", "left_reverse", "overhead"] if two_hand_review else ["shield_stow"] if shield_stow_only else ["draw"] if draw_only else SEQUENCE_IDS):
	...
	var manifest := {
		...
		"qa_scope": "two_hand_sword" if two_hand_review else "primary_shield_stow" if shield_stow_only else "overhead_motion_on_existing_arm" if motion_only else ("overhead_integration_and_existing_motion_regressions" if partial_overhead else ("equipment_draw" if draw_only else "full_motion_delivery")),
		...
		"contact_sheet": {"image": "contact_sheet.png", "columns": 4, "tile_size": [IMAGE_SIZE.x, IMAGE_SIZE.y], "row_major_pose_ids": (["shield_stow", "run", "takeoff", "air", "land", "right_diagonal", "left_reverse", "overhead"] if two_hand_review else ["shield_stow"] if shield_stow_only else ["draw"] if draw_only else CONTACT_IDS)},
		"failures": failures,
	}
...
func _capture_sequence(sequence_id: String) -> void:
	...
	if partial_overhead and not motion_only:
		...
		if not bool(sleeve.get("ok", false)) or int(sleeve.get("bone_count", 0)) != 7 or int(sleeve.get("mesh_count", 0)) != 3:
			...
			await process_frame
			return
	if two_hand_review and sequence_id != "shield_stow":
	...
	for frame in range(frame_count):
		...
		if (shield_stow_only or two_hand_review) and float(snapshot.sword_support_progress) >= 1.0:
...
func _save_contact_sheet() -> void:
	...
	var ids: Array = ["shield_stow", "run", "takeoff", "air", "land", "right_diagonal", "left_reverse", "overhead"] if two_hand_review else ["shield_stow"] if shield_stow_only else ["draw"] if draw_only else CONTACT_IDS
