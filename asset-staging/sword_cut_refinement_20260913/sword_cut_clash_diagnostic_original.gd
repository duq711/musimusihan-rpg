...
func _run() -> void:
	...
	output["samples"] = {}
	for version in ["current_v3", "baseline"]:
		DATA._clips = current if version == "current_v3" else before.clips
		...
		enemy.global_position = Vector3(.1, floor_height, -1.7)
		enemy.rotation.y = PI
		...
		await process_frame
	DATA._clips = current
	var path := ProjectSettings.globalize_path("res://").path_join("../asset-staging/sword_cut_refinement_20260913/clash_diagnostic_v3.json")
