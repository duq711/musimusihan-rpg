...
func run():
	...
	output=ProjectSettings.globalize_path("res://artifacts/visual_qa/wooden_torch_pocket_motion")
	...
	p.configure_safe_zone(true)
	p.set_torch_enabled(false);p._update_viewmodel(2.0)
	p.set_torch_enabled(true)
	var elapsed := 0.0
	for sample in [0.20, 0.40, 0.60, 0.80, 1.05]:
		p._update_viewmodel(sample - elapsed);p._update_torch(sample - elapsed)
		elapsed = sample
		p.viewmodel_renderer.sync_view()
		await capture(vp,"draw_%03d" % roundi(sample * 100))
	p._update_viewmodel(2.0);p.viewmodel_renderer.sync_view()
	await capture(vp,"game_full")
	for direction in [-1.0, 1.0, 0.0]:
		p.velocity = p.camera.global_basis.x * p.SPRINT_SPEED * direction
		for i in 90:
			p._update_viewmodel(1.0/60.0);p._update_torch(1.0/60.0)
		p.viewmodel_renderer.sync_view()
		await capture(vp, "move_left" if direction < 0 else ("move_right" if direction > 0 else "stopped"))
	p.set_torch_enabled(false)
	elapsed = 0.0
	for sample in [0.15, 0.30, 0.45, 0.65, 0.90]:
		p._update_viewmodel(sample - elapsed);p._update_torch(sample - elapsed)
		elapsed = sample
		p.viewmodel_renderer.sync_view()
		await capture(vp,"stow_%03d" % roundi(sample * 100))
	p._update_viewmodel(2.0);p.viewmodel_renderer.sync_view();await capture(vp,"extinguished")
	p.set_torch_enabled(true);p._update_viewmodel(2.0);p.viewmodel_renderer.sync_view();await capture(vp,"relit")
	...
	camera.cull_mask=1<<19;camera.fov=58;camera.near=.01
	var center:Vector3=p.torch_pivot.to_global(Vector3(0,.50,0))
	var key:=DirectionalLight3D.new();key.layers=1<<19;key.light_cull_mask=1<<19;key.light_energy=.7;p.get_parent().add_child(key)
	for side in [1.0,-1.0]:
		camera.global_position=center+p.camera.global_basis.z*1.65*side
