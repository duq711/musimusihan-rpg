...
lo=np.r_[lo,[-.3]*4];hi=np.r_[hi,[.3]*4]
qstart=np.r_[np.radians([-12,-15,-10]+[-20,-25,-15]*4+[20]),.085,-.045,.12,0,0,0,0]
...
def residual(q):
 ...
 return np.r_[collision,np.array(contacts)*100,np.array(midcontact)*55,np.maximum(gaps-.022,0)*50,reg,q[19:]*.006,q[18]*.001]
runs=[]
for seed in range(1):
 start=qstart.copy();start[:15]+=np.random.default_rng(seed+17).normal(0,.17,15);start[15]=[.3,-.3,.6,-.6][seed];start[16]=[.08,.09,.065,.06][seed]
 start=np.clip(np.array(json.loads((s/'audit/fitted_pose_04.json').read_text())['parameters']),lo+1e-5,hi-1e-5)
 rr=least_squares(residual,start,bounds=(lo,hi),diff_step=1e-4,max_nfev=80,ftol=1e-6,xtol=1e-6,gtol=1e-6)
...
result={'parameters':q.tolist(),'degrees':dict((d,np.degrees(q[di*3:di*3+3]).tolist()) for di,d in enumerate(digits)),'thumb_opposition_degrees':float(np.degrees(q[15])),'shaft_in_hand':{'center':[0,float(q[16]),float(q[17])],'axis':[float(np.cos(q[18])),float(np.sin(q[18])),0]},'full_skin_min_clearance_m':float(dist.min()),'penetrating_vertices':int(np.count_nonzero(dist<-.0005)),'contacts':dict((l,{'minimum_m':float(dist[c].min()),'closest_patch_mean_m':float(np.sort(dist[c])[:max(3,int(len(c)*.06))].mean())}) for l,c in zip(labels,contact)),'root_adduction_radians':dict(zip(digits[1:],q[19:].tolist())),'cost':res.cost,'evaluations':res.nfev,'runtime_delta_reproduction_error':float(np.max(np.abs(T-x['runtime_deltas']))),'handle_rings':ring}
(s/'audit/fitted_pose_04.json').write_text(json.dumps(result,indent=2));np.save(s/'audit/fitted_vertices_04.npy',p)
