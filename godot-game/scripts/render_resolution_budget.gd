...
static func render_size_for(target: Viewport) -> Vector2i:
	if target is SubViewport:
		return (target as SubViewport).size
	# Canvas-items stretching changes the visible rect into logical UI units.
	# Undo that scale to budget the actual world render pixels; viewport-mode
	# stretching already has an unscaled internal visible rect.
	var pixels := target.get_visible_rect().size * target.get_stretch_transform().get_scale()
	return Vector2i(pixels.round())
