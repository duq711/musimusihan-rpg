...
const DETAILED_LEFT_PATH := "res://assets/3d/player/fp_arms/left.scn"
const DETAILED_RIGHT_PATH := "res://assets/3d/player/fp_arms/right.scn"
...
func setup(side: int) -> void:
	...
	set_meta("source_side", "L" if _side < 0 else "R")
...
func set_visual_profile(profile: String) -> bool:
	...
	if not is_instance_valid(selected):
		...
		var adapter_script := load("res://scripts/greybox_arm_visual.gd" if profile == "greybox" else "res://scripts/supplied_fp_arm.gd") as Script
		...
		if not bool(candidate.call("setup", _side, source_profile, bool(get_meta("torch_source", false)))):
			candidate.free()
			return false
		...
		candidate.position = CONTACT_OFFSET if profile == "greybox" else Vector3.ZERO
	...
	set_meta("continuous_skin", profile != "greybox")
