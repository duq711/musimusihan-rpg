...
func _create_equipment_lights() -> void:
	...
	for carried_light: Light3D in source_camera.find_children("*", "Light3D", true, false):
		sources.append(carried_light)
	# Sun/room key lights are part of the real scene too. Without their light,
	# weapons become black when the player puts out a torch in a lit space.
	for world_light: DirectionalLight3D in source_camera.get_tree().root.find_children("*", "DirectionalLight3D", true, false):
		if world_light.get_world_3d() == source_camera.get_world_3d() and (world_light.layers & _original_cull_mask) != 0 and not world_light.get_meta("equipment_mirror", false):
			sources.append(world_light)
	for source: Light3D in sources:
		var light := source.duplicate(0) as Light3D
		light.name = "Equipment" + source.name
		light.set_meta("equipment_mirror", true)
		...
		if light is DirectionalLight3D:
			(light as DirectionalLight3D).sky_mode = DirectionalLight3D.SKY_MODE_LIGHT_ONLY
...
func _sync_environment() -> void:
	...
	# Close skin, glove seams and enarmes need their real depth contact shade.
	equipment_environment.ssao_enabled = true
	equipment_environment.ssao_radius = 0.12
	equipment_environment.ssao_intensity = 2.0
	equipment_environment.ssao_power = 1.4
	if equipment_environment.sky == null:
		# A low energy diffuse-room reflection derived from the actual ambient
		# light gives steel a response even in scenes with a color background.
		var reflection := ProceduralSkyMaterial.new()
		var ambient := equipment_environment.ambient_light_color
		reflection.sky_top_color = ambient * 0.40
		reflection.sky_horizon_color = ambient * 0.85
		reflection.ground_bottom_color = ambient * 0.45
		reflection.ground_horizon_color = ambient * 0.60
		reflection.sky_energy_multiplier = equipment_environment.ambient_light_energy
		reflection.ground_energy_multiplier = equipment_environment.ambient_light_energy
		var sky := Sky.new()
		sky.sky_material = reflection
		equipment_environment.sky = sky
		equipment_environment.reflected_light_source = Environment.REFLECTION_SOURCE_SKY
