...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo and not event.alt_pressed and not event.ctrl_pressed and not event.meta_pressed:
		...
		if key == KEY_B and game_mode in [GameMode.RUNNING, GameMode.INVENTORY]:
			...
			return
		if key == KEY_L and game_mode == GameMode.RUNNING and not get_tree().paused:
			if not player.is_item_use_active(): player.toggle_torch()
			get_viewport().set_input_as_handled()
			return
...
func _spawn_enemy(enemy_name: String, spawn_position: Vector3, hp: float, damage: float, speed: float, tint: Color, ailment_id := "", archetype := "warden") -> void:
	...
	if archetype == "creep" and preload("res://scripts/creep_enemy.gd").is_available():
		enemy.free()
		enemy = preload("res://scripts/creep_enemy.gd").new()
