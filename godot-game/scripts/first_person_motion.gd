...
static func sword(phase: String, elapsed: float, charge: float = 0.0, guard := false, shield := true, variant := "right_diagonal") -> Transform3D:
	...
	if phase == "ready" and not guard:
		return CHOREOGRAPHY.ready()
	if not guard and phase in ["ready", "windup", "active", "recovery"] and REFERENCE.has_track(variant):
		return REFERENCE.retimed_attack(phase, elapsed, charge, variant, false)
...
static func shield(blocking: bool, impact_remaining: float, phase: String) -> Transform3D:
	...
	if blocking:
		result = CHOREOGRAPHY.shield_guard()
	...
	if impact_remaining > 0.0:
		...
		var braced := CHOREOGRAPHY.shield_guard()
		braced.origin += Vector3(0.0, -0.025, 0.055)
