...
static func build_object(id: String, variant_index: int = 0) -> Node3D:
	...
	match str(entry.factory_type):
		...
		"imported_asset": object = _imported_asset(entry)
		"sword_shield": object = _sword_shield(str(entry.id))
...
static func _imported_asset(entry: Dictionary) -> Node3D:
	# Retain the authored UVs and PBR maps instead of the dungeon floor material.
	var scene := _scene(str(entry.asset_path))
	var mesh_name := str(entry.get("mesh_node", ""))
	if mesh_name.is_empty():
		return scene
	var part := scene.find_child(mesh_name, true, false) as Node3D
	if part == null:
		scene.free()
		push_error("Missing authored asset part: " + mesh_name)
		return null
...
static func _equipment(id: String) -> Node3D:
	...
	match id:
		...
		"iron_cage_torch":
			...
			selected = ["WoodenTorchVisual", "Flame"]
	return _take_selected(helper, selected)


static func _sword_shield(id: String) -> Node3D:
	# The gallery uses the same imported mesh resources and material preparation
	# as the equipped viewmodel, without constructing hands or entering gameplay.
	var packed: PackedScene = DungeonPlayer.SWORD_SCENE if id == "rusted_longsword" else DungeonPlayer.SHIELD_SCENE
	var model := DungeonPlayer.SWORD_LONG_GRIP.create_sword() if id == "rusted_longsword" else packed.instantiate() as Node3D
	if id != "rusted_longsword":
		var helper := DungeonPlayer.new()
		helper._prepare_sword_shield_materials(model)
		helper.free()
	if id == "weathered_round_shield":
		# Match production: the boss faces -Z, rear enarmes face +Z.
		model.rotation.y = PI
	model.set_meta("production_scene", packed.resource_path)
	return model
