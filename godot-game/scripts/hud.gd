...
var item_use_player: Node
var _item_use_crosshair_was_visible := true
var _item_use_was_visible := false
var item_use_progress: Control

var combat_panel: Control
var combat_enabled := false
var legacy_panels: Array[Control] = []
...
func _ready() -> void:
	...
	_build_interface()
	item_use_progress = preload("res://scripts/item_use_progress.gd").new()
	item_use_progress.name = "ItemUseProgress"
	add_child(item_use_progress)
...
func _process(_delta: float) -> void:
	...
	if is_instance_valid(item_use_player): update_item_use(item_use_player.get_item_use_snapshot())
	_refresh_archery_reticle_visibility()
	if is_instance_valid(combat_panel):
		combat_panel.visible = combat_enabled and not get_tree().paused and not overlay.visible
		if combat_enabled:
			for panel in legacy_panels: panel.hide()
...
func _build_interface() -> void:
	...
	health_bar.max_value = BODY_HEALTH.MAX_HEALTH
	health_bar.value = BODY_HEALTH.MAX_HEALTH
	top_left.add_child(health_bar)
	health_label = _label("체력 %d / %d" % [roundi(BODY_HEALTH.MAX_HEALTH), roundi(BODY_HEALTH.MAX_HEALTH)], 13, Color(0.98, 0.91, 0.83))
	...
	_build_overlay(root)
	legacy_panels.assign([top_left, top_right, controls, magic_panel, archery_panel, flail_panel])
...
func _build_magic_panel(root: Control) -> void:
	...
	magic_slots_label = _label("[1] —   [2] —   [3] —   [4] —   [5] —   [6] —", 12, Color(0.6, 0.69, 0.66))
...
func update_conditions(conditions: Dictionary, drain_multiplier := 1.0) -> void:
	...
	for condition_id in ["bleeding", "fracture", "curse", "paralysis", "poison"]:
...
func _archery_reticle_should_show(mouse_captured: bool) -> bool:
	return (
		...
		and not (is_instance_valid(item_use_player) and item_use_player.is_item_use_active())
		and mouse_captured
...
func enable_combat_interface(actor: DungeonPlayer, inventory: ExpeditionInventory, provider: Callable) -> void:
	if not is_instance_valid(combat_panel):
		combat_panel = preload("res://scripts/dungeon_combat_hud.gd").new()
		combat_panel.name = "DungeonCombatHUD"
		get_node("HUDRoot").add_child(combat_panel)
	combat_panel.setup(actor, inventory, provider)
	set_combat_interface_enabled(true)


func set_combat_interface_enabled(enabled: bool) -> void:
	combat_enabled = enabled
	if is_instance_valid(combat_panel): combat_panel.visible = enabled
	crosshair.text = "" if enabled else "·"
	for index in legacy_panels.size():
		legacy_panels[index].visible = not enabled and index < 3


func update_item_use(state: Dictionary) -> void:
	if is_instance_valid(item_use_progress): item_use_progress.update_state(state)
	if is_instance_valid(crosshair):
		if not state.is_empty() and not _item_use_was_visible:
			_item_use_crosshair_was_visible = crosshair.visible
			crosshair.hide()
		elif state.is_empty() and _item_use_was_visible:
			crosshair.visible = _item_use_crosshair_was_visible
	_item_use_was_visible = not state.is_empty()
