...
func set_time(seconds: float) -> void:
	...
	# Counter-move the receiving arm while the working hand keeps its path.
	# Both endpoints move together, preserving the seated board and wrap.
	var dodge_weight := smoothstep(SUPPORT_END, SUPPORT_END + 0.30, elapsed) * (1.0 - smoothstep(WIND_END, WIND_END + 0.40, elapsed))
	var reference_axis := Vector3(-0.38, 0.05, 0.24).normalized()
	var reference_normal := (Vector3(0, 0.65, 1) - reference_axis * reference_axis.dot(Vector3(0, 0.65, 1))).normalized()
	var reference_binormal := reference_normal.cross(reference_axis).normalized()
	var dodge := (reference_normal * cos(winding) + reference_binormal * sin(winding)) * 0.065 * dodge_weight
	dodge += Vector3(-0.035, 0, 0) * dodge_weight
	sway += dodge
	...
	var roll_position := contact + radial * 0.18 - dodge
