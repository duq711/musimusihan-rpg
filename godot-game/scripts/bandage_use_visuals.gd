...
const STOW_END := 0.40
const HANDS_START := 0.80
const HANDS_END := 4.95
const DURATION := 5.35
const WRAP_START := 0.0
const WRAP_END := 3.0
const TEAR_START := 3.0
const TEAR_BREAK := 3.18
const TEAR_END := 3.38
const TURNS := 3.0
const WIDTH := 0.080
...
func setup() -> void:
	...
	add_child(left_arm)
	left_arm.setup(-1)
	_forearm_mesh = left_arm.arm_meshes[0]
	_measure_sleeve()
	right_arm = ARM.new()
...
func begin(equipment: Array = []) -> void:
	# Load the supplied rigs only on the first eligible use.
	if not is_instance_valid(left_arm): setup()
	clear()
	for item: Node3D in equipment:
		if is_instance_valid(item):
			_carried.append({"node": item, "local": item.transform, "camera": get_parent().global_transform.affine_inverse() * item.global_transform})
	active = true
	elapsed = 0.0
	set_time(0.0)
...
func clear() -> void:
	for entry: Dictionary in _carried:
		if is_instance_valid(entry.node): entry.node.transform = entry.local
	_carried.clear()
	active = false
	elapsed = 0.0
	winding = 0.0
...
func equipment_hidden() -> bool:
	return active and elapsed >= STOW_END and elapsed < HANDS_END

func _position_equipment() -> void:
	var tucked := smoothstep(0.0, STOW_END, elapsed)
	if elapsed >= HANDS_END: tucked = 1.0 - smoothstep(HANDS_END, DURATION, elapsed)
	# Move the complete held rig together so both hands keep their grip.
	var offset := Transform3D(Basis(Vector3.RIGHT, -0.55 * tucked), Vector3(0.02, -1.10, 0.12) * tucked)
	for entry: Dictionary in _carried:
		if is_instance_valid(entry.node):
			entry.node.global_transform = get_parent().global_transform * offset * (entry.camera as Transform3D)

func set_time(seconds: float) -> void:
	elapsed = clampf(seconds, 0.0, DURATION)
	_position_equipment()
	visible = elapsed >= STOW_END and elapsed < HANDS_END
	var motion_time := maxf(0.0, elapsed - HANDS_START)
	var lift := smoothstep(STOW_END, HANDS_START, elapsed)
	var leave := smoothstep(TEAR_END, 4.15, motion_time)
	var shift := Vector3(0.0, -0.55, 0.12) * (1.0 - lift + leave)
	# Keep the receiving arm low; use only a small settling lift for the tear.
	shift += Vector3(0.0, 0.03, 0.0) * smoothstep(TEAR_START, 3.14, motion_time) * (1.0 - leave)
	# Escape from Tarkov reference Q9hG7zeQ3xM, 50–56.5s:
	# Lift into the existing winding pose without unfolding the bandage.
	# Keep the three winding passes; flow straight into tension, tear and withdrawal.
	var wrap_time := clampf((motion_time - WRAP_START) / (WRAP_END - WRAP_START), 0.0, 1.0)
	winding = wrap_time * TURNS * TAU
	var beat := sin(winding * 0.5) * sin(winding * 0.5)
	var working := 1.0 - smoothstep(WRAP_END, 3.45, motion_time)
	...
	left_arm.set_grip(1.0, 0.40)
	left_arm.fit_arm(to_global(Vector3(-0.37, -0.34, -0.16)), to_global(_elbow))
	...
	var roll_position := contact + radial * 0.085
	...
	var right_basis := _hand_basis(arm_direction, Vector3(0.0, 0.45, 1.0))
	# Build tension immediately after the last pass, then a short outward snap.
	# The receiving arm anchors the dressing while the roll hand pulls away.
	var tension := smoothstep(TEAR_START, TEAR_BREAK, motion_time)
	var recoil := smoothstep(TEAR_BREAK, TEAR_END, motion_time)
	var pull_direction := Vector3(0.72, 0.68, 0.10).normalized()
	roll_position += pull_direction * (0.075 * tension + 0.14 * recoil)
	roll_position += Vector3(0.0, 0.035, 0.08) * tension
	right_basis = right_basis.rotated(right_basis.z, -0.28 * tension - 0.12 * recoil)
	torn = motion_time >= TEAR_BREAK
	right_arm.transform = Transform3D(right_basis, roll_position - right_basis * GRIP)
	right_arm.set_grip(0.96, 0.70)
	right_arm.set_finger_curl(3, lerpf(-1.0, -1.15, tension), lerpf(-1.35, -1.45, tension))
	...
	var strip_end := roll_position - radial * 0.022
	# After separation, keep a short torn tail with the roll instead of a
	# stretched bridge back to the arm. Deposited cloth stays on the sleeve.
	var strip_start := strip_end - pull_direction * 0.045 if torn else contact
	_build_strip(strip_start, strip_end, -right_basis.x)
...
func _measure_sleeve() -> void:
	...
	for index in 10:
		...
		# Axis extrema alone do not enclose the folds of the supplied sleeve:
		# its diagonal vertices can still lie outside that ellipse.
		var envelope := 1.0
		for vertex in vertices:
			if absf(vertex.z - z) < 0.026:
				envelope = maxf(envelope, Vector2(vertex.x / radius.x, vertex.y / radius.y).length())
		radius *= envelope
