...
func _build_interface() -> void:
	...
	# Keep bound status controls available for game updates, but leave the
	# exploration view clear of permanent cards and control instructions.
...
func _build_center_feedback(root: Control) -> void:
	crosshair = _hud_label("·", CROSSHAIR_FONT_SIZE, Color(0.78, 0.82, 0.75, 0.78))
	crosshair.name = "HideoutCrosshair"
	crosshair.anchor_left = 0.5
	crosshair.anchor_top = 0.5
	crosshair.anchor_right = 0.5
	crosshair.anchor_bottom = 0.5
	crosshair.offset_left = -36.0
	crosshair.offset_top = -42.0
	crosshair.offset_right = 36.0
	crosshair.offset_bottom = 42.0
