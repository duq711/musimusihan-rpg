...
# Eye camera and lips are separate anchors. Keep the opening below the view,
# close to the lower face; targeting the optical centre reads as nose drinking.
const DRINK_MOUTH := Vector3(0.0, -0.15, -0.035)
var bottle: Node3D
...
func set_time(seconds: float) -> void:
	...
	# The neck stays below the camera while the body fills the lower centre.
	# Roll the wrist with the flask so the fingers wrap across its visible side.
	var tilt := (2.18 + 0.16 * drink + 0.025 * gulp) * sip
	var bottle_basis := Basis(Vector3.RIGHT, tilt) * Basis(Vector3.UP, 0.35 * sip)
	# Open at lower chest height. A little more depth keeps the flask in view
	# while both connected hands settle below the previous raised-arm pose.
	var position_target := Vector3(0.06, -0.18, -0.30).lerp(DRINK_MOUTH - bottle_basis * (BOTTLE_LIP * BOTTLE_SIZE * MODEL_SCALE), sip)
	...
	right_arm.fit_arm(to_global(Vector3(0.60, -0.60, 0.02)), right_arm._active_visual.global_position + global_basis * hand_basis.z * lerpf(0.25, 0.40, sip))
...
func _animate_cap(bottle_basis: Basis) -> void:
	...
	cap_lift = 0.0015 * first + 0.002 * second + 0.040 * smoothstep(2.17, 2.42, elapsed)
	...
	# Reach in close to the stopper's height instead of hovering above it.
	contact += Vector3(-0.22, 0.03, 0.04) * approach
	...
	# Keep the elbow low beside the torso while the wrist turns the stopper.
	# Swinging the elbow with the hand's yaw brought the sleeve into the lens.
	var elbow := to_global(Vector3(-0.58, -0.24, -0.02))
