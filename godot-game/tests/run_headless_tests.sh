...
for test_path in "${test_paths[@]}"; do
	...
	current_test_timeout_seconds="$test_timeout_seconds"
	# These suites repeatedly construct real maps/players and decode their
	# assets. Measured runs exceed 180s, including camp and bow fixtures.
	# An explicit user override still controls every selected suite.
	if [[ -z "${GODOT_TEST_TIMEOUT_SECONDS:-}" ]]; then
		case "$test_name" in
			bow_draw_stamina_test.gd)
				# Measured at 594s with concurrent background validation.
				current_test_timeout_seconds=900
				;;
			loot_spawn_test.gd|cooking_system_test.gd|cave_flow_test.gd|camp_system_test.gd|bow_accuracy_test.gd)
				current_test_timeout_seconds=600
				;;
		esac
	fi
	...
	while kill -0 "$current_test_pid" 2>/dev/null; do
		if (( SECONDS - test_started_at >= current_test_timeout_seconds )); then
			timed_out=1
			print -u2 -r -- "TIMEOUT after ${current_test_timeout_seconds}s: $test_name"
			stop_current_test
			break
		fi
		/bin/sleep 0.25
