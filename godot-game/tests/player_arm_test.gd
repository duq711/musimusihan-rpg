...
func _test_shared_geometry_and_joints() -> void:
	...
	for side in [-1, 1]:
		var path := "res://assets/3d/player/fp_arms/%s.scn" % ("left" if side < 0 else "right")
		...
		for part in parts:
			var rendered_source: Mesh = part.mesh
		...
		_check(skeleton != null and skeleton.get_bone_count() == 20, "default supplied hands must use the actual wrist and fifteen finger bones")
		if skeleton == null:
			source.free()
			continue
		for digit: String in ["thumb", "index", "middle", "ring", "little"]:
			var parent := skeleton.find_bone("palm" if digit in ["little","ring"] else "wrist")
			for joint in 3:
		...
		var arm_before := _joint_transforms(rig)
		var hand_before := skeleton.get_bone_global_pose(skeleton.find_bone("wrist"))
		...
		_check(_joint_transforms(rig) != arm_before, "fitting an arm must move its actual sleeve and forearm meshes")
		_check(skeleton.get_bone_global_pose(skeleton.find_bone("wrist")).is_equal_approx(hand_before) and rig.global_transform.is_equal_approx(wrist_before) and stage.global_transform.is_equal_approx(parent_before), "shoulder and elbow fitting must preserve supplied skin, wrist and carried weapon")
...
func _test_actual_preview_poses() -> void:
	...
	for pose_id: String in PREVIEW.POSE_IDS:
		...
		elif pose_id in ["sword_active", "bow_draw"]:
		...
		if pose_id in ["idle", "sword_windup", "sword_active", "shield_guard"]:
			_check("fp_arms/right.scn" in str(player.weapon_arm.get_meta("source_model", "")), "sword must use the new supplied right arm")
			_check(player.weapon_arm.get_snapshot().skeleton_bone_count == 20, "sword arm must retain the actual supplied finger and arm bones")
...
func _test_trial_roundtrip() -> void:
	...
	for iteration in range(2):
		...
		_check(bag.slots == slots and bag.equipment == equipment and player.health == DungeonPlayer.MAX_HEALTH and player.stamina == 100.0, "fixture must preserve selected equipment and bag while restoring health and stamina")
		...
		_check(player.is_timed_interacting() and not player.chest_hands.active and not player.chest_hands.visible and player.chest_equipment_stowed, "trial chest must invoke real timed contact animation and stow carried equipment")
		for side in [-1, 1]:
