...
static func capture_locations(value: String) -> Array[String]:
	var requested := value.strip_edges().split(",", false)
	var locations: Array[String] = []
	for location in requested:
		if location not in ["hideout", "dungeon", "mine"]:
			return locations
	for location in ["hideout", "dungeon", "mine"]:
		if requested.is_empty() or location in requested:
...
static func populate_viewport(viewport: SubViewport) -> void:
	...
	player.name = "ProductionTorchCamera"
	if scene_id == "hideout":
		preload("res://scripts/hideout_ruin_visual.gd").apply_exploration_light(player)
...
func _run() -> void:
	if DisplayServer.get_name() != "embedded":
		...
		quit(2)
		return
	var locations := capture_locations(OS.get_environment("DARK_FANTASY_SCENE_LOCATIONS"))
	if locations.is_empty():
		push_error("DARK_FANTASY_SCENE_LOCATIONS only accepts hideout,dungeon,mine.")
		quit(2)
		return
	...
	manifest["selected_locations"] = locations
	manifest["complete_locations"] = locations.size() == 3 and records.size() == SHOTS.size() and not failed
	if file == null:
		failed = true
	else:
