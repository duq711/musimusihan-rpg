...
func _run() -> void:
	...
	if player:
		...
		_check(player.torch_pivot.find_child("WoodenTorchVisual", true, false) != null, "torch GLB must be mounted to EquippedTorch")
		_check(player.torch_flame.find_child("ClothFlame_0", true, false) is MeshInstance3D, "Unity flipbook torch flame must be mounted")
