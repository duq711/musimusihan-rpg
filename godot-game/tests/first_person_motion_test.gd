...
func _test_actual_hand_contacts_and_interrupts() -> void:
	...
	player._begin_chest_opening()
	player._chest_stow_amount = 0.3
	player._update_viewmodel(1.0 / 60.0)
	_check(player.get_first_person_motion_snapshot().visible_arm_count == 0, "partial chest stowing must hide every carried arm without showing chest-opening hands")
