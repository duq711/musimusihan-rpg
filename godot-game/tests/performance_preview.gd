...
static func create_viewport(size: Vector2i, window_layout: bool = false) -> SubViewport:
	var viewport := SubViewport.new()
	viewport.name = "GameplayPerformanceViewport"
	viewport.size = size
	if window_layout:
		var layout := window_layout_for(size)
		viewport.size = Vector2i(layout.render_size[0], layout.render_size[1])
		viewport.size_2d_override = Vector2i(layout.logical_size[0], layout.logical_size[1])
		viewport.size_2d_override_stretch = true
		viewport.set_meta("performance_window_layout", layout)
...
static func window_layout_for(size: Vector2i) -> Dictionary:
	# A detached Window never creates an OS window. Reproduce the same sizing
	# inputs Main::start reads, including the project's default KEEP aspect.
	var fixture := Window.new()
	fixture.visible = false
	fixture.content_scale_size = Vector2i(int(ProjectSettings.get_setting("display/window/size/viewport_width", 1280)), int(ProjectSettings.get_setting("display/window/size/viewport_height", 720)))
	var mode := str(ProjectSettings.get_setting("display/window/stretch/mode", "disabled"))
	var aspect := str(ProjectSettings.get_setting("display/window/stretch/aspect", "keep"))
	fixture.content_scale_mode = {"disabled": Window.CONTENT_SCALE_MODE_DISABLED, "canvas_items": Window.CONTENT_SCALE_MODE_CANVAS_ITEMS, "viewport": Window.CONTENT_SCALE_MODE_VIEWPORT}[mode]
	fixture.content_scale_aspect = {"ignore": Window.CONTENT_SCALE_ASPECT_IGNORE, "keep": Window.CONTENT_SCALE_ASPECT_KEEP, "keep_width": Window.CONTENT_SCALE_ASPECT_KEEP_WIDTH, "keep_height": Window.CONTENT_SCALE_ASPECT_KEEP_HEIGHT, "expand": Window.CONTENT_SCALE_ASPECT_EXPAND}[aspect]
	fixture.content_scale_factor = float(ProjectSettings.get_setting("display/window/stretch/scale", 1.0))
	fixture.content_scale_stretch = Window.CONTENT_SCALE_STRETCH_INTEGER if ProjectSettings.get_setting("display/window/stretch/scale_mode", "fractional") == "integer" else Window.CONTENT_SCALE_STRETCH_FRACTIONAL
	fixture.size = size
	var physical := SAMPLER.physical_render_size(fixture)
	var logical := Vector2i(fixture.get_visible_rect().size.round())
	var result := {"window_size": [size.x, size.y], "render_size": [physical.x, physical.y], "logical_size": [logical.x, logical.y], "stretch_mode": mode, "stretch_aspect": aspect, "stretch_scale": fixture.content_scale_factor, "detached_window_only": not fixture.is_inside_tree(), "window_composition_and_letterbox_clear_excluded": true}
	fixture.free()
	return result
...
func _run() -> void:
	...
	var window_layout := OS.get_environment("PERFORMANCE_QA_WINDOW_LAYOUT") == "1"
	...
	for size in sizes:
		for scene_id in ["hideout", "mine", "dungeon"]:
			...
			seed(412560)
			var viewport := create_viewport(size, window_layout)
			...
			for scenario in relevant:
				...
				record["render_surface_resolution"] = [viewport.size.x, viewport.size.y]
				record["logical_ui_resolution"] = [viewport.get_visible_rect().size.x, viewport.get_visible_rect().size.y]
				if window_layout:
					record["window_layout"] = viewport.get_meta("performance_window_layout")
...
func _capture_monitor(sandbox: Node, viewport: SubViewport, output: String, file_name: String) -> Dictionary:
	...
	# CanvasLayer rejects nullptr and would retain the soon-freed viewport.
	# Return the hidden canvas to the still-live unhosted root before cleanup.
	overlay.custom_viewport = root
