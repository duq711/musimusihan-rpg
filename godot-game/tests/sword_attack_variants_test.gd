...
func _run() -> void:
	...
	_test_release_and_hold_contract()
	_test_default_cycle()
...
func _test_fixed_variants() -> void:
	for variant: String in VARIANTS:
		...
		_check(player._sword_attack_coordinated and player._sword_direct_entry, "production paired variants must exercise the real direct-entry path")
...
func _test_release_and_hold_contract() -> void:
	# A short tap, a charged release and the unchanged held-button deadline
	# must all pass through the same real cost, hit and recovery checks.
	for hold_time: float in [0.0, 0.74, 1.239]:
		_ready_for_attack()
		player.set_sword_attack_mode("right_diagonal")
		var started := player.begin_sword_attack()
		_check(bool(started.accepted), "forehand release/hold fixture must begin")
		if bool(started.accepted):
			_complete_started_attack("right_diagonal", hold_time, hold_time > 1.0)
	# Without a direct-entry capture the coordinated fallback retains .22s.
	# This process-only fixture leaves the production assets and cache intact.
	_ready_for_attack()
	if bool(player.begin_sword_attack().accepted):
		player._sword_direct_entry = false
		_complete_started_attack("right_diagonal", 0.0)
	else:
		_check(false, "coordinated fallback release fixture must begin")
...
func _complete_started_attack(expected_variant: String, hold_time: float = 0.10, automatic_commit: bool = false) -> void:
	...
	var immediate := paired and expected_variant == "right_diagonal" and player._sword_direct_entry
	_check(is_equal_approx(player._sword_minimum_windup_seconds(), 0.0 if immediate else 0.22), "only coordinated forehand with direct entry may remove the minimum windup; other styles and solo/fallback retain .22s")
	...
	if hold_time >= 0.24:
		_tick(0.24)
		_check(player.combat_state == DungeonPlayer.CombatState.WINDUP and is_zero_approx(player.attack_charge) and player.stamina == stamina_before, "holding through exactly .24s must preserve zero charge and spend nothing")
		_tick(hold_time - 0.24)
	else:
		_tick(hold_time)
	...
	_check(is_equal_approx(player.attack_charge, clampf(hold_time - 0.24, 0.0, 1.0)), "held charge must retain the original time-minus-.24 calculation")
	...
	player.attack_release_requested = not automatic_commit
	var commit_time := hold_time
	if automatic_commit:
		_check(not player.attack_release_requested and hold_time < 1.24, "automatic commit must be reached with the button still held")
		_tick(1.24 - hold_time + 0.000001)
		commit_time = 1.240001
	elif not immediate and hold_time < 0.219:
		...
		_tick(0.0001)
		commit_time += 0.0001
	_check(player.combat_state == DungeonPlayer.CombatState.ACTIVE and is_zero_approx(player.state_time), "release or the 1.24s held deadline must enter the actual active phase at zero; immediate forehand must do so on its first release tick")
	if player.combat_state != DungeonPlayer.CombatState.ACTIVE:
		return
	_check(is_equal_approx(player.attack_charge, clampf(commit_time - 0.24, 0.0, 1.0)), "commit must retain the original charge clock for immediate, delayed and automatic attacks")
