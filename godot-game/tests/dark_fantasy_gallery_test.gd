...
func _run() -> void:
	...
	for entry in entries:
		...
		if id.begins_with("beef_jerky"):
			var meshes: Array[MeshInstance3D] = []
			_collect_meshes(gallery.object_root, meshes)
			_check(meshes.size() == (8 if id in ["beef_jerky_pile", "beef_jerky_variants"] else 1), "jerky selection must contain its actual authored pieces: " + id)
			for mesh in meshes:
				var material := mesh.get_active_material(0) as StandardMaterial3D
				_check(material != null and material.albedo_texture != null and material.normal_enabled and material.normal_texture != null and material.roughness_texture != null, "jerky must retain supplied base color, normal and roughness maps: " + id)
				var bounds := mesh.get_aabb()
				_check(bounds.size.x > 0.06 and bounds.size.x < 0.22 and bounds.size.y > 0.002, "jerky must preserve meter scale and actual thickness: " + id)
				var arrays := mesh.mesh.surface_get_arrays(0)
				_check(arrays[Mesh.ARRAY_TEX_UV].size() == arrays[Mesh.ARRAY_VERTEX].size(), "jerky must preserve texture coordinates on every vertex: " + id)
...
func _test_room_flow(original: ExpeditionInventory, saved: Dictionary, selected_id: String) -> void:
	...
	var feature_id := "beef_jerky_assets" if selected_id.begins_with("beef_jerky") else "dark_fantasy_gallery"
	var matches: Array = room.feature_entries.filter(func(entry: Dictionary) -> bool: return entry.id == feature_id)
	...
	for iteration in range(2):
		...
		if not is_instance_valid(gallery):
			continue
		if feature_id == "beef_jerky_assets":
			_check(gallery.object_id == "beef_jerky_pile" and gallery.selected_view == "top", "jerky trial must immediately open the real pile from above")
		var entries := CATALOG.entries()
