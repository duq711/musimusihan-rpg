...
func _run() -> void:
	...
	var finger_min := INF
	var finger_max := -INF
	var receiving_near := -INF
	var receiving_far := INF
	...
	for i in 431:
		...
		if motion.visible and motion.roll.visible:
			_check(motion.roll.position.distance_to(motion._roll_grip_point()) < 0.001, "roll must follow the posed finger cradle")
		if motion.elapsed >= 0.5 and motion.elapsed <= 1.75:
			var held_board: Vector3 = motion.splint.transform * Vector3(0, -0.006, -0.10)
			_check(held_board.distance_to(motion._finger_point("index2")) < 0.001, "index must support the board until it is seated")
		if motion.elapsed > 2.0 and motion.elapsed < 5.3:
			receiving_near = maxf(receiving_near, motion.left_arm.position.z)
			receiving_far = minf(receiving_far, motion.left_arm.position.z)
			var flex: float = motion.right_arm._active_visual.get_joint_snapshot().angles_radians.index.x
			# Reconstruct the fitted shoulder from the deform root. It must
			# stay at the body instead of orbiting with the detached-looking hand.
			var adapter: Node3D = motion.right_arm._active_visual
			var upper: int = adapter.skeleton.find_bone("upper")
			var fit: Transform3D = adapter.skeleton.get_bone_global_pose(upper) * adapter._rest[upper].affine_inverse()
			var anchor: Vector3 = adapter.to_global(fit * adapter._vec(adapter._data.points.shoulder))
			_check(anchor.distance_to(motion.to_global(Vector3(0.60, -0.60, -0.15))) < 0.001, "working shoulder must remain anchored to body")
			finger_min = minf(finger_min, flex)
			finger_max = maxf(finger_max, flex)
		if motion.support_seated and motion.visible:
			var nearest_surface := INF
			for sample in motion._support_contacts.size():
				var local_surface: Vector3 = motion.splint.transform.affine_inverse() * motion._support_contact(sample)
				_check(local_surface.y <= -0.004, "board must not cut through sleeve along its length")
				nearest_surface = minf(nearest_surface, absf(local_surface.y + 0.006))
			support_gap = maxf(support_gap, nearest_surface)
		...
		for arm: Node3D in [motion.left_arm, motion.right_arm]:
			...
			var sleeve: Vector3 = adapter.get_forearm_frame().origin
	...
	_check(receiving_near - receiving_far > 0.06, "receiving arm must visibly counter-move during wrapping")
	_check(wrist_gap < 0.001, "hands must remain attached to sleeves")
	_check(finger_max - finger_min > 0.35, "working index must visibly release and regrip during winding")
	_check(support_gap < 0.002, "board underside must stay seated on dorsal sleeve throughout winding and pressing")
	print("Wood support maximum surface contact error: ", support_gap)
