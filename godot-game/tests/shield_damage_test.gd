...
func _test_item_instances() -> void:
	...
	_check(bag.equipment.offhand == "round_shield", "the data setter alone must not delete or unequip a zero-condition shield")
...
func _test_blocked_wear() -> void:
	...
	bag.set_equipment_durability("offhand", 12.0)
	...
	_check_stage("low", 7.0)
	bag.set_equipment_durability("offhand", 0.0)
	player._update_viewmodel(0.0)
	_check(bag.equipment.offhand == "round_shield" and not player._has_shield_equipped() and not player.shield_pivot.visible, "a zero-condition data edit must preserve the item but hide and disable its shield use")
	_prepare_guard(0.1, 50.0)
	var timed := player.receive_attack(20.0, _front())
	_check(not bool(timed.get("shield_guard", false)), "zero-condition equipment must not provide timed shield protection")
	bag.set_equipment_durability("offhand", 75.0)
	_prepare_guard(0.1, 50.0)
	var condition_events: Array = []
