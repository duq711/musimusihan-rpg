...
func _run() -> void:
	...
	for family: String in FAMILIES:
		...
		for iteration in range(2):
			...
			await physics_frame
			await physics_frame
			_check(not paused and not room.panel_open and player.health == DungeonPlayer.MAX_HEALTH and player.stamina == 100, "motion trial must resume real play with restored actor: " + family)
...
func _exercise_shield_raise(room: Node3D) -> void:
	...
	var screen := player.camera.unproject_position(grip.global_position) / player.camera.get_viewport().get_visible_rect().size
	_check(screen.x > .70 and screen.x < .90 and screen.y > .85 and screen.y < 1.15, "reference resting sword must occupy the lower-right edge")
	_check(tip.global_position.y > grip.global_position.y + .8, "reference carry must keep the blade upright")
	_check(lowered.origin.x < -.40 and lowered.origin.y < -.60, "resting shield must sit at the lower-left screen edge")
	_check(player._shield_corner_offset().is_zero_approx(), "complete authored carry must not receive a second framing offset")
