...
var _hand_auditor := HAND_AUDIT.new()
...
func _run() -> void:
	...
	print("PLAYER FINGER JOINTS %s: thirty independent local joints, source-localized morphs and weighted supplied anatomy, original nail attachment, limits/reset, bow/chest linkage, real controls and isolated F2/repeat/switch/reset/exit" % ("PASS" if failures.is_empty() else "FAIL"))
...
func _test_independent_joints_and_correctives() -> void:
	...
	for side in [-1, 1]:
		...
		var coverage: Dictionary = _hand_auditor.inspect(arm, "independent joints/%s" % side)
		...
		for digit: String in DIGITS:
			...
			for joint in 3:
				...
				# The supplied neutral anatomy is already curved. These are the
				# additional flexion ranges, separate from the straight greybox.
				var limit_degrees: float = [25.0, 35.0, 45.0][joint] if digit == "thumb" else [60.0, 45.0, 45.0][joint]
				...
				for previous: Vector3 in corrective_centers:
					_check(center.distance_to(previous) > 0.002, "different joints must affect distinct supplied finger sections: " + context)
				corrective_centers.append(center)
				var tip_before := _hand_auditor.sample_nail(arm, digit, true)
				var tip_world_before := _hand_auditor.sample_nail(arm, digit)
				...
				_check(open_skin.size() > 100 and _max_displacement(open_skin, posed) > 0.001, "independent flexion must move actual weighted supplied hand geometry: " + context)
				...
				_check(tip_before.size() >= 12 and _max_displacement(tip_before, _hand_auditor.sample_nail(arm, digit, true)) < 0.00002, "the actual supplied nail must stay rigidly attached to its distal bone: " + context)
				_check(_max_displacement(tip_world_before, _hand_auditor.sample_nail(arm, digit)) > 0.001, "every joint must move its own original supplied nail: " + context)
				...
				_check(_poses_equal(_bone_poses(skeleton), rest) and _max_displacement(open_skin, _skin_points(arm)) < 0.00001, "zero added flexion must restore the actual supplied neutral geometry: " + context)
		...
		for digit: String in DIGITS:
			var expected := Vector3(60.0, 70.0, 80.0) if digit == "thumb" else Vector3(90.0, 110.0, 80.0)
			_check((gray.get_joint_snapshot().limits_radians[digit] as Vector3).is_equal_approx(expected * PI / 180.0), "preserved straight greybox must retain its original joint limits: " + digit)
	stage.queue_free()
	await process_frame
...
func _audit_corrective(arm: Node3D, digit: String, joint: int, context: String) -> Vector3:
	...
	var axial_displacement := 0.0
	var nonradial_displacement := 0.0
	for mesh in _meshes(arm):
		...
		for surface: Dictionary in source.surfaces:
			...
			for vertex in vertices.size():
				...
				# The supplied body can share named influences at finger roots.
				# Vertices with no influence from this digit must remain identical.
				if not has_digit_influence:
					unrelated_maximum = maxf(unrelated_maximum, delta.length())
					# The float32 source-to-canonical export can leave sub-0.1 µm
					# roundoff on an unchanged point; this is far below a skin key.
					if delta.length() > 0.0000001: wrong_digit += 1
				elif not owned and delta.length() >= 0.0000001:
					mixed_vertices_changed += 1
				if delta.length() < 0.000001: continue
				changed += 1
				var local_delta := to_joint.basis * delta
				maximum = maxf(maximum, local_delta.length())
				axial_displacement = maxf(axial_displacement, absf(local_delta.y))
				var radial := Vector3(point.x, 0.0, point.z).normalized()
				nonradial_displacement = maxf(nonradial_displacement, (local_delta - radial * local_delta.dot(radial)).length())
				affected.append(point)
				centroid += joint_frame * point
	# These keys were authored on the supplied source itself. Its rig report
	# records a maximum 14 mm axial support and 0.34 mm radial correction.
	# Measure from the actual imported joint head/axis, including the rolled
	# thumb frame, with only 20 micrometres of export tolerance.
	for point: Vector3 in affected:
		farthest = maxf(farthest, point.length())
		axial_extent = maxf(axial_extent, absf(point.y))
	var axial_limit := 0.01402
	if OS.get_environment("PLAYER_FINGER_JOINTS_QA_DIAGNOSTIC") == "1":
		print("JOINT GEOMETRY DIAGNOSTIC: " + JSON.stringify({"context": context, "changed_vertices": changed, "maximum_delta_m": maximum, "actual_joint_support_radius_m": farthest, "axial_extent_m": axial_extent, "axial_limit_m": axial_limit, "axial_displacement_m": axial_displacement, "nonradial_displacement_m": nonradial_displacement, "unrelated_vertices_changed": wrong_digit, "unrelated_maximum_delta_m": unrelated_maximum, "mixed_vertices_changed": mixed_vertices_changed}))
	_check(changed >= 50 and maximum > 0.00001 and maximum < 0.00036, "each supplied joint must have real, small corrective vertex deltas: " + context + " changed=" + str(changed) + " max=" + str(maximum))
	_check(farthest < 0.04 and axial_extent < axial_limit and wrong_digit == 0 and unrelated_maximum < 0.0000001, "corrective geometry must stay near its actual source joint and preserve vertices with no target-digit weight: " + context + " radius=" + str(farthest) + " axial=" + str(axial_extent) + " axial_limit=" + str(axial_limit) + " unrelated=" + str(wrong_digit) + " unrelated_max=" + str(unrelated_maximum))
	_check(axial_displacement < 0.000002 and nonradial_displacement < 0.000002, "supplied volume correctives must act radially around the real joint instead of sliding skin along the finger: " + context)
