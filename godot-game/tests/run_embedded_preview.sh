...
case "$preview_name" in
	rear_takedown_preview.gd) preview_pass_marker="^REAR_TAKEDOWN_PREVIEW_COMPLETE PASS:" ;;
	dagger_assassination_preview.gd) preview_pass_marker="^DAGGER_ASSASSINATION_PREVIEW_COMPLETE PASS:" ;;
	...
	shield_corner_preview.gd) preview_pass_marker='SHIELD CORNER PREVIEW PASS' ;;
	shield_damage_preview.gd) preview_pass_marker='^SHIELD DAMAGE PREVIEW PASS:' ;;
	shield_shatter_preview.gd) preview_pass_marker='^SHIELD SHATTER PREVIEW PASS:' ;;
	hand_hold_preview.gd) preview_pass_marker='HAND HOLD PREVIEW PASS' ;;
	loot_spawn_preview.gd) preview_pass_marker='^LOOT SPAWN PREVIEW PASS:' ;;
	wooden_torch_preview.gd) preview_pass_marker='WOODEN TORCH PREVIEW PASS' ;;
	torch_anatomy_preview.gd) preview_pass_marker='^TORCH ANATOMY PREVIEW PASS:' ;;
	torch_grip_preview.gd) preview_pass_marker='^TORCH GRIP PREVIEW PASS:' ;;
	hideout_ruin_preview.gd) preview_pass_marker='^HIDEOUT RUIN PREVIEW PASS:' ;;
	hideout_hud_preview.gd) preview_pass_marker='^HIDEOUT HUD PREVIEW PASS:' ;;
	item_detail_preview.gd) preview_pass_marker='^ITEM DETAIL PREVIEW PASS:' ;;
	health_panel_preview.gd) preview_pass_marker='^HEALTH PANEL PREVIEW PASS:' ;;
	hideout_cooking_preview.gd) preview_pass_marker='^HIDEOUT COOKING PREVIEW PASS:' ;;
	distillation_preview.gd) preview_pass_marker='^DISTILLATION PREVIEW PASS:' ;;
	alchemy_preview.gd) preview_pass_marker='^ALCHEMY PREVIEW PASS:' ;;
	sword_shield_choreography_preview.gd) preview_pass_marker='^SWORD SHIELD CHOREOGRAPHY PREVIEW PASS:' ;;
	sword_shield_multiview_preview.gd) preview_pass_marker='^SWORD SHIELD MULTIVIEW PREVIEW PASS:' ;;
	sword_shield_preview.gd) preview_pass_marker='^SWORD SHIELD PREVIEW PASS:' ;;
	first_person_motion_preview.gd) preview_pass_marker='^FIRST PERSON MOTION PREVIEW PASS:' ;;
	...
	player_hands_greybox_preview.gd) preview_pass_marker='^PLAYER HANDS GREYBOX PREVIEW PASS:' ;;
	player_hands_detailed_preview.gd) preview_pass_marker='^PLAYER HANDS DETAILED PREVIEW PASS:' ;;
	player_finger_joints_preview.gd) preview_pass_marker='^PLAYER FINGER JOINTS PREVIEW PASS:' ;;
	*)
...
if [[ "$preview_name" == "rear_takedown_preview.gd" || "$preview_name" == "dagger_assassination_preview.gd" || "$preview_name" == "creep_ragdoll_preview.gd" || "$preview_name" == "creep_execution_preview.gd" || "$preview_name" == "shield_damage_preview.gd" || "$preview_name" == "shield_shatter_preview.gd" || "$preview_name" == "creep_transition_preview.gd" ]]; then
	runner+=(--fixed-fps 30)
