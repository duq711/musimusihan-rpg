...
로컬 `assets/licensed/roger/roger_medival.glb`가 설치·가져오기 되어 있으면 같은 외형·이동 프리뷰가 Roger의 맞춤 전신을 촬영합니다. 매니페스트의 `model`, `model_sha256`, `licensed_character`로 실제 선택된 원본을 구분합니다. 이 로컬 바이너리는 공개 저장소에 포함하지 않습니다. The same previews capture the optional local Roger body when installed and imported, and identify the active asset and hash in their manifests. The licensed binary remains excluded from the public repository.
