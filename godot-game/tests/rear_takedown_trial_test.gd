...
func _test_complete_sequence() -> void:
	...
	_advance_to(MOTION.STAB_HIT + .001)
	_check(creep.health > 0.0 and not creep.dismemberment.severed.has("head") and defeats == prior_defeats, "deep stab leaves target alive with head intact")
	_advance_to((MOTION.HOLD_END + MOTION.CUT_HIT) * .5)
	_check(creep.health > 0.0 and not creep.dismemberment.severed.has("head") and room.loot_count == prior_loot, "early lateral extraction remains alive and awards nothing")
	_advance_to(MOTION.CUT_HIT - .001)
	_check(creep.health > 0.0 and not creep.dismemberment.severed.has("head") and defeats == prior_defeats, "lateral extraction cannot kill before the cutting contact")
	_advance_to(MOTION.CUT_HIT + .001)
	_check(creep.health == 0.0 and creep.ai_state == DungeonEnemy.AIState.DEAD and creep.dismemberment.severed.is_empty() and creep.dismemberment.detached.is_empty() and creep.ragdoll.parts.has("Head"), "actual lateral cut kills with head attached to the physical corpse")
	_check(defeats == prior_defeats + 1 and room.enemies_alive == 0 and room.loot_count == prior_loot + 1 and room.inventory.count_item("rune_fragment") == prior_fragments + 1, "actual room callback awards exactly one death and reward")
...
func _test_pause_cancel_reselect_reset() -> void:
	...
	_check(creep.health == health and not creep.dismemberment.severed.has("head") and room.player.weapon_pivot.transform.is_equal_approx(weapon_pose), "F2 retains the alive held target and blade pose")
	await _press_f2()
	_advance_to((MOTION.HOLD_END + MOTION.CUT_HIT) * .5)
	var loot_before: int = room.loot_count
	room.player.cancel_execution()
	_check(not room.player.is_execution_active() and creep.ai_state != DungeonEnemy.AIState.EXECUTION and creep.health > 0.0, "cancel releases living target from the paired action")
	room.player.advance_execution(5.0)
	_check(not creep.dismemberment.severed.has("head") and room.loot_count == loot_before, "cancelled sequence cannot deliver delayed death or rewards")
