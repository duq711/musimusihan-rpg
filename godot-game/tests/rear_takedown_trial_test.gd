...
func _test_complete_sequence() -> void:
	...
	_check(creep.health > 0.0 and not creep.dismemberment.severed.has("head") and room.loot_count == prior_loot, "withdrawal remains alive and awards nothing")
	_advance_to(MOTION.CUT_HIT - .001)
	_check(creep.health > 0.0 and not creep.dismemberment.severed.has("head") and defeats == prior_defeats, "wide cut windup cannot sever before the cutting contact")
	_advance_to(MOTION.CUT_HIT + .001)
	_check(creep.health == 0.0 and creep.ai_state == DungeonEnemy.AIState.DEAD and creep.dismemberment.severed.has("head"), "final actual cut severs the head and kills")
...
func _test_pause_cancel_reselect_reset() -> void:
	...
	room.player.advance_execution(5.0)
	_check(not creep.dismemberment.severed.has("head") and room.loot_count == loot_before, "cancelled sequence cannot deliver delayed decapitation or rewards")
