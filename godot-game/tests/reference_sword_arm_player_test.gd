...
func _test_combat_sampling(player: DungeonPlayer, fps: int) -> void:
	for variant: String in DATA.ATTACK_CLIPS:
		...
		_check(is_equal_approx(entry_duration, 0.10), "Every cut must use the same 100ms held-to-authored entry.")
	...
	for variant: String in DATA.ATTACK_CLIPS:
		...
		for offset: float in [-0.000001, 0.0, 0.000001]:
			...
			if offset == 0.0:
				_check(not player._uses_direct_sword_entry("active"), "Every cut must finish its direct entry at exactly 100ms: " + variant)
		...
		_check(_near_pose(player.weapon_pivot.transform, boundary_sample.raw_sword), "The shared entry endpoint must restore the exact authored blade pose: " + variant)
		...
		_check(_near_pose(player.weapon_pivot.transform, expected.raw_sword), "Exactly 145ms must use the unmodified authored contact pose: " + variant)
		...
		_check(_near_pose(player.weapon_pivot.transform, DATA.sample(variant, recovery_time)), "Every cut must retain its authored recovery endpoint: " + variant)
		_check_geometry(player, "authored recovery endpoint " + variant)
