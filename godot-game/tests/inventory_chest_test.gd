...
func _test_equipment_to_bag_transactions() -> void:
	...
	_check(overlay.combined_loadout_panel.equipment_buttons.head.is_visible_in_tree(), "Occupied equipment must be reachable on the unified page.")
	_emit_equipment_double_click(overlay, "head")
	...
	overlay._on_equipment_slot_pressed("head")
	_check(not overlay.combined_loadout_panel.equipment_buttons.head.disabled, "Full-bag equipment must remain inspectable and support an unequip attempt.")
	_emit_equipment_double_click(overlay, "head")
	_check(overlay.combined_loadout_panel.status_label.text.contains("가득"), "A rejected full-bag unequip must explain the failure on the visible unified page.")
...
func _test_live_status_readout() -> void:
	...
	_check(overlay.health_chrome.is_visible_in_tree() and overlay.combined_loadout_panel.is_visible_in_tree() and not overlay.status_footer.visible, "Unified inventory must use the new live survival footer alongside equipment.")
	...
	var container_before := container.items.duplicate(true)
	overlay.health_tab_button.pressed.emit()
	_check(overlay.combined_loadout_panel.visible and overlay.container_return_button.visible and not overlay.container_root.visible, "A chest must offer its actual unified character page and a return action.")
	overlay.container_return_button.pressed.emit()
	_check(not overlay.health_tab_active and overlay.container_root.visible and overlay.loot_container == container and container.items == container_before and overlay.inventory_model == inventory, "Returning from the unified page must retain the exact chest and bag without transferring or regenerating contents.")
...
func _check_status_readout(
	...
	if overlay.health_tab_active:
		var meters: Dictionary = overlay.health_chrome.survival_bars
		_check(overlay.combined_loadout_panel.capacity_label.is_visible_in_tree() and overlay.combined_loadout_panel.capacity_label.text.contains("%.1f kg" % inventory.total_weight()), context + " must render the real carried weight beside the backpack.")
		_check(meters.stamina.is_visible_in_tree() and is_equal_approx(meters.stamina.value, expected_stamina) and is_equal_approx(meters.stamina.max_value, expected_max_stamina), context + " must render actual stamina on the unified page.")
		_check(is_equal_approx(meters.hunger.value, expected_hunger) and is_equal_approx(meters.thirst.value, expected_thirst), context + " must render real survival values on the unified page.")
...
func _test_realtime_inventory_and_chest() -> void:
	...
	_check(preload("res://tests/loot_test_helpers.gd").valid_count(game), "realtime dungeon must spawn its visit's selected loot sites")
	if game.loot_chests.is_empty():
		game.queue_free()
		await process_frame
		return

	# Inventory timing and transfer use their own fixed contents; map loot
	# occupancy and supplied-model collision are covered by loot_spawn_test.
	var chest := DungeonLootChest.new().configure("인벤토리 연결 시험", [{"id": "linen_bandage", "quantity": 2}])
	chest.position = Vector3(0.0, 0.0, 7.0)
	chest.open_requested.connect(game._open_container)
	game.add_child(chest)
	game.loot_chests.append(chest)
	game.player.position = chest.position + Vector3(0.0, 1.0, 1.6)
...
func _emit_equipment_double_click(overlay: InventoryOverlay, slot: String) -> void:
	var event := InputEventMouseButton.new()
	event.button_index = MOUSE_BUTTON_LEFT
	event.pressed = true
	event.double_click = true
	overlay.combined_loadout_panel.equipment_buttons[slot].gui_input.emit(event)
	overlay.combined_loadout_panel.equipment_buttons[slot].pressed.emit()
...
func _check_preserved_navigation_tabs(overlay: InventoryOverlay) -> void:
	...
	for expected in preserved_tabs:
		...
		_check(tab.position.x >= expected_position.x and tab.position.y == 3 and tab.get_rect().end.x < overlay._compact_close_button.position.x, "%s must remain ordered and reachable after merging equipment and health navigation" % tab_name)
