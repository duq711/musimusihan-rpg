...
func _run() -> void:
	...
	for scope: String in ["quick_attacks", "quick_attacks_shield"]:
		_check(PREVIEW.sequence_ids_for_scope(scope) == ["right_diagonal", "left_reverse", "overhead"] and PREVIEW.contact_ids_for_scope(scope) == ["right_diagonal", "left_reverse", "overhead"], "all-attack capture must cover three actual sword attacks with no missing selected pose")
	sandbox.begin()
	for variant: String in PREVIEW.sequence_ids_for_scope("quick_attacks"):
	...
	print("REFERENCE SWORD CUT PREVIEW %s: three production attacks after real shield stow, both-hand capture records, input isolation and expedition/source preservation; no pixel claim" % ("PASS" if failures.is_empty() else "FAIL"))
