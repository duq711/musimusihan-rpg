...
func _test_catalog() -> void:
	...
	for entry in room.feature_entries:
		...
		ids[entry.id] = true
		_check(entry.action in ["rear_takedown", "dagger_assassination", "creep_execution", "creep_dismemberment", "creep_ragdoll", "creep", "movement", "performance", "armed", "shield_damage", "wall_equipment", "skeleton", "flail", "camping", "cooking", "hideout_cooking", "hideout_ruin", "stress", "survival_controls", "body_health", "archery", "archery_accuracy", "archery_power", "torch", "inventory", "inventory_details", "player_appearance", "player_arm_motion", "player_hands_greybox", "player_hands_detailed", "player_finger_joints", "reference_sword_motion", "first_person_motion", "dark_fantasy_gallery", "chest", "loot_container", "traps", "ai", "respawn", "extraction", "death", "spell", "learn_books", "needs", "wounded", "time", "cleanse", "condition", "item", "scene", "cave_zone", "smithing", "alchemy", "distilling", "bandage_forearm", "splint_forearm", "potion_drink", "jerky_eat", "timed_item_use", "dungeon_combat_hud", "storage_equipment"], "every entry must have an executable action: %s / %s" % [entry.id, entry.action])
	_check(ids.has("creep_execution") and ids.has("creep_execution:both_legs"), "single and both-leg crawler execution must expose playable F2 fixtures")
	_check(room.has_method("_prepare_creep_execution_trial") and room.has_method("_cancel_creep_execution_trial"), "execution entries must connect to actual setup and cleanup; production cuts, stab timing and session restoration are exercised by creep_execution_trial_test")
	_check(ids.has("rear_takedown") and ids.has("rear_takedown:alerted") and ids.has("rear_takedown:front"), "rear sword takedown must expose unaware, alerted and front trials")
	_check(room.has_method("_prepare_rear_takedown"), "rear takedown entries must prepare the actual healthy enemy and production sword")
	...
	for variant: String in DungeonLootChest.SUPPLIED_MODELS:
		_check(ids.has("loot_container:" + variant), "every supplied container must expose its executable model trial")
	_check(ids.has("loot_spawn:reliquary") and ids.has("loot_spawn:blackwater_cave"), "both production maps need executable randomized-entry trials")
...
func _test_pause_and_recovery() -> void:
	...
	_check(room.player.health == DungeonPlayer.MAX_HEALTH and room.player.stamina == 100, "recovery must refill health and stamina")
...
func _test_reset_and_results() -> void:
	...
	_check(room.panel_open and room.player.health == DungeonPlayer.MAX_HEALTH and room.player.combat_state == DungeonPlayer.CombatState.READY, "R after death must reset this room, not enter the dungeon")
...
func _test_camping_fixture() -> void:
	...
	_check(room.player.health == DungeonPlayer.MAX_HEALTH * 0.5 and room.player.stamina == 20.0 and ExpeditionSession.hunger == 25.0 and ExpeditionSession.thirst == 25.0, "camping fixture must prepare visibly reduced health, stamina and needs")
	...
	_check(room.camp.state == "planning" and paused and is_equal_approx(room.player.health, health_before - 30.0 + 10.0) and is_equal_approx(room.player.stamina, minf(100.0, stamina_before + 70.0)), "completed rest must retain 60 seconds of bleeding, halve its healing under curse and return to paused planning")
	...
	_check(is_equal_approx(room.player.health, health_before - 45.0 + 19.0) and room.camp.warmth == 1, "completed treatment must retain bleeding damage, restore nine health under curse and spend one further warmth")
	...
	_check(room.camp.state == "planning" and is_equal_approx(room.player.health, meal_health - 90.0 + 22.5) and room.player.stamina == 100.0, "completed meal must retain three minutes of bleeding, halve healing under curse and fully restore stamina")
	...
	var interrupted_after_damage: float = room.player.health
	await _press_key(KEY_F2)
	var interrupted_hunger := ExpeditionSession.hunger
	var cancelled_health: float = room.player.health
	room.camp.advance_rest(30.0)
	_check(room.panel_open and paused and room.camp.state == "closed" and is_equal_approx(room.player.health, cancelled_health) and cancelled_health <= interrupted_after_damage and cancelled_health < interrupted_health, "F2 during rest must preserve bleeding received until cancellation without later damage or completion rewards")
...
func _test_cooking_fixture() -> void:
	...
	for recipe_id in CampCookingCatalog.ordered_recipe_ids():
		...
		_check(is_equal_approx(room.player.health, minf(DungeonPlayer.MAX_HEALTH, health_before + float(recipe.health))) and is_equal_approx(room.player.stamina, minf(100.0, stamina_before + float(recipe.stamina))), "finishing must automatically eat the real recipe and grant only its health/stamina effects: " + recipe_id)
...
func _test_stress_fixtures() -> void:
	...
	_check(ExpeditionSession.stress == 65.0 and room.player.health == DungeonPlayer.MAX_HEALTH and ExpeditionSession.active_conditions.is_empty() and room.player.torch_enabled, "audio comparison must prepare healthy, lit, reproducible stress 65")
...
func _test_chest_fixture() -> void:
	...
	for weapon_id in ["rusted_sword", "hunting_bow", "chain_flail", "weathered_staff", ""]:
		...
		_check(not room.player.chest_hands.is_visible_in_tree(), "Chest opening must keep its hand rig hidden.")
		_check(not room.player.chest_hands.active and not room.player.chest_hands.visible and not room.player.chest_hands.left_hand.visible and not room.player.chest_hands.right_hand.visible, "both chest hands must stay inactive and hidden during production opening")
		...
		room.player.advance_timed_interaction(0.5)
		_check(not room.player.chest_hands.active and absf(chest.lid_pivot.rotation.x) > 0.01 and not chest.opened, "the lid must lift without chest hands before completion")
