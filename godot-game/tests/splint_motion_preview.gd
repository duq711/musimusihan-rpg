...
var output := "res://artifacts/visual_qa/splint_forearm/wood_wrap_01"
...
func _run() -> void:
	...
	var player: DungeonPlayer = fixture.player
	var accepted: Dictionary = player.begin_item_use("splint", fixture.inventory)
	...
	var hashes := {}
	for source in ["res://scripts/splint_use_visuals.gd", "res://scripts/bandage_use_visuals.gd", "res://scripts/player.gd", "res://scripts/supplied_fp_arm.gd", "res://scripts/player_arm_visual.gd", "res://assets/3d/player/fp_arms/left.scn", "res://assets/3d/player/fp_arms/right.scn", "res://assets/3d/items/splint/wood_support.glb"]:
		hashes[source] = FileAccess.get_sha256(source)
	var frames: Array = []
	var sparse := OS.get_environment("SPLINT_QA_STILLS") == "1"
	var count := 20 if sparse else 235
	...
	for i in count:
		...
		if picture.save_png(path.path_join(filename)) != OK:
			...
			quit(1)
			return
		frames.append({"file": filename, "time": i * step, "active": player.is_bandage_motion_active(), "stage": player.splint_hands.stage, "winding": player.splint_hands.winding, "pressed": player.splint_hands.pressed, "hands_visible": player.splint_hands.visible, "weapon_visible": player.weapon_pivot.visible, "fracture": ExpeditionSession.condition_affects_part("fracture", "left_arm"), "count": fixture.inventory.count_item("splint")})
		player._update_viewmodel(step)
		player.advance_item_use(step)
	var completed: bool = not player.is_item_use_active() and not ExpeditionSession.condition_affects_part("fracture", "left_arm") and fixture.inventory.count_item("splint") == 2
	viewport.queue_free()
	await process_frame
	sandbox.finish()
	...
	print("SPLINT MOTION PREVIEW %s: %d production frames" % ["PASS" if preserved and accepted.accepted and completed else "FAIL", count])
