...
func _run() -> void:
	...
	for id in ["rusted_longsword", "weathered_round_shield", "iron_cage_torch", "weathered_staff", "wall_torch"]:
		...
		match id:
			...
			"iron_cage_torch":
				_check(object.find_child("WoodenTorchVisual", true, false) != null, "torch uses supplied rustic wood model")
				_check(object.find_child("ClothFlame_0", true, false) is MeshInstance3D, "real production flame remains connected")
			"weathered_staff":
