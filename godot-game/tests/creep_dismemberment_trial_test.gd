...
const REGIONS: Array[String] = ["left_arm", "right_arm", "left_leg", "right_leg", "head", "distributed"]
...
func _run() -> void:
	...
	var coverage := "five localized cuts, distributed-hit comparison, living limb loss, single head-death reward, real timers and F2 pause, reselection/cancel/reset, session restoration" if asset_checks_executed else "catalog, absent-asset guidance and session restoration; actual cuts SKIPPED"
...
func _test_trial(region: String) -> void:
	...
	var first_region := "left_arm" if region == "distributed" else region
	...
	_check(completed, "second real hit completes the trial: " + region)
	if not completed:
		room._show_test_panel()
		return
	...
	if region == "head":
		...
		_check(creep.ai_state != DungeonEnemy.AIState.DEAD and is_equal_approx(creep.health, creep.max_health - 36.0) and room.enemies_alive == 1, "limb loss or distributed hits preserve the damaged live enemy: " + region)
		_check(creep.is_physics_processing() and room.enemy_ai_enabled, "real AI resumes after the two hits: " + region)
