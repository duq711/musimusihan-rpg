...
사용자는 방패를 내구도 상·중·하 세 단계의 실제 파손 형태로 만들고 1인칭에서도 그 차이가 보이게 요청했다. 상 단계는 기존 원본을 보존하고 중·하 단계는 손잡이·팔 연결을 유지한 새 파손 메시를 쓴다. 구현 판단으로 방패 개체별 최대 내구도 100, 현재/최대 비율 2/3 초과는 상·1/3 초과는 중·나머지는 하로 둔다. 실제 막힌 타격의 피해×0.25를 소모하며 저스트 가드도 포함한다. 0은 하 단계에 포함하고 자동 삭제·네 번째 파괴 상태·방어 성능 약화를 추가하지 않는다. 기존 완전 방어·기력·적 스턴 규칙은 유지한다. 수치는 조정 가능한 구현값이며 이번 요청이 수리 시스템이나 다른 장비 내구도까지 확정한 것은 아니다. F2의 세 외형과 실제 마모 시험을 연결하고 자동 검사 6종을 통과했다. 최종 Mac GPU `shield_damage_20260919_02`에서 비교·전환 PNG 16장과 8초·240프레임을 직접 검토했다. 촬영의 실제 40 피해 다섯 번을 막아 75→65→55→45→35→25로 마모되고 가드·건강·파지와 원정·커서를 유지했다. 기존 검사 2종의 실패는 수정 전과 같으며 새 통과 근거와 구분한다. 영상 파일 검증과 GitHub 확인은 최종 기록을 따른다. [현재 구현·검수 상태](../docs/SHIELD_DAMAGE.md).

The user requested real high/medium/low shield damage, visible in first person. High retains the original model; medium/low use new damaged meshes with existing grip/arm attachment. Implementation choices are a default per-instance maximum of 100, high above two-thirds condition, medium above one third and low otherwise. Actual blocked hits consume damage × 0.25 condition, including just guard. Zero remains low without automatic deletion, a fourth destroyed stage or reduced defensive performance. Existing full blocking, stamina and attacker-stun rules remain. Values are tunable implementation decisions, not approval of repairs or durability for other gear. Three appearance trials and actual wear are connected to F2, and six suites passed. Final Mac GPU `shield_damage_20260919_02` inspection covers 16 comparison/transition PNGs and eight seconds/240 frames. Five actual blocked 40-damage hits in the capture produce 75→65→55→45→35→25 while preserving guard, health, grip and expedition/cursor state. Two legacy failures match the unchanged baseline and remain separate from passing evidence. Video-file validation and GitHub confirmation follow the final record. See the current implementation and validation guide.
